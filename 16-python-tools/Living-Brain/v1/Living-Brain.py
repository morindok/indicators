...
# -*- coding: utf-8 -*-
"""
🧬 مغز زنده — نسخه 0.11 (تک‌فایلی)
ژنوم تریدر برتر | سوپرهیومن | مدیریت لحظه‌ای معاملات | کنترل ترس/طمع | درک عددی/بصری فوق‌پیشرفته
...
def _benchmark(seconds=0.25):
    try:
        n=256; a=np.random.rand(n,n).astype(np.float32); b=np.random.rand(n,n).astype(np.float32)
...
def _classify(g,c):
    if g<15 or c<=2: return "ضعیف",0.5,10
    if g<60: return "متوسط",1.0,15
    if g<150: return "قوی",2.0,20
    return "سرور",3.0,25
@dataclass
...
def profile_hardware():
    ...
    gf=_benchmark(); tier,scale,ticks=_classify(gf,cores)
    if mem<4: scale*=0.6
    elif mem>=16: scale*=1.1
    scale=min(scale,4.0)
    workers=max(2,min(6,phys or cores//2))
    parallel=cores>=6 and scale>=1.0
...
config.PROJECT_NAME="مغز زنده"; config.VERSION="0.11-apex"
config.SUBTITLE="ژنوم تریدر برتر | سوپرهیومن | کنترل ترس و طمع"
config.TICKS_PER_SECOND=config.HW.ticks
config.DT=1.0/config.TICKS_PER_SECOND
config.NEURON_SCALE=config.HW.scale*3.0
config.MAX_NEURONS=30000
...
config.SCAN_DEEP_LIMIT=8
config.SCAN_PERIOD=3.0
config.SCAN_RESCAN_SEC=6
config.SCAN_INTERVAL="15"
config.SCAN_KLINE_LIMIT=150
TRADING_MODES={
    "swing":{"label":"🎯 سویینگ (لوریج ۱۰)","leverage":10,"tp_mult":3.0,"sl_mult":1.4,
             "conviction_threshold":0.66,"decision_interval":6,"use_geometry":True,
             "margin_fraction":0.6,"max_daily":10,"cost_aware":True},
    "scalp":{"label":"⚡ اسکالپ (لوریج ۲۰)","leverage":20,"tp_mult":1.0,"sl_mult":0.7,
             "conviction_threshold":0.50,"decision_interval":2,"use_geometry":False,
             "margin_fraction":0.4,"max_daily":60,"cost_aware":True},
}
config.HISTORY_WINDOW=400
config.MAX_THOUGHTS_SHOWN=25
config.DASH_INTERVAL_MS=600
config.DEFAULT_SPEED=2
...
class MemoryDB:
    def __init__(self,path):
        ...
        print(f"💾 دیتابیس حافظه مغز: {path}")
    def _init(self):
        with self.lock:
...
class RawPriceAnalyzer:
    ...
    def analyze(self,o,h,l,c,key=""):
        ...
        if is_doji: signal*=0.5; reasons.append("بی‌تصمیمی")
        ...
        return {"ready":True,"price":float(c[-1]),"signal":float(signal),
                ...
                "is_doji":bool(is_doji),"bull_rejection":bool(bull_rejection),
                "bear_rejection":bool(bear_rejection),"reasons":reasons}
...
# ═══════════════════════ ۹) درک عددی فوق‌پیشرفته ═══════════════════════
class AdvancedNumericalPerception:
    """قیمت را به‌عنوان یک ساختار ریاضی زنده ادراک می‌کند: مشتقات، فیبوناچی، هارمونیک."""
    def __init__(self,numerical_iq,superhuman_factor):
        self.numerical_iq=numerical_iq
        self.shf=superhuman_factor
        self.price_history=deque(maxlen=512)
    ...
    def derivatives(self):
        """سرعت، شتاب و جرک (تغییر شتاب) قیمت."""
    ...
    def fib_structure(self):
        """میزان انطباق قیمت فعلی با سطوح فیبوناچی."""
        if len(self.price_history)<20:
            return {"fib_alignment":0.0,"golden_present":False,"levels":[]}
        p=list(self.price_history)[-100:]
        hi=max(p); lo=min(p); rng=hi-lo
        if rng<=0: return {"fib_alignment":0.0,"golden_present":False,"levels":[]}
        ...
        return {"fib_alignment":alignment,"golden_present":golden_present,"levels":levels}
    def harmonic_score(self):
        """چقدر حرکت قیمت ریتمیک و هارمونیک است (خودهمبستگی مشتقات)."""
        if len(self.velocity_history)<16: return 0.0
        try:
            arr=np.array(list(self.velocity_history)[-32:])
            arr=arr-arr.mean()
    ...
    def perceive(self,price):
        self.feed(price)
        d=self.derivatives()
        fib=self.fib_structure()
        harm=self.harmonic_score()
...
# ═══════════════════════ ۱۰) درک بصری فوق‌پیشرفته ═══════════════════════
class VisualPerception:
    """الگوهای کندلی را به‌صورت گشتالت ادراک می‌کند."""
    def __init__(self,visual_iq,superhuman_factor):
        self.visual_iq=visual_iq
        self.shf=superhuman_factor
        self.candles=deque(maxlen=128)
    def feed(self,o,h,l,c):
        self.candles.append((float(o),float(h),float(l),float(c)))
    def detect_patterns(self):
        if len(self.candles)<3:
            return {"pattern":"none","strength":0.0,"bias":0,"patterns":[]}
        ...
        return {"pattern":pattern,"strength":clamp(strength,0,1),"bias":bias,"patterns":patterns}
    def perceive(self,o,h,l,c):
        self.feed(o,h,l,c)
        return self.detect_patterns()

# ═══════════════════════ ۱۱) تنظیم‌گر احساسات (کنترل ترس/طمع) ═══════════════════════
class EmotionalRegulator:
    """ترس و طمع را پایش و با انضباط ژنتیکی مهار می‌کند."""
    ...
    def update(self,drawdown=0.0,volatility=0.0):
        ...
        self.fear=clamp(self.fear-decay)
        self.greed=clamp(self.greed-decay)
    ...
    def fear_dominant(self): return self.fear>0.5
    def greed_dominant(self): return self.greed>0.5

# ═══════════════════════ ۱۲) مدیر لحظه‌ای معاملات ═══════════════════════
class ActivePositionManager:
    """هر لحظه معاملات را بازبینی می‌کند: تریلینگ، قفل سود، قطع ضرر، خروج اضطراری."""
    ...
    def manage(self,pos,tk,regulator,analysis=None):
        ...
        # ۱) تریلینگ‌استاپ: قفل سود در حال رشد (جلوگیری از از دست رفتن سود)
        if r_mult>=1.0:
            trail_r=min(r_mult-0.5,r_mult*0.6)
            if side>0:
                new_sl=entry+trail_r*initial_risk
                if new_sl>pos["sl"]: pos["sl"]=new_sl; pos["trailed"]=True
            else:
                new_sl=entry-trail_r*initial_risk
                if new_sl<pos["sl"]: pos["sl"]=new_sl; pos["trailed"]=True
        # ۲) کنترل طمع: سود بزرگ ولی مومنتوم در حال بازگشت → قفل سود
        greed_threshold=2.5-regulator.greed*0.5
        if r_mult>=greed_threshold and momentum_aligned<0:
            return ("close","قفل سود (ضد طمع)")
        # ۳) مدیریت ترس: ضرر + مومنتوم شدیداً علیه → قطع زودهنگام (صبر آستانه می‌دهد)
        if r_mult<-0.3 and momentum_aligned<-0.4:
            patience_buffer=-0.3-self.patience*0.3
            if r_mult<patience_buffer:
                return ("close","قطع ضرر زودهنگام (ضد ترس)")
        # ۴) خروج اضطراری نزدیک حد ضرر
        if r_mult<=-0.9:
...
class VitalitySystem:
    """تنفس، سرزندگی و آدرنالین — به ارگانیسم حیات می‌دهد."""
    def __init__(self):
...
class TradingEngine:
    ...
    def _init_fresh(self):
        ...
        self.daily_trades=0; self.daily_date=time.strftime("%Y-%m-%d")
        self.lesson_stats={}
    def _load_from_db(self):
        state=self.db.load_engine_state(self.name)
        if state:
            caps=state["slot_capitals"]
            for i in range(min(self.n_slots,len(caps))):
                self.slots[i]["capital"]=caps[i]
        ...
        for r in closed:
            r["mode"]=self.name; r["exit"]=r.get("exit_price")
        ...
        if open_pos or state or self.lesson_stats:
            print(f"🔄 [{self.name}] بازیابی: {len(open_pos)} باز، {len(closed)} بسته، {len(self.lesson_stats)} آموخته")
    ...
    def record_lesson(self,reasons,pnl):
        for r in reasons:
            if r not in self.lesson_stats:
                self.lesson_stats[r]={"wins":0,"losses":0,"pnl":0.0}
    ...
    def top_lessons(self,n=5):
        ...
        for r,st in self.lesson_stats.items():
            ...
            items.append((r,wr,total,st["pnl"]))
        items.sort(key=lambda x:-x[3])
        return items[:n]
    def _reset_day(self):
        today=time.strftime("%Y-%m-%d")
        if today!=self.daily_date: self.daily_date=today; self.daily_trades=0
    def assign_coin(self,coin):
        with self.lock:
            if coin in self.coin_to_slot: return self.coin_to_slot[coin]
            for i,s in enumerate(self.slots):
                if s["coin"] is None:
                    s["coin"]=coin; self.coin_to_slot[coin]=i; return i
            return None
    ...
    def total_margin_used(self):
        return sum(s["margin_used"] for s in self.slots)
    def open_position(self,coin,side,price,spread,margin,leverage,tp,sl,conviction,reasons,tick,mode):
        with self.lock:
            ...
            avail=self.available_margin(coin)
            ...
            pos={"coin":coin,"side":side,"entry":entry,"size":size,"margin":margin,
                 ...
                 "peak":entry,"trailed":False}
            self.open_positions[coin]=pos
            self.db.save_open_position(self.name,pos)
            self._persist_state()
            return pos
    def live_pnl(self,pos,bid,ask):
        side=pos["side"]
        cur=bid if side>0 else ask
        ...
        return {"gross":gross,"net":net,"roi":roi_on_margin,"cur":cur,"total_fees":total_fees}
    ...
    def close_position(self,coin,exit_price,reason):
        with self.lock:
            ...
            self.record_lesson(pos.get("reasons",[]),net)
            self._persist_state()
            return net
    ...
    def settle_on_reconnect(self,http):
        ...
        for coin in list(self.open_positions.keys()):
            ...
            self.close_position(coin,exit_price,reason)
            settled+=1
        return settled
    def reset(self):
        with self.lock:
            self.db.reset_engine(self.name)
            self._init_fresh()
            self._persist_state()
    def equity(self,unrealized):
        return sum(s["capital"] for s in self.slots)+unrealized
    def win_rate(self):
    ...
    def stats(self,unrealized):
        ...
        base=self.slot_margin*self.n_slots
...
GENE_CATALOG=[
    ...
    # 🧬 ژن‌های تریدر برتر (آپکس)
    ("GEN-DISC","انضباط آهنین","discipline",0.85),
    ("GEN-RISKMG","مدیریت ریسک برتر","risk_management",0.85),
    ("GEN-EMOCTL","کنترل احساسات","emotional_control",0.85),
    ("GEN-PATIENCE","صبر شکارچی","patience",0.80),
    ("GEN-PRECISION","دقت اسنایپر","precision",0.85),
    ("GEN-NUMIQ","هوش عددی برتر","numerical_iq",0.90),
    ("GEN-VISIQ","هوش بصری برتر","visual_iq",0.90),
    ("GEN-ADAPT","سازگاری سریع","adaptability",0.80),
    ("GEN-CALM","آرامش تحت فشار","calm",0.85),
    ("GEN-FORESIGHT","آینده‌نگری","foresight",0.80),
]
...
TRADER_GENE_KEYS=["discipline","risk_management","emotional_control","patience","precision",
                  "numerical_iq","visual_iq","adaptability","calm","foresight"]
@dataclass
class Gene:
...
class Genome:
    ...
    def build_phenotype(self):
        ...
        apex=[p.discipline,p.risk_management,p.emotional_control,p.numerical_iq,
              p.visual_iq,p.precision,p.calm,p.foresight]
        p.superhuman_factor=1.0+(sum(apex)/len(apex))
        return p
    def describe(self):
...
def prime_apex_trader(genome):
    """🧬 پرایمینگ ژنوم به‌عنوان تریدر برتر: متیلاسیون حداقلی برای ژن‌های آپکس."""
    for key in TRADER_GENE_KEYS:
        symbol=KEY_TO_SYMBOL.get(key)
        if symbol and symbol in genome.epigenome.methyl:
            genome.epigenome.methyl[symbol]=0.02
...
REG_ORDER=["BRAINSTEM","THALAMUS","AMYGDALA","HIPPOCAMPUS",
           "VISUAL_CORTEX","NUMERICAL_CORTEX","BASAL_GANGLIA","CORTEX"]
...
class Brain:
    def __init__(self,phenotype,rng,scale=None):
        ...
        self.eeg=0.0
        self._pool=(ThreadPoolExecutor(max_workers=config.N_WORKERS) if config.PARALLEL_BRAIN else None)
    ...
    def _gather_target(self,dst):
        ...
        for b in self.syn[dst]:
            ...
            w=b["w"]*(self._exc_gain if b["w"][0]>=0 else self._inh_gain)
            np.add.at(Rt.I,b["post"][sp],w[sp])
    def step(self,sensory,neurochem,quantum_events,fatigue,plasticity_factor=1.0,
             ...
             market_signal=0.0,sniper_focus=0.0,num_depth=0.0,vis_strength=0.0):
        ...
        regs["CORTEX"].add_current(None,3.6*L["acetylcholine"])
        regs["NUMERICAL_CORTEX"].add_current(None,3.0*abs(market_signal)+4.0*sniper_focus+3.0*num_depth)
        regs["VISUAL_CORTEX"].add_current(None,2.0*abs(market_signal)+3.0*sniper_focus+3.0*vis_strength)
        if numeric_input:
            regs["NUMERICAL_CORTEX"].add_current(None,2.2*numeric_input.get("conviction",0))
        if cardiac_afferent>0.001:
            regs["BRAINSTEM"].add_current(None,2.2*cardiac_afferent)
            regs["AMYGDALA"].add_current(None,1.6*cardiac_afferent)
...
class CardiacSystem:
    ...
    def step(self,dt,sympathetic,parasympathetic,stretch,o2):
        fib_mod=self._fib_modulation()
        target_hr=self.base_hr*(1+0.5*sympathetic-0.3*parasympathetic)*fib_mod
        target_hr=clamp(target_hr,30,220)
        ...
        self.icns_activity=0.85*self.icns_activity+0.15*clamp(sympathetic+stretch,0,1)
...
class Body:
    ...
    def feed_information(self,novelty):
        ...
        return feed
    def regulate(self,autonomic,env_state,tick):
        ...
        if env_state.get("food_event"):
            self.glucose=clamp(self.glucose+25,20,160)
            self.glycogen=clamp(self.glycogen+0.10)
    ...
    def evolve(self,amount):
        self.evolution_level=clamp(self.evolution_level+amount,0.0,1.0)
    def vitals(self):
    ...
    def hormones_fa(self):
        return {HORMONES_FA[k]:float(v) for k,v in self.hormones.items()}

# ═══════════════════════ ۲۲) آگاهی ═══════════════════════
STATE_LABELS=[(25,"هوشیار"),(50,"متمرکز"),(72,"بیدارِ کامل"),(90,"خودآگاه"),(101,"شهودیِ اسنایپر")]
class ConsciousnessMeter:
    def __init__(self,n_regions,window=160):
        self.window=window
        self.history=deque(maxlen=window)
        self.arousal=0.6; self.integration=self.differentiation=0.0
        self.phi=self.workspace=0.0; self.level=40.0
    def update(self,activities,brainstem,body_ok_factor,quantum_coh,info_level=0.5,wakefulness=1.0):
        vec=np.array(list(activities.values()),dtype=float)
        self.history.append(vec)
        self.arousal=ema(self.arousal,clamp(0.45+brainstem*2.5+0.3*wakefulness),0.15)
        if len(self.history)>=24:
            ...
            if np.all(std>1e-6):
                ...
                self.integration=ema(self.integration,clamp(0.5+0.5*float(C[iu].mean())),0.1)
        p=vec+1e-4; p=p/p.sum()
        ent=-float(np.sum(p*np.log(p)))/math.log(max(2,len(p)))
        self.differentiation=ema(self.differentiation,ent,0.1)
        self.phi=ema(self.phi,self.integration*self.differentiation,0.1)
        x=(0.45*activities.get("CORTEX",0)+0.25*activities.get("THALAMUS",0)
           +0.15*activities.get("HIPPOCAMPUS",0)+0.15*activities.get("BASAL_GANGLIA",0))
        self.workspace=ema(self.workspace,1.0/(1.0+math.exp(-55.0*(x-0.075))),0.12)
        raw=100.0*clamp(0.42*self.workspace+0.22*self.arousal+0.20*self.phi+0.16*self.integration)
        raw*=body_ok_factor*(0.85+0.15*quantum_coh)*(0.85+0.25*info_level)
        raw=max(raw,25.0)
        self.level=ema(self.level,raw,0.08)
        return self.level
    ...
    def body_ok_factor_from(self,body):
        f=1.0
        if body.blood_o2<60: f*=0.45
        elif body.blood_o2<80: f*=0.75
        if body.glucose<45: f*=0.6
        if body.fatigue>0.85: f*=0.85
        return f

# ═══════════════════════ ۲۳) ذهن (زنده و متمرکز بر مارکت) ═══════════════════════
CATEGORY_FA={"market":"تحلیل مارکت","trade":"معامله‌گری","insight":"بینش",
             "memory":"حافظه معاملاتی","emotion":"احساس کنترل‌شده","self":"خودآگاهی",
             "quantum":"کوانتومی","heart":"قلب"}
CATEGORY_COLORS={"market":"#38bdf8","trade":"#4ade80","insight":"#fde047","memory":"#a78bfa",
                 "emotion":"#f472b6","self":"#f8fafc","quantum":"#22d3ee","heart":"#fb7185"}
T={"market":[
        "📊 ساختار قیمت را بررسی می‌کنم؛ {obs}",
        "📈 الگوی کندل‌ها را تحلیل می‌کنم؛ {obs}",
        "🎯 نقاط کلیدی حمایت و مقاومت را رصد می‌کنم.",
        "📐 سطوح فیبوناچی را روی چارت تطبیق می‌دهم.",
        "🔍 در حال اسکن {n} ارز برای بهترین موقعیت هستم.",
        "🧮 مشتقات قیمت را محاسبه می‌کنم: سرعت، شتاب، جرک."],
    "trade":[
        "💼 یک موقعیت معاملاتی را ارزیابی می‌کنم.",
        "💼 ریسک به ریوارد این معامله را می‌سنجم.",
        "💼 بهترین نقطه ورود را محاسبه می‌کنم."],
    "insight":[
        "💡 الگویی در مارکت یافتم که با تجربه‌های قبلی هم‌خوانی دارد.",
        "💡 شهودم می‌گوید این ساختار قیمت آشناست.",
        "📐 یک نقطه اسنایپر روی سطح طلایی دیده می‌شود.",
        "🧮 ارزش موردانتظار این معامله مثبت است."],
    "memory":[
        "💾 تجربه‌ی معامله‌ای قبلی را به یاد آوردم.",
        "💾 آموخته‌هایم را مرور می‌کنم؛ {lesson}",
        "💾 این سناریو شبیه یک معامله‌ی گذشته است."],
    "emotion":[
        "🧘 ترسم را حس می‌کنم ولی با انضباط مهارش می‌کنم.",
        "🧘 طمع را می‌شناسم و سود را در نقطه منطقی قفل می‌کنم.",
        "🧘 احساساتم را تماشا می‌کنم؛ اجازه نمی‌دهم تصمیم بگیرند.",
        "🧘 آرامشم را زیر فشار حفظ می‌کنم."],
    "self":[
        "من هستم و مارکت را تماشا می‌کنم.",
        "تمرکزم کاملاً روی جریان قیمت است."],
...
class StreamOfConsciousness:
    ...
    def step(self,tick,cons,activities,neurochem,body,sensory,quantum_events,
             ...
             emotional_state="آرام"):
        self.mood=neurochem.mood_index
        p_think=0.030+0.16*cons.workspace*cons.arousal+0.06*body.information_level
        ...
        weights={
            ...
            "self":0.4,
            "quantum":0.4 if quantum_events else 0.05,
            "heart":0.3,
        }
        keys=list(weights)
        ...
        return t
    def _compose(self,cat,qevents,market_signal,n_scanned,top_lesson):
        if cat=="quantum":
            tmpl=T["quantum"][int(self.rng.integers(0,len(T["quantum"])))]
            return tmpl.format(bits=qevents[0].bits if qevents else "…")
        if cat=="market":
            tmpl=T["market"][int(self.rng.integers(0,len(T["market"])))]
            return tmpl.format(obs=self._market_obs(market_signal),n=n_scanned)
        if cat=="memory":
            tmpl=T["memory"][int(self.rng.integers(0,len(T["memory"])))]
            lesson=top_lesson if top_lesson else "هنوز درسی ثبت نشده"
            return tmpl.format(lesson=lesson)
        tmpl=T[cat][int(self.rng.integers(0,len(T[cat])))]
        return tmpl
...
class Organism:
    ...
    def set_mode(self,mode):
        if mode in TRADING_MODES:
            self.mode=mode; self.mode_params=TRADING_MODES[mode]
    def rebirth(self,seed=None):
        self.tick=0
        self.seed=seed if seed is not None else int(np.random.randint(1,2**31-1))
        self.rng=np.random.default_rng(self.seed)
        self.genome=Genome(self.seed)
        prime_apex_trader(self.genome)  # 🧬 پرایم به‌عنوان تریدر برتر
        ...
        self.focus_geometry={}
        self.last_num_perception={}
        self.last_vis_perception={}
        self.milestones,self._ms_flags=[],set()
        self._add_milestone(f"🧬 متولد شد — ژنوم تریدر برتر {self.genome.signature()} | "
                            f"سوپرهیومن ×{self.phenotype.superhuman_factor:.2f}")
    ...
    def _make_signal(self,coin,tk,an,geom,energy,mp):
        if not an.get("ready"): return None
        price=tk["price"]; spread=tk["spread"]
        sig=an["signal"]
        # ⚡ ضریب سوپرهیومن روی دقت سیگنال
        shf=self.phenotype.superhuman_factor
        conviction=clamp(abs(sig)*(0.5+0.5*energy)*(1+0.3*self.phenotype.intuition)
                         *(0.8+0.2*shf),0,1)
    ...
    def _is_rational(self,sig,mp):
        ...
        cost_pct=spread_pct+2*self.engine.fee_rate
    ...
    def _trading_step(self):
        ...
        snap=self.scanner.snapshot()
        if not snap.get("connected"): return
        tickers=snap["tickers"]; deep=snap["deep"]
        energy=self.body.energy
        self._assign_slots(deep,tickers)
        # ساخت دیکشنری تحلیل‌ها برای مدیریت لحظه‌ای
        analyses={}
        for coin,d in deep.items():
            analyses[coin]=d.get("analysis")
        # ── مدیریت لحظه‌ای پوزیشن‌های باز ──
        for coin in list(self.engine.open_positions.keys()):
            ...
            if reason:
                ...
                continue
            # ۲) مدیریت فعال (تریلینگ/قفل سود/قطع ضرر/اضطراری)
        ...
        for coin,d in deep.items():
            ...
            geom=self.geometry.analyze(d["klines"]["o"],d["klines"]["h"],d["klines"]["l"],d["klines"]["c"])
            sig=self._make_signal(coin,tk,an,geom,energy,mp)
            if sig:
                sig["geometry"]=geom
                signals.append(sig)
        ...
        # ⚖️ تنظیم‌گر احساسات اندازه پوزیشن و آستانه را تعدیل می‌کند
        ...
        for sig in signals:
            ...
            if avail<5: continue
            margin=min(avail*mp["margin_fraction"]*size_mult,avail)
            ...
            if pos:
                ...
                self._add_milestone(f"🎯 [{self.mode}] شکار {coin} {'🟢' if pos['side']>0 else '🔴'} "
                                    f"لوریج{mp['leverage']} @ {pos['entry']:.4f}")
                break
    def _on_closed(self,pos,net,reason,exit_price):
        outcome="win" if net>0 else "lose"
        side_txt="buy" if pos["side"]>0 else "sell"
        self.memory_db.store(self.tick,pos["coin"],side_txt,pos["entry"],exit_price,net,outcome)
        # 🧘 بازخورد احساسی
        ...
        if net>0:
            ...
            self.genome.epigenome.adapt("learning",-0.0003)
    ...
    def _trade_thought_open(self,pos):
        ...
        t=Thought(self.mind.next_id,self.tick,"trade",text,pos["conviction"],
                  ["NUMERICAL_CORTEX","BASAL_GANGLIA"])
    ...
    def step(self):
        ...
        self._trading_step()
        snap_tickers=self.scanner.snapshot().get("tickers",{}) if self.scanner else {}
        ...
        q_events=self.quantum.step(config.DT,config.BASE_DECOHERENCE,config.ACTIVITY_DECOHERENCE,acts_prev)
        ...
        sniper_focus=self.current_signals[0]["conviction"] if self.current_signals and self.current_signals[0].get("sniper") else 0.0
        # 🔢👁️ درک عددی و بصری فوق‌پیشرفته از قیمت کانونی
        focus=self.focus_symbol or (self.current_signals[0]["symbol"] if self.current_signals else None)
        num_depth=0.0; vis_strength=0.0
        if focus and focus in snap_tickers:
            price=snap_tickers[focus]["price"]
            self.last_num_perception=self.num_perception.perceive(price)
            num_depth=clamp(self.last_num_perception.get("fib_alignment",0)
                            +self.last_num_perception.get("harmonic",0)*0.5,0,1)
        snap_deep=self.scanner.snapshot().get("deep",{}) if self.scanner else {}
        if focus and focus in snap_deep:
            kl=snap_deep[focus]["klines"]
            self.last_vis_perception=self.vis_perception.perceive(kl["o"][-1],kl["h"][-1],kl["l"][-1],kl["c"][-1])
        ...
        out=self.brain.step(sensory,neuro,q_events,self.body.fatigue,
                            ...
                            sniper_focus=sniper_focus,num_depth=num_depth,vis_strength=vis_strength)
        ...
        level=self.cons.update(out["activities"],out["activities"].get("BRAINSTEM",0),
                               ...
                               wakefulness=self.body.wakefulness)
        # 🧘 به‌روزرسانی تنظیم‌گر احساسات با drawdown و نوسان
        unreal=self._unrealized(snap_tickers)
        eq=self.engine.equity(unreal)
        self.peak_equity=max(self.peak_equity,eq)
        drawdown=safe_div(self.peak_equity-eq,self.peak_equity,0)
        avg_vol=clamp(np.mean([s.get("volatility",0) for s in self.current_signals]) if self.current_signals else 0,0,1)
        ...
        n_scanned=self.scanner.snapshot().get("n_symbols",0) if self.scanner else 0
    ...
    def _check_milestones(self,q_events,thought):
        ...
        if self.cons.level>88: once("c88","🔮 وضعیت شهودیِ اسنایپر فعال شد")
    ...
    def snapshot(self):
        ...
        vit=getattr(self,"last_vitality",{"breath":0,"vitality":0.9,"adrenaline":0})
        return {
            ...
            "hist_consciousness":self.hist_cons.last(200),"eeg":self.hist_eeg.last(200),
            "regions":acts,
            ...
            "equity_history":self.hist_equity.last(300),
            "mode":self.mode,
            "mode_params":self.mode_params,
            "focus_symbol":self.focus_symbol,
            "focus_geometry":self.focus_geometry,
            ...
            "emotional_state":self.regulator.state,
            "vitality":vit.get("vitality",0.9),"breath":vit.get("breath",0),
            "adrenaline":vit.get("adrenaline",0),
            # 🔢 درک عددی/بصری
            "num_perception":self.last_num_perception,
            "vis_perception":self.last_vis_perception,
        }
...
def build_focus_chart(s,scanner_snap):
    ...
    if pos:
        ...
        fig.add_hline(y=pos["sl"],line_color="#f87171",line_width=1.5,
                      annotation_text="SL",annotation_font_size=10,annotation_font_color="#f87171")
    mood=s.get("mood",0.5)
    state=s.get("state_label","")
    mode_fa="سویینگ" if s.get("mode")=="swing" else "اسکالپ"
    emo=s.get("emotional_state","")
    fig.update_layout(template="plotly_dark",paper_bgcolor="rgba(0,0,0,0)",
                      plot_bgcolor="rgba(2,6,23,0.35)",
                      ...
                      title={"text":f"🧠 {focus} | {state} | {emo} | حالت {mode_fa}","x":0.5,
...
main_tab=dcc.Tab(label="🧠 داشبورد زنده",value="tab-main",className="tab",
    ...
    html.Div(className="grid",children=[
        html.Div(className="panel glow s4",children=[
            html.H3([html.Span(className="dot"),"سطح آگاهی (بیدارِ کامل)"]),
            dcc.Graph(id="gauge",config={"displayModeBar":False},style={"height":230}),
            html.Div(id="cons-detail",style={"fontSize":12,"color":"#94a3b8"}),
        ]),
        html.Div(className="panel s5",children=[
            html.H3([html.Span(className="dot"),"روند آگاهی"]),
            dcc.Graph(id="trend",config={"displayModeBar":False},style={"height":260}),
        ]),
        html.Div(className="panel s3",children=[
            html.H3([html.Span(className="dot"),"🧘 کنترل احساسات و سرزندگی"]),
            html.Div(id="emotion-panel"),
        ]),
    ],style={"marginBottom":14}),
    html.Div(className="grid",children=[
        html.Div(className="panel s5",children=[
            html.H3([html.Span(className="dot"),"جریان تفکرات (متمرکز بر مارکت)"]),
        ...
        html.Div(className="panel s4",children=[
            ...
            dcc.Graph(id="regions",config={"displayModeBar":False},style={"height":300}),
        ]),
        html.Div(className="panel s3",children=[
    ...
    html.Div(className="grid",children=[
        ...
        html.Div(className="panel s4",children=[
            html.H3([html.Span(className="dot"),"🧬 ژنوم تریدر برتر"]),
            dcc.Graph(id="genes",config={"displayModeBar":False},style={"height":350}),
        ]),
        html.Div(className="panel s4",children=[
...
signals_tab=dcc.Tab(label="📊 سیگنال‌ها و معاملات",value="tab-signals",className="tab",
    ...
    html.Div(className="grid",children=[
        html.Div(className="panel glow s8",children=[
            html.H3([html.Span(className="dot-green"),"📈 چارت زنده — نگاهِ مغز به مارکت"]),
    ...
    html.Div(className="grid",children=[
        html.Div(className="panel s7",children=[
            html.H3([html.Span(className="dot-green"),"💼 پوزیشن‌های باز (مدیریت لحظه‌ای)"]),
    ...
    html.Div(className="grid",children=[
        html.Div(className="panel s7",children=[
            html.H3([html.Span(className="dot-gold"),"📜 معاملات بسته‌شده اخیر"]),
            html.Div(id="closed-table",style={"maxHeight":260,"overflowY":"auto"}),
        ]),
        html.Div(className="panel s5",children=[
            html.H3([html.Span(className="dot-gold"),"📚 آموخته‌ها و درک عددی/بصری"]),
...
def render_emotion_panel(s):
    ...
    return html.Div([
        html.Div(f"وضعیت: {state}",style={"fontWeight":"bold","color":state_color,"fontSize":14,"marginBottom":10}),
        ...
        html.Div(className="emo-meter",children=[
            html.Div(f"⚡ آدرنالین: {adrenaline*100:.0f}٪",style={"color":"#fb923c","marginBottom":2}),
            html.Div(className="bar-bg",children=[
                html.Div(className="bar-fg",style={"width":f"{adrenaline*100:.0f}%","background":"#fb923c"})])]),
        html.Div(f"⚡ ضریب سوپرهیومن: ×{s.get('superhuman_factor',1):.2f}",
                 style={"color":"#a78bfa","fontWeight":"bold","marginTop":8}),
    ])
def render_mode_info(s):
    ...
    children=[
        ...
        html.Div(f"آستانه اطمینان: {int(mp['conviction_threshold']*100)}٪ | مارجین هر اسلات: {config.SLOT_MARGIN:.0f}$"),
...
def render_signals(signals):
    if not signals:
        return html.Div("در حال اسکن… منتظر نقطه شکار",style={"fontSize":12,"color":"#94a3b8"})
    cards=[]
    for s in signals:
        ...
        sniper_badge=html.Span("🎯 اسنایپر",style={"color":"#fbbf24","fontWeight":"bold","fontSize":11}) if s.get("sniper") else ""
        lesson_badge=""
        la=s.get("lesson_adj",0)
        if abs(la)>0.02:
            lesson_badge=html.Span(f"📚 {la:+.2f}",style={"color":"#a78bfa","fontSize":10})
        cards.append(html.Div(className="sig-card",
            ...
            html.Div(className="sig-head",children=[
                ...
                sniper_badge,lesson_badge,
                html.Span(f"اطمینان {int(s['conviction']*100)}٪",
...
def render_positions(positions):
    ...
    header_row=html.Tr([html.Th("نماد"),html.Th("جهت"),html.Th("لوریج"),html.Th("ورود"),html.Th("فعلی"),
                        html.Th("TP"),html.Th("SL"),html.Th("مارجین"),html.Th("تریل"),
                        html.Th("PnL زنده"),html.Th("ROI٪")])
    rows=[]
    for p in positions:
        ...
        rows.append(html.Tr([
            ...
            html.Td(fmt_num(p["sl"],4),style={"color":"#f87171"}),
            html.Td(f"{p['margin']:.0f}$"),
            html.Td(trail_txt,style={"color":"#4ade80"}),
...
def render_closed(closed):
    ...
    header_row=html.Tr([html.Th("نماد"),html.Th("جهت"),html.Th("حالت"),html.Th("لوریج"),
                        html.Th("ورود"),html.Th("خروج"),html.Th("دلیل"),html.Th("PnL خالص")])
    rows=[]
    for t in closed:
        net=t.get("net_pnl",0)
        ...
        rows.append(html.Tr([
            ...
            html.Td(side_txt),
            html.Td(mode_fa),
            html.Td(f"×{t['leverage']}"),
            html.Td(fmt_num(t["entry"],4)),
            html.Td(fmt_num(exit_price,4)),
...
def render_lessons(s):
    ...
    num=s.get("num_perception",{})
    vis=s.get("vis_perception",{})
    items=[]
    if num:
        items.append(html.Div(f"🔢 عمق درک عددی: {num.get('depth',0):.2f} | "
                              f"انطباق فیب: {num.get('fib_alignment',0)*100:.0f}٪ | "
                              f"هارمونیک: {num.get('harmonic',0)*100:.0f}٪ | "
                              f"{'🌟 سطح طلایی فعال' if num.get('golden_present') else ''}",
                              style={"fontSize":11,"color":"#7dd3fc","marginBottom":6}))
    if vis:
        items.append(html.Div(f"👁️ الگوی بصری: {vis.get('pattern','none')} | "
                              f"قدرت: {vis.get('strength',0)*100:.0f}٪",
                              style={"fontSize":11,"color":"#7dd3fc","marginBottom":6}))
    if not lessons:
        items.append(html.Div("— هنوز درسی آموخته نشده —",
...
@app.callback(
    [Output("gauge","figure"),Output("trend","figure"),Output("eeg","figure"),
     Output("regions","figure"),Output("hormones","figure"),Output("genes","figure"),
    ...
    State("store-paused","data"),State("store-speed","data"),State("mode-selector","value"))
def update(n,paused,speed,mode):
    try:
        return _update_impl(n,paused,speed,mode)
    except Exception:
        import traceback; traceback.print_exc()
        return [no_update]*23
def _update_impl(n,paused,speed,mode):
    if mode: ORGANISM.set_mode(mode)
    if not paused:
        for _ in range(int(speed or config.DEFAULT_SPEED)):
            ORGANISM.step()
    s=ORGANISM.snapshot()
    ...
    gauge=go.Figure(go.Indicator(
        ...
        gauge={"axis":{"range":[0,100],"tickcolor":"#94a3b8"},"bar":{"color":"#8b5cf6"},
               "steps":[{"range":[0,25],"color":"#1e1b4b"},{"range":[25,50],"color":"#312e81"},
                        {"range":[50,72],"color":"#4338ca"},{"range":[72,100],"color":"#6d28d9"}]}))
    ...
    gnames=[g[0] for g in s["genes"]][::-1]
    gexpr=[g[1]*100 for g in s["genes"]][::-1]
    gmeth=[g[2] for g in s["genes"]][::-1]
    genes=go.Figure(go.Bar(x=gexpr,y=gnames,orientation="h",
                           marker_color=["#fbbf24" if "GEN-" in "" or True else "#64748b" for _ in gmeth],
                           text=[f"{v:.0f}٪" for v in gexpr],textposition="outside"))
    genes=go.Figure(go.Bar(x=gexpr,y=gnames,orientation="h",
                           marker_color=["#22d3ee" if m<0.3 else "#64748b" for m in gmeth],
                           text=[f"{v:.0f}٪" for v in gexpr],textposition="outside"))
    style_fig(genes)
    genes.update_layout(xaxis=dict(range=[0,110]),margin=dict(l=140,r=34,t=20,b=28))
    cards=[]
    ...
    chips=[
        ...
        html.Span(f"🧬 {s['genome_sig']}",className="chip"),
        ...
        html.Span(f"🧘 {s['emotional_state']}",className="chip "+("chip-ok" if "آرام" in s["emotional_state"] else "chip-gold")),
    ...
    cons_detail=html.Div([
        ...
        html.Div(f"فضای کار: {s['workspace']:.2f} | برانگیختگی: {s['arousal']:.2f} | خلق: {s['mood']:.2f}"),
    ...
    scanner_status=html.Div(
        ...
        f"کارمزد: {config.TAKER_FEE*100:.3f}٪ | 📚 یادگیری فعال",
        style={"color":"#94a3b8"})
    focus_chart=build_focus_chart(s,scanner_snap)
    return [gauge,trend,eeg,regions,hormones,genes,thoughts_children,
            vitals_children,chips,milestones,cons_detail,
            ecg_fig,fib_heart,emotion_panel,
...
def toggle_pause(n,paused): return not paused
@app.callback(Output("store-speed","data"),Input("speed","value"))
def set_speed(v): return v or config.DEFAULT_SPEED
...
def reset_memory(n,mode):
    ...
    try:
        ...
        print("🗑️ ریست حافظه معاملات انجام شد (هر دو موتور)")
        return f"🗑️ حافظه معاملات هر دو موتور ریست شد — {config.RESET_COOLDOWN_TICKS} تیک استراحت"
    except Exception as e:
        import traceback; traceback.print_exc()
...
def interact(*args):
    ...
    if trig=="btn-rebirth":
        ORGANISM.rebirth()
        return html.Span("🧬 موجود جدید با ژنوم تریدر برتر متولد شد.",style={"color":"#a78bfa"})
...
if __name__=="__main__":
    ...
    print(f"💵 هر موتور: {config.INITIAL_CAPITAL}$ بین {config.N_SLOTS} ارز | ژنوم تریدر برتر")
