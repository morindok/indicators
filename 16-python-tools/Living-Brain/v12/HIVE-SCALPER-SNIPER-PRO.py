...
def init_db():
    ...
    # اضافه کردن فیلدهای جدید در صورت نبودن (بدون حذف اطلاعات قبلی)
    try: c.execute("ALTER TABLE trades ADD COLUMN stop_loss REAL")
    except: pass
    try: c.execute("ALTER TABLE trades ADD COLUMN take_profit REAL")
    except: pass
    try: c.execute("ALTER TABLE trades ADD COLUMN strategy TEXT")
    except: pass
    try: c.execute("ALTER TABLE trades ADD COLUMN screenshot TEXT")
    except: pass
    try: c.execute("ALTER TABLE trades ADD COLUMN emotion TEXT")
    except: pass
...
def save_trade(t):
    ...
    c.execute("""INSERT INTO trades
        (symbol,side,entry,size_usd,qty,leverage,liq_price,entry_time,status,reason,votes,regime,features,partials,trail_stop,stop_loss,take_profit,strategy,screenshot,emotion)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
        (t["symbol"], t["side"], t["entry"], t["size_usd"], t["qty"], t["leverage"], t["liq_price"],
         ...
         t.get("trail_stop"), t.get("stop_loss"), t.get("take_profit"), t.get("strategy"), 
         t.get("screenshot"), t.get("emotion")))
    conn.commit()
    tid = c.lastrowid
    conn.close()
    return tid
...
def get_closed_trades(limit=150):
    conn = sqlite3.connect(DB_PATH)
    conn.row_factory = sqlite3.Row
    rows = conn.execute("SELECT * FROM trades WHERE status='closed' ORDER BY id DESC LIMIT ?", (limit,)).fetchall()
    conn.close()
    return [_parse_row(r) for r in rows]
...
class HivePro:
    ...
    def decide(self, feat, regime):
        ...
        dominant_strategy = max(agreeing, key=lambda x: abs(x["score"]) * x["w"])["role"]
        return {
            "side": side, "conv": conv, "votes": {v["org"]: v for v in votes},
            "reason": f"HIVE[{len(agreeing)}] {reason}", "regime": regime,
            "strategy": f"Hive {dominant_strategy}"
        }
    ...
    def calc_size(self, conv, atr_pct, regime, opens):
        ...
        risk *= vol_mult
        free = max(0.0, self.capital - sum(t.get("size_usd",0) for t in opens))
    ...
    def try_open(self, sym, ticker, decision, feat, regime):
        with self._lock:
            ...
            rp = REGIME_PARAMS.get(regime, REGIME_PARAMS["RANGING"])
            sl_price = entry * (1 - rp["sl"]) if decision["side"] == "long" else entry * (1 + rp["sl"])
            tp_price = entry * (1 + rp["tp"]) if decision["side"] == "long" else entry * (1 - rp["tp"])
            trail = entry * (1 - rp["sl"] * 1.1) if decision["side"] == "long" else entry * (1 + rp["sl"] * 1.1)
            
            emotion = "Confident 🚀" if decision["conv"] > 0.75 else "Calculated 🤖" if decision["conv"] > 0.5 else "Cautious ⚖️"
            ...
            t = {
                ...
                "stop_loss": sl_price, "take_profit": tp_price, "strategy": decision.get("strategy", "Hive Base"),
                "screenshot": "Auto-Capture 📸", "emotion": emotion
            }
            tid = save_trade(t)
    ...
    def manage_positions(self):
        ...
        for t in opens:
            ...
            # PnL دلاری با لوریج
            remaining_size = t["size_usd"] * (1 - sum(p.get("frac", 0) for p in t.get("partials", [])))
    ...
    def equity(self):
        ...
        for t in opens:
            ...
            remain = 1.0 - sum(p.get("frac", 0) for p in t.get("partials", []))
            unreal += pp * t.get("size_usd", 0) * remain * LEVERAGE - t.get("size_usd", 0) * remain * COMMISSION * 2
...
app.layout = html.Div([
    ...
    dcc.Tabs(id="tabs", value="live", children=[
        ...
        dcc.Tab(label="لیست معاملات (ژورنال)", value="history", style={"background": CARD, "color": TXT}, selected_style={"background": LINE, "color": GOLD}),
...
def live_tab():
    ...
    if opens:
        ...
        for t in opens:
            ...
            if t["side"] == "long":
                ...
                pp = (t["entry"] - last) / t["entry"]
            remain = 1.0 - sum(p.get("frac", 0) for p in t.get("partials", []))
            pnl = pp * t.get("size_usd", 0) * remain * LEVERAGE - t.get("size_usd", 0) * remain * COMMISSION * 2
            color = UP if pnl >= 0 else DN
            rows.append(html.Tr([
                html.Td(t["id"]), html.Td(t["symbol"]),
                html.Td(t["side"].upper(), style={"color": UP if t["side"]=="long" else DN}),
                html.Td(t.get("regime", "")), html.Td(f"{t['entry']:.5f}"), html.Td(f"{last:.5f}"),
                html.Td(f"${t.get('size_usd', 0):.1f}"), html.Td(f"{pnl:+.2f}", style={"color": color, "fontWeight": "bold"}),
                html.Td(f"{t.get('trail_stop', 0):.5f}" if t.get("trail_stop") else "—", style={"fontSize": 11}),
                html.Td(f"{len(t.get('partials', []))}", style={"fontSize": 11}),
            ]))
        table = dbc.Table([
    ...
    wr = (sum(1 for t in closed if t.get("pnl") and t["pnl"] > 0) / len(closed) * 100) if closed else 0
...
def history_tab():
    trades = get_open_trades() + get_closed_trades(150)
    if not trades: return html.Div("هیچ داده‌ای در تاریخچه موجود نیست.", style={"color": MUT})
    rows = []
    
    def fmt_time(ts_str):
        if not ts_str: return "—"
        try: return datetime.fromisoformat(ts_str).strftime("%H:%M  %d %b")
        except: return ts_str

    for t in sorted(trades, key=lambda x: x["id"], reverse=True):
        st = t.get("status", "")
        # رفع خطای NoneType با جایگزینی 0 برای PNL هایی که هنوز ثبت نشده‌اند
        pnl = t.get("pnl") or 0.0 
        
        bg_col = "rgba(22, 160, 133, 0.1)" if pnl > 0 and st == "closed" else ("rgba(231, 76, 60, 0.1)" if st == "closed" else "transparent")
        
        rows.append(html.Tr([
            html.Td(t.get("id")), 
            html.Td(fmt_time(t.get("entry_time"))), 
            html.Td(fmt_time(t.get("exit_time"))),
            html.Td(t.get("symbol"), style={"fontWeight": "bold"}),
            html.Td(t.get("side", "").upper(), style={"color": UP if t.get("side")=="long" else DN}),
            html.Td(f"${t.get('size_usd', 0):.1f}"),
            html.Td(f"{t.get('entry', 0):.4f}"), 
            html.Td(f"{t.get('exit_price', 0):.4f}" if t.get("exit_price") else "Active"),
            html.Td(f"{t.get('stop_loss', 0):.4f}", style={"color": DN}), 
            html.Td(f"{t.get('take_profit', 0):.4f}", style={"color": UP}),
            html.Td(f"{pnl:+.2f}", style={"color": UP if pnl >= 0 else DN, "fontWeight": "bold"}),
            html.Td(t.get("strategy", "—"), style={"color": ACCENT}), 
            html.Td(t.get("regime", "—")),
            html.Td(t.get("emotion", "—"), style={"fontSize": 12}),
            html.Td((t.get("reason") or "").split(" | ")[0] + "...", style={"fontSize": 10, "maxWidth": "150px", "overflow": "hidden"}),
            html.Td(html.A("نمایش", href="#", style={"color": NEON}) if t.get("screenshot") else "—"),
        ], style={"background": bg_col}))

    table = dbc.Table([
        html.Thead(html.Tr([html.Th(x) for x in ["ID","Entry Time","Exit Time","Symbol","Side","Size","Entry","Exit","Stop Loss","Take Profit","P/L","Strategy","Market Regime","Emotion","Reason","Screenshot"]])),
        html.Tbody(rows)
    ], bordered=True, hover=True, size="sm", style={"background": CARD, "fontSize": "12px", "whiteSpace": "nowrap"})

    return html.Div([
        html.H5("ژورنال کامل معاملات (Trade List)", style={"color": GOLD}),
