...
# -*- coding: utf-8 -*-
"""
🧠 مغز زنده — نسخه 0.22 (تک‌فایلی کامل)
۱۲۰۰۰ حس زنده | تحلیل عددی | قلب عصبی | حواس جمع ۱۰۰٪ | تریل‌استاپ
...
def _benchmark(seconds=0.15):
    try:
        n=192; a=np.random.rand(n,n).astype(np.float32); b=np.random.rand(n,n).astype(np.float32)
...
def profile_hardware():
    ...
    gf=_benchmark()
    if gf<15 or cores<=2: tier,scale,ticks="ضعیف",0.5,10
    elif gf<60: tier,scale,ticks="متوسط",1.0,15
    elif gf<150: tier,scale,ticks="قوی",2.0,20
    else: tier,scale,ticks="سرور",3.0,25
    if mem<4: scale*=0.6
    elif mem>=16: scale*=1.1
    scale=min(scale,4.0)
    workers=max(2,min(8,phys or cores//2))
    parallel=cores>=8
...
config.PROJECT_NAME="مغز زنده"; config.VERSION="0.22-live-sense"
config.SUBTITLE="۱۲۰۰۰ حس زنده | تحلیل عددی | قلب عصبی | حواس جمع ۱۰۰٪"
config.TICKS_PER_SECOND=config.HW.ticks
config.DT=1.0/config.TICKS_PER_SECOND
config.NEURON_SCALE=config.HW.scale*(1.9+min(config.HW.cores,16)*0.20)
config.MAX_NEURONS=min(48000,8000+config.HW.cores*2200)
config.PARALLEL_BRAIN=config.HW.parallel
config.N_WORKERS=config.HW.workers
config.N_QUANTUM_REGISTERS=8; config.N_QUBITS_PER_REG=8
...
config.SCAN_SYMBOL_LIMIT=150
config.SCAN_DEEP_LIMIT=14
config.SCAN_PERIOD=3.0
config.SCAN_RESCAN_SEC=4
...
# 🌌 ۱۲۰۰۰ حس زنده: ۵ حس پایه × ۸۰۰ + ۸۰۰۰ حس فرازمینی
config.SENSORS_PER_SENSE=800
config.N_XENO_SENSES=8000

# 🎯 قدرت حس‌ها و حواس جمع
config.SENSE_POWER=100.0
config.FULL_ATTENTION=True

# 🛡️ فیلترهای دفاعی برای کاهش ضرر
config.MAX_SPREAD_PCT={"swing":0.0025,"scalp":0.0012}
config.MIN_SENSE_ABS=0.35
config.MIN_SENSE_COHERENCE=0.65
config.MIN_REGIME_CONF=0.55
config.MIN_ANTICIPATION=0.45
config.MIN_NUMERIC_SCORE=0.35
config.MIN_CARDIAC_SWING=0.55
config.MIN_CARDIAC_SCALP=0.70
config.DAILY_LOSS_LIMIT_PCT=3.0
config.LOSS_COOLDOWN_TICKS=120
config.CONSENSUS_MIN=0.72

TRADING_MODES={
    "swing":{"label":"🎯 سویینگ (لوریج ۱۰)","leverage":10,"tp_mult":3.0,"sl_mult":1.4,
             "conviction_threshold":0.38,"decision_interval":4,"use_geometry":True,
             "margin_fraction":0.6,"max_daily":15,"cost_aware":True,
             "cost_margin":1.15,"ev_margin":0.20,
             "trail_start_r":1.0,"trail_keep_ratio":0.6},
    "scalp":{"label":"⚡ اسکالپ (لوریج ۲۰)","leverage":20,"tp_mult":1.3,"sl_mult":0.9,
             "conviction_threshold":0.26,"decision_interval":2,"use_geometry":False,
             "margin_fraction":0.4,"max_daily":100,"cost_aware":True,
             "cost_margin":1.05,"ev_margin":0.08,
             "trail_start_r":0.6,"trail_keep_ratio":0.5},
}
    ...
    def save_open_position(self,engine,pos):
        with self.lock:
            ...
            c.execute("""INSERT INTO positions (engine,coin,side,entry,size,margin,leverage,notional,
                tp,sl,conviction,reasons,commission,spread_cost,open_time,open_ts,open_tick,slot_id,status)
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?, 'open')""",
            ...
            conn.commit(); conn.close()
    def update_position_sl(self,engine,coin,new_sl):
        with self.lock:
            conn=sqlite3.connect(self.path); c=conn.cursor()
            c.execute("UPDATE positions SET sl=? WHERE engine=? AND coin=? AND status='open'",
                      (new_sl,engine,coin))
            conn.commit(); conn.close()
...
class RawPriceAnalyzer:
    ...
    def analyze(self,o,h,l,c,key=""):
        ...
        if velocity>0.3: reasons.append("شتاب صعودی")
        elif velocity<-0.3: reasons.append("شتاب نزولی")
        signal+=trend_strength*0.30
        if trend_strength>0.5: reasons.append("روند صعودی پایدار")
        elif trend_strength<-0.5: reasons.append("روند نزولی پایدار")
        if is_strong_bull: signal+=0.25; reasons.append("کندل قوی صعودی")
        if is_strong_bear: signal-=0.25; reasons.append("کندل قوی نزولی")
        if bull_rejection: signal+=0.20; reasons.append("ریجکشن از پایین")
        if bear_rejection: signal-=0.20; reasons.append("ریجکشن از بالا")
        if pos_in_range>0.9: signal-=0.15; reasons.append("نزدیک مقاومت")
        elif pos_in_range<0.1: signal+=0.15; reasons.append("نزدیک حمایت")
        signal=clamp(signal,-1.5,1.5)
        return {"ready":True,"price":float(c[-1]),"signal":float(signal),
                "conviction":clamp(abs(signal),0,1),"velocity":float(velocity),
                "acceleration":float(acceleration),"trend_strength":float(trend_strength),
                "volatility":float(volatility),"resistance":resistance,"support":support,
                "pos_in_range":float(pos_in_range),
                "is_strong_bull":bool(is_strong_bull),"is_strong_bear":bool(is_strong_bear),
                "bull_rejection":bool(bull_rejection),"bear_rejection":bool(bear_rejection),
                "reasons":reasons}
...
# ═══════════════════════ ۹.۵) 🔢 تحلیل عددی زنده ═══════════════════════
class LiveNumericalAnalyst:
    """
    🔢 ارگانیسم باید عدد بفهمد:
    مومنتوم، شتاب، Z-Score، OBV، حجم، آنتروپی، هارمونیک، فیبوناچی و overextension.
    """
    def __init__(self): pass
    def analyze(self, kl, key=""):
        try:
            o,h,l,c,v = kl["o"], kl["h"], kl["l"], kl["c"], kl.get("v")
        except Exception:
            return {"ready": False}
        ...
        ret = np.diff(c[-61:]) / np.maximum(c[-61:-1], 1e-12)
        mom1 = safe_div(c[-1] - c[-2], c[-2])
        mom5 = safe_div(c[-1] - c[-6], c[-6]) if len(c) >= 6 else 0.0
        mom20 = safe_div(c[-1] - c[-21], c[-21]) if len(c) >= 21 else 0.0
        ma20 = float(np.mean(c[-20:])); sd20 = float(np.std(c[-20:])) + 1e-9
        zscore = (float(c[-1]) - ma20) / sd20
        vol20 = float(np.std(ret[-20:])) if len(ret) >= 20 else 0.0
        vol60 = float(np.std(ret[-60:])) if len(ret) >= 60 else vol20
        vol_ratio = vol20 / (vol60 + 1e-9)
        volume_ratio = 1.0; volume_dir = 0.0; obv_slope = 0.0
        if v is not None and len(v) >= 20:
            vma = float(np.mean(v[-20:]))
            volume_ratio = float(v[-1]) / (vma + 1e-9)
            price_up = c[-1] >= o[-1]
            volume_dir = (volume_ratio - 1.0) * (1.0 if price_up else -1.0)
            direction = np.where(c[-20:] >= o[-20:], 1.0, -1.0)
            obv = np.cumsum(direction * v[-20:])
            x = np.arange(len(obv))
            denom = float(np.mean(np.abs(obv))) + 1e-9
            try: obv_slope = float(np.polyfit(x, obv, 1)[0]) / denom
            except Exception: obv_slope = 0.0
        last_n = min(8, len(c))
        bull = int(np.sum(c[-last_n:] >= o[-last_n:])); bear = last_n - bull
        micro_imbalance = (bull - bear) / max(1, last_n)
        entropy = 0.5
        if len(ret) > 10:
            hist, _ = np.histogram(ret[-50:], bins=12)
            p = hist / (hist.sum() + 1e-9) + 1e-9
            entropy = float(-np.sum(p * np.log(p))) / np.log(12)
        harmonic = 0.0
        if len(ret) > 12:
            r = ret[-32:] - np.mean(ret[-32:])
            if np.std(r) > 1e-12:
                try: harmonic = float(np.corrcoef(r[:-1], r[1:])[0, 1])
                except Exception: harmonic = 0.0
        hi = float(h[-100:].max()); lo = float(l[-100:].min()); rng = hi - lo
        fib_align = 0.0
        if rng > 1e-9:
            levels = [hi, hi - 0.382 * rng, hi - 0.5 * rng, hi - 0.618 * rng, lo]
            min_dist = min(abs(c[-1] - lvl) / rng for lvl in levels)
            fib_align = clamp(1.0 - min_dist * 8.0, 0.0, 1.0)
        overext = clamp(abs(zscore) / 3.0, 0.0, 1.0)
        raw = (0.28 * np.tanh(mom1 * 150.0) + 0.22 * np.tanh(mom5 * 40.0)
               + 0.18 * np.tanh(mom20 * 12.0) + 0.16 * obv_slope + 0.10 * volume_dir
               + 0.10 * micro_imbalance + 0.08 * fib_align + 0.06 * harmonic - 0.10 * overext)
        score = float(np.tanh(raw))
        direction = 1 if score > 0 else -1 if score < 0 else 0
        confidence = clamp(abs(score) * (0.6 + 0.4 * (1.0 - entropy))
                           * clamp(1.0 / max(vol_ratio, 1e-6), 0.5, 1.5), 0.0, 1.0)
        if score > 0.2: reason = "عدد صعودی"
        elif score < -0.2: reason = "عدد نزولی"
        else: reason = "عدد خنثی"
        return {"ready":True,"score":score,"direction":direction,"confidence":confidence,
                "zscore":float(zscore),"mom1":float(mom1),"mom5":float(mom5),"mom20":float(mom20),
                "vol20":vol20,"vol_ratio":float(vol_ratio),"volume_ratio":float(volume_ratio),
                "obv_slope":float(obv_slope),"micro":float(micro_imbalance),"entropy":float(entropy),
                "harmonic":float(harmonic),"fib":float(fib_align),"overext":float(overext),"reason":reason}

# ═══════════════════════ ۱۰) 🌌 آرایه حسی ۱۲۰۰۰ + فضازمان ═══════════════════════
...
class XenoSensoryArray:
    """
    🌌 آرایه حسی فرازمینی — ۱۲۰۰۰ حس زنده
    ۵ حس پایه × ۸۰۰ + ۸۰۰۰ حس فرازمینی = ۱۲۰۰۰
    قدرت همه حس‌ها ۱۰۰٪ است.
    """
    WINDOWS=[5,10,20,40,60]
    TRANSFORMS=["raw","diff","accel","rank","entropy","curvature","spread"]
    def __init__(self,rng,sensors_per_sense,n_alien):
        self.rng=rng
        labels=[]; directory=[]
        for sense in XENO_SENSE_TYPES:
            labels.extend([sense]*sensors_per_sense)
            directory.extend([f"{sense} — گیرنده {i+1}" for i in range(sensors_per_sense)])
        for i in range(n_alien):
            base_name = XENO_ALIEN_SENSE_NAMES[i % len(XENO_ALIEN_SENSE_NAMES)]
            labels.append(f"✨{base_name}")
            directory.append(f"✨{base_name} — گیرنده {i+1}")
        self.sense_labels=np.array(labels)
        self.sense_directory=directory
        self.total=len(labels)
        self.window_idx=rng.integers(0,len(self.WINDOWS),self.total)
        self.transform_idx=rng.integers(0,len(self.TRANSFORMS),self.total)
        self.gain=np.clip(rng.normal(1.0,0.12,self.total),0.5,1.6)
        self.bias=rng.normal(0.0,0.03,self.total)
        self.activation=np.zeros(self.total)
        self.sense_power=np.full(self.total, float(config.SENSE_POWER))
        self.sensory_signal=0.0; self.sensory_coherence=0.0
        self.activation_history=deque(maxlen=128)
        self.signal_history=deque(maxlen=64)
        print(f"🌌 آرایه حسی فرازمینی: {self.total} حس زنده فعال | قدرت: {config.SENSE_POWER:.0f}٪")
    def _transforms_for_window(self,w):
        mean_abs=np.mean(np.abs(w))+1e-9
        vals=np.zeros(len(self.TRANSFORMS))
        vals[0]=abs(w[-1]-w[0])/mean_abs
        d=np.diff(w)
        vals[1]=np.mean(np.abs(d))/mean_abs if len(d)>0 else 0.0
        vals[2]=np.mean(np.abs(np.diff(d)))/mean_abs if len(d)>1 else 0.0
        vals[3]=np.argsort(np.argsort(w))[-1]/len(w)
        dd=np.abs(d); s=dd.sum()
        if s>1e-9:
            p=dd/s; vals[4]=float(-np.sum(p*np.log(p+1e-9)))/np.log(len(p)+1e-9)
        vals[5]=self._curvature(w)
        vals[6]=(w.max()-w.min())/mean_abs
        return np.clip(vals,0.0,1.0)
    def _curvature(self,w):
        if len(w)<3: return 0.0
        d1=np.diff(w); d2=np.diff(d1)
        denom=(1+d1[:-1]**2)**1.5
        return float(np.clip(np.mean(np.abs(d2)/(denom+1e-9))*10,0,1))
    def _compute(self,prices):
        if prices is None or len(prices)<2:
            return np.zeros(self.total),0.0,0.0,0.0
        prices=np.asarray(prices,dtype=float)
        base_table=np.zeros((len(self.WINDOWS),len(self.TRANSFORMS)))
        directions=np.zeros(len(self.WINDOWS))
        for wi,w_size in enumerate(self.WINDOWS):
            seg=prices[-w_size:]
            if len(seg)<2: continue
            base_table[wi,:]=self._transforms_for_window(seg)
            directions[wi]=1.0 if seg[-1]>seg[0] else (-1.0 if seg[-1]<seg[0] else 0.0)
        vals=base_table[self.window_idx,self.transform_idx]
        act=np.clip(vals*self.gain+self.bias,0.0,1.0)
        sensor_dirs=directions[self.window_idx]
        weighted=act*sensor_dirs
        denom=float(np.sum(act))+1e-9
        sensory_signal=float(np.clip(np.sum(weighted)/denom,-1.0,1.0))
        strong=act>0.5
        if np.any(strong):
            sd=sensor_dirs[strong]
            maj=1.0 if np.sum(sd)>=0 else -1.0
            coherence=float(np.mean(sd==maj))
        else: coherence=0.0
        mean_act=float(np.mean(act))
        return act,sensory_signal,coherence,mean_act
    def fire_all(self,prices):
        act,sensory_signal,coherence,mean_act=self._compute(prices)
        self.activation=act; self.sensory_signal=sensory_signal; self.sensory_coherence=coherence
        self.activation_history.append(mean_act)
        self.signal_history.append(sensory_signal)
        return mean_act,act
    def sense_live(self,prices,history=None):
        act,sensory_signal,coherence,mean_act=self._compute(prices)
        if history is not None: history.append(sensory_signal)
        else: self.signal_history.append(sensory_signal)
        anticipated=self._anticipate_next_move(history if history is not None else self.signal_history)
        regime=self._detect_market_regime(prices,sensory_signal,coherence)
        return {"signal":sensory_signal,"coherence":coherence,"intensity":mean_act,
                "anticipated":anticipated,"regime":regime,"power":float(np.mean(self.sense_power))}
    def _anticipate_next_move(self,hist):
        if len(hist) < 6: return {"dir":0,"strength":0.0,"pattern":"نامشخص"}
        arr=np.array(list(hist)[-24:])
        vel=np.diff(arr)
        acc=np.diff(vel) if len(vel)>1 else np.array([0.0])
        cur=float(arr[-1]); v=float(vel[-1]) if len(vel)>0 else 0.0
        a=float(acc[-1]) if len(acc)>0 else 0.0
        if cur > 0.45 and v > 0.05 and a >= -0.02:
            return {"dir":1,"strength":clamp(abs(cur)*0.7+abs(v)*2.0,0,1),"pattern":"ادامه صعود قوی"}
        if cur < -0.45 and v < -0.05 and a <= 0.02:
            return {"dir":-1,"strength":clamp(abs(cur)*0.7+abs(v)*2.0,0,1),"pattern":"ادامه نزول قوی"}
        if abs(cur) < 0.15 and abs(v) < 0.08: return {"dir":0,"strength":0.2,"pattern":"رنج/انتظار"}
        if v > 0.12 and a > 0.02: return {"dir":1,"strength":clamp(abs(v)*1.6,0,1),"pattern":"شتاب‌گیری صعودی"}
        if v < -0.12 and a < -0.02: return {"dir":-1,"strength":clamp(abs(v)*1.6,0,1),"pattern":"شتاب‌گیری نزولی"}
        if cur > 0.25 and v < -0.10: return {"dir":-1,"strength":clamp(abs(v)*2.0,0,1),"pattern":"بازگشت نزولی"}
        if cur < -0.25 and v > 0.10: return {"dir":1,"strength":clamp(abs(v)*2.0,0,1),"pattern":"بازگشت صعودی"}
        return {"dir":int(np.sign(cur + v)),"strength":clamp(abs(cur+v),0,1),"pattern":"حرکت ملایم"}
    def _detect_market_regime(self,prices,signal,coherence):
        if prices is None or len(prices) < 25: return {"name":"نامشخص","confidence":0.0,"bias":0}
        p=np.asarray(prices,dtype=float)[-80:]
        if len(p) < 25: return {"name":"نامشخص","confidence":0.0,"bias":0}
        ret=np.diff(p)/np.maximum(p[:-1],1e-12)
        vol=float(np.std(ret)) if len(ret)>2 else 0.0
        try:
            x=np.arange(len(p))
            slope=float(np.polyfit(x,p,1)[0])/max(abs(float(np.mean(p))),1e-9)
        except Exception: slope=0.0
        rng=(float(p.max())-float(p.min()))/max(abs(float(np.mean(p))),1e-9)
        short_vol=float(np.std(ret[-6:])) if len(ret)>=6 else vol
        long_vol=float(np.std(ret[-30:])) if len(ret)>=30 else vol
        expansion=short_vol/(long_vol+1e-9)
        if slope > 0.0012 and signal > 0.25:
            return {"name":"صعودی قوی","bias":1,"confidence":clamp(abs(signal)*0.6+coherence*0.4,0,1)}
        if slope < -0.0012 and signal < -0.25:
            return {"name":"نزولی قوی","bias":-1,"confidence":clamp(abs(signal)*0.6+coherence*0.4,0,1)}
        if vol > 0.008:
            bias=int(np.sign(signal)) if abs(signal)>0.35 else 0
            return {"name":"آشوب/پرنوسان","bias":bias,"confidence":clamp(vol*80,0,1)}
        if rng < 0.005 and vol < 0.0022: return {"name":"رنج کم‌نوسان","bias":0,"confidence":0.55}
        if slope > 0.0004 and signal > 0.12:
            return {"name":"صعودی ملایم","bias":1,"confidence":clamp(abs(signal)*0.5+coherence*0.3,0,1)}
        if slope < -0.0004 and signal < -0.12:
            return {"name":"نزولی ملایم","bias":-1,"confidence":clamp(abs(signal)*0.5+coherence*0.3,0,1)}
        if rng > 0.012 and expansion > 1.3 and abs(signal) > 0.45:
            return {"name":"شکست/هیجان","bias":int(np.sign(signal)),"confidence":clamp(abs(signal)*0.7+coherence*0.3,0,1)}
        return {"name":"تعادل","bias":0,"confidence":0.30}
    def sense_summary(self):
        uniq,inv=np.unique(self.sense_labels,return_inverse=True)
        sums=np.bincount(inv,weights=self.activation,minlength=len(uniq))
        counts=np.bincount(inv,minlength=len(uniq))
        return {uniq[i]:float(sums[i]/max(1,counts[i])) for i in range(len(uniq))}
    def top_active(self,n=8):
        idx=np.argsort(-self.activation)[:n]
        return [(str(self.sense_labels[i]),round(float(self.activation[i]),2)) for i in idx]
...
class InteroceptionSystem:
    def __init__(self): self.body_awareness=0.5; self.signals={}
    def perceive(self,body):
        ...
        if config.FULL_ATTENTION: self.body_awareness=1.0
        return self.body_awareness

class AttentionSystem:
    def __init__(self):
        self.focus_level=1.0 if config.FULL_ATTENTION else 0.5
        self.attention_target=None
    def focus(self,signals,consciousness):
        if not signals:
            self.attention_target=None
            if config.FULL_ATTENTION: self.focus_level=1.0
            else: self.focus_level=ema(self.focus_level,0.35,0.1)
...
class MarketSyncPerception:
    def __init__(self): self.sync_score=0.35; self.coherence=0.0; self.breadth=0.0; self.n_seen=0
    def perceive(self,prelim_signals):
        if not prelim_signals:
            self.sync_score=ema(self.sync_score,0.30,0.10); return self.sync_score
        dirs=[math.copysign(1.0,s["signal"]) for s in prelim_signals if abs(s["signal"])>0.05]
        self.n_seen=len(prelim_signals)
        if not dirs:
            self.sync_score=ema(self.sync_score,0.30,0.10); return self.sync_score
        self.coherence=abs(float(np.mean(dirs)))
        self.breadth=clamp(len(dirs)/max(1,len(prelim_signals)))
        avg_conv=float(np.mean([s["conviction"] for s in prelim_signals]))
        raw=clamp(0.5*self.coherence+0.3*self.breadth+0.2*avg_conv)
        self.sync_score=ema(self.sync_score,raw,0.12)
...
class MathematicalMind:
    ...
    def analyze_from_signals(self,signals,current_price):
        ...
        n_conf=min(1.0,len(signals)/8); avg_conv=sum(convs)/len(convs)
...
# ═══════════════════════ ۱۴) مدیر لحظه‌ای با تریل‌استاپ ═══════════════════════
class ActivePositionManager:
    def __init__(self,discipline,patience):
        self.discipline=discipline; self.patience=patience; self.trail_log=[]
    def manage(self,pos,tk,regulator,analysis=None):
        ...
        if r_mult >= 0.5 and not pos.get("breakeven"):
            pos["sl"]=entry + side*max(spread, initial_risk*0.05)
            pos["breakeven"]=True
        momentum=analysis.get("signal",0) if analysis else 0
        momentum_aligned=momentum*side
        trail_start_r=pos.get("trail_start_r",config.TRAIL_START_R)
        trail_keep_ratio=pos.get("trail_keep_ratio",config.TRAIL_KEEP_RATIO)
        if r_mult>=trail_start_r:
            trail_r=min(r_mult-0.5, r_mult*trail_keep_ratio)
            old_sl=pos["sl"]
            if side>0:
                new_sl=entry+trail_r*initial_risk
                if new_sl>pos["sl"]:
                    pos["sl"]=new_sl; pos["trailed"]=True; pos["trail_amount"]=new_sl-old_sl
            else:
                new_sl=entry-trail_r*initial_risk
                if new_sl<pos["sl"]:
                    pos["sl"]=new_sl; pos["trailed"]=True; pos["trail_amount"]=old_sl-new_sl
...
class VitalitySystem:
    def __init__(self): self.breath_phase=0.0; self.vitality=0.9; self.adrenaline=0.0
...
class TradingEngine:
    ...
    def open_position(self,coin,side,price,spread,margin,leverage,tp,sl,conviction,reasons,tick,mode,
                      trail_start_r=None,trail_keep_ratio=None):
        with self.lock:
            ...
            pos={"coin":coin,"side":side,"entry":entry,"size":size,"margin":margin,
                 ...
                 "peak":entry,"trailed":False,"trail_amount":0.0,"spread":spread,
                 "trail_start_r":trail_start_r if trail_start_r is not None else config.TRAIL_START_R,
                 "trail_keep_ratio":trail_keep_ratio if trail_keep_ratio is not None else config.TRAIL_KEEP_RATIO}
            self.open_positions[coin]=pos
...
class MultiSymbolScanner:
    def __init__(self,http,symbol_limit,deep_limit,interval,kline_limit):
        ...
        self.analyzer=RawPriceAnalyzer()
...
REG_ORDER=["BRAINSTEM","THALAMUS","AMYGDALA","HIPPOCAMPUS",
           "VISUAL_CORTEX","NUMERICAL_CORTEX","SPACETIME_CORTEX","BASAL_GANGLIA","CORTEX"]
REGION_FA={"BRAINSTEM":"ساقه‌ی مغز","THALAMUS":"تالاموس","AMYGDALA":"آمیگدال",
           ...
           "SPACETIME_CORTEX":"قشر فضازمانی","BASAL_GANGLIA":"عقده‌های قاعده‌ای","CORTEX":"قشر مغز"}
PATHWAYS=[
...
class Region:
    ...
    def reset_current(self,noise=0.35): self.I=self.rng.normal(2.2,noise,self.n)
...
class Brain:
    def __init__(self,phenotype,rng,scale=None):
        ...
        counts={"BRAINSTEM":100*scale,"THALAMUS":200*scale,"AMYGDALA":130*scale,
                ...
                "NUMERICAL_CORTEX":(50+220*max(ns,geo)*shf)*scale,
        ...
        print(f"🧠 مغز: {sum(counts.values())} نورون | موازی: {config.PARALLEL_BRAIN}")
    def _build_connectome(self):
        for src,dst,p,strength,plast in PATHWAYS:
            ...
            for d in range(1,MAX_DELAY):
                ...
                self.syn[dst].append({"src":src,"pre":pre[m],"post":post[m],"w":w[m],"delay":d,"plastic":plast})
    ...
    def step(self,sensory,neurochem,quantum_events,fatigue,plasticity_factor=1.0,
             substeps=5,numeric_input=None,cardiac_afferent=0.0,metabolic_supply=1.0,
             ...
             heart_coherence=0.5,market_sync=0.0):
        ...
        for r in regs.values(): r.reset_current(noise=0.40*ms)
        ...
        regs["AMYGDALA"].add_current(None,(6.0*sensory.get("pain",0)+3.5*sensory.get("danger",0))
                                     *(1.15-0.3*heart_coherence))
        regs["HIPPOCAMPUS"].add_current(None,6.0*L["acetylcholine"])
        regs["BASAL_GANGLIA"].add_current(None,7.0*L["dopamine"])
        ...
        for dst in ("CORTEX","HIPPOCAMPUS","NUMERICAL_CORTEX","VISUAL_CORTEX","SPACETIME_CORTEX"):
            Rt=regs[dst]
            for b in self.syn[dst]:
...
class CardiacSystem:
    ...
    def coherence(self,emotional_calm=1.0,attention=1.0,vitality=0.8):
        rhythm=self.rhythm_coherence()
        neural_coh=self.neural.coherence
        try:
            breath=clamp(1.0-abs((self.phase/max(self.last_interval,1e-6))-0.5)*2.0,0,1)
        except Exception:
            breath=0.6
        market_penalty=clamp(self.market_excitement*0.25,0,0.25)
        raw=(0.35*rhythm+0.25*neural_coh+0.15*emotional_calm
             +0.15*breath+0.10*vitality-market_penalty)
...
class ConsciousnessMeter:
    def __init__(self,n_regions,window=160):
        ...
        self.arousal=0.65; self.integration=0.5; self.differentiation=0.5
        self.phi=0.4; self.workspace=0.55; self.level=62.0
    def update(self,activities,brainstem,body_ok_factor,quantum_coh,info_level=0.5,
               ...
               heart_vitality=0.6,body_awareness=0.5,attention=0.5,market_sync=0.0):
        vec=np.array(list(activities.values()),dtype=float)
        self.history.append(vec)
        self.arousal=ema(self.arousal,clamp(0.40+brainstem*2.2+0.20*wakefulness
                                            +0.15*heart_vitality+0.10*body_awareness),0.10)
        self.arousal=max(self.arousal,0.32)
        ...
        x=(0.34*activities.get("CORTEX",0)+0.18*activities.get("THALAMUS",0)
           +0.10*activities.get("HIPPOCAMPUS",0)+0.10*activities.get("BASAL_GANGLIA",0)
           +0.16*activities.get("SPACETIME_CORTEX",0)+0.07*attention)
        self.workspace=ema(self.workspace,1.0/(1.0+math.exp(-40.0*(x-0.05))),0.10)
        self.workspace=max(self.workspace,0.22)
        raw=100.0*clamp(0.21*self.workspace+0.19*self.arousal+0.10*self.phi
                        +0.06*self.integration+0.06*presence+0.08*sixth_sense
                        +0.05*spacetime_richness+0.06*heart_vitality
                        +0.05*body_awareness+0.05*attention+0.09*market_sync)
        raw*=(0.75+0.25*body_ok_factor)*(0.90+0.10*quantum_coh)*(0.90+0.10*info_level)
        raw=max(raw,28.0)
        self.level=ema(self.level,raw,0.07)
        return self.level
...
T={"market":["📊 ساختار قیمت را بررسی می‌کنم؛ {obs}",
             "📈 الگوی کندل‌ها را تحلیل می‌کنم؛ {obs}",
             "🎯 نقاط کلیدی حمایت و مقاومت را رصد می‌کنم.",
             "🔍 در حال اسکن {n} ارز برای بهترین موقعیت هستم."],
   ...
   "insight":["💡 الگویی در مارکت یافتم که با تجربه‌های قبلی هم‌خوانی دارد.",
   ...
   "heart":["قلبم با آهنگ فیبوناچی می‌تپد؛ همگام با ضربان مارکت."]}
...
class StreamOfConsciousness:
    ...
    def _market_obs(self,market_signal):
        if market_signal>0.4: return "فشار خرید قوی و حرکت صعودی"
        elif market_signal<-0.4: return "فشار فروش قوی و حرکت نزولی"
        elif market_signal>0.15: return "تمایل ملایم به صعود"
        elif market_signal<-0.15: return "تمایل ملایم به نزول"
        else: return "بازار در تعادل و بدون جهت مشخص"
    def step(self,tick,cons,activities,neurochem,body,sensory,quantum_events,
        ...
        return t
    def _compose(self,cat,qevents,market_signal,n_scanned,top_lesson,sensory_array,spacetime,body):
        if cat=="quantum":
...
# ═══════════════════════ ۲۷) موجود زنده — لایو‌محور با ۱۲۰۰۰ حس ═══════════════════════
...
class Organism:
    ...
    def rebirth(self,seed=None):
        ...
        self.market_breadth_n=0
        self.market_sync=MarketSyncPerception()
        # 🔢 تحلیل عددی زنده
        self.numerical_analyst=LiveNumericalAnalyst()
        self.last_numerical={}
        # 🌌 تاریخچه حس هر ارز برای پیش‌بینی حرکت بعدی
        self.symbol_sense_history={}
        # 🛡️ کنترل ضرر
        ...
        self._add_milestone(f"🌌 متولد شد — ژنوم {self.genome.signature()} | "
                            f"سوپرهیومن ×{shf:.2f} | {self.xeno_array.total} حس زنده")
    ...
    def cardiac_coherence(self):
        emotional_calm=self.regulator.emotional_calm()
        vitality=self.last_vitality.get("vitality",0.8)
        attention=getattr(self.attention,"focus_level",1.0)
        return self.body.cardiac.coherence(emotional_calm,attention,vitality)
    ...
    def _perceive_spacetime_and_senses(self,deep,tickers,focus):
        ...
        for sym,d in deep.items():
            ...
            hist=self.symbol_sense_history.setdefault(sym, deque(maxlen=64))
            self.live_senses[sym]=self.xeno_array.sense_live(c, hist)
            self.last_numerical[sym]=self.numerical_analyst.analyze(kl, sym)
        closes_list=[d["klines"]["c"] for d in deep.values() if d.get("klines") is not None]
        if closes_list:
            min_len=min(len(c) for c in closes_list)
            if min_len>=2:
                normalized=[]
                for c in closes_list:
                    seg=np.asarray(c[-min_len:],dtype=float)
                    base=seg[0] if abs(seg[0])>1e-12 else 1e-9
                    normalized.append((seg-base)/base)
                market_pulse=np.mean(np.array(normalized),axis=0)
                self.xeno_array.fire_all(market_pulse)
                self.last_xeno_summary=self.xeno_array.sense_summary()
                self.market_breadth_n=len(closes_list)
    def _make_signal(self,coin,tk,an,geom,kl,energy,mp,attention,prelim_signals=None):
        if not an.get("ready"): return None
        price=tk["price"]; spread=tk["spread"]
        raw_sig=clamp(an["signal"],-1,1)
        live=self.live_senses.get(coin,{})
        num=self.last_numerical.get(coin,{})
        sense_signal=live.get("signal",0.0)
        sense_coh=live.get("coherence",0.0)
        regime=live.get("regime",{})
        antic=live.get("anticipated",{})
        num_score=num.get("score",0.0)
        num_dir=num.get("direction",0)
        num_conf=num.get("confidence",0.0)
        en_dir=self.eternal_now.last_direction
        en_score=self.eternal_now.eternal_now_score
        math_analysis=self.math_mind.analyze_from_signals(
            prelim_signals if prelim_signals is not None else self.current_signals,price)
        math_prob=math_analysis["probability"]
        math_conf=math_analysis["confidence"]
        math_dir=1 if math_prob>0.55 else (-1 if math_prob<0.45 else 0)
        math_strength=abs(math_prob-0.5)*2.0*math_conf
        components=[
            ("price", float(raw_sig), 0.26),
            ("sense", float(sense_signal), 0.30),
            ("numeric", float(num_score), 0.24),
            ("eternal", float(en_dir)*float(en_score), 0.10),
            ("math", float(math_dir)*float(math_strength), 0.10),
        ]
        sig=0.0
        for _,val,w in components:
            if np.isfinite(val): sig += val*w
        sig=clamp(sig,-1.5,1.5)
        side=1 if sig>0 else -1
        active_dirs=[np.sign(val) for _,val,_ in components if abs(val)>0.05]
        if active_dirs:
            alignment=float(np.mean([1 if d==side else 0 for d in active_dirs]))
        else: alignment=0.0
        conviction=clamp(
            abs(sig)*(0.45+0.55*energy)*(1+0.3*self.phenotype.intuition)
            *(0.8+0.2*self.phenotype.superhuman_factor)*(0.7+0.3*attention),0,1)
        conviction *= (0.5 + 0.6*alignment)
        ...
        if mp.get("use_geometry") and geom.get("ready") and geom.get("levels"):
            for lvl_name,lvl_price in geom["levels"].items():
                ...
                if dist<0.004 and geom.get("trend")==side:
                    conviction=clamp(conviction+0.18*self.phenotype.geometry,0,1)
                    sniper=True; break
        if abs(sense_signal)>0.2:
            if np.sign(sense_signal)==side:
                conviction=clamp(conviction*(1+0.35*sense_coh),0,1)
                reasons.append(f"🌌 حس {sense_signal:+.2f}")
            else: conviction *= 0.65
        if regime.get("bias",0)==side and regime.get("confidence",0)>0.3:
            conviction=clamp(conviction*(1+0.22*regime["confidence"]),0,1)
            reasons.append(f"🌊 {regime['name']}")
        elif regime.get("bias",0)==-side and regime.get("confidence",0)>0.5:
            conviction *= 0.65
        if antic.get("dir",0)==side and antic.get("strength",0)>0.3:
            conviction=clamp(conviction*(1+0.25*antic["strength"]),0,1)
            reasons.append(f"🔮 {antic.get('pattern','الگو')}")
        elif antic.get("dir",0)==-side and antic.get("strength",0)>0.45:
            conviction *= 0.65
        if num.get("ready") and num_dir==side and abs(num_score)>0.15:
            conviction=clamp(conviction*(1+0.25*num_conf),0,1)
            reasons.append(f"🔢 {num.get('reason','تحلیل عددی')}")
        elif num.get("ready") and num_dir==-side and abs(num_score)>0.35:
            conviction *= 0.60
        if en_dir!=0 and np.sign(sig)==en_dir:
            conviction=clamp(conviction*(1+0.20*en_score),0,1)
        sixth=self.xeno_array.activation_history[-1] if self.xeno_array.activation_history else 0.0
        st_rich=self.spacetime.richness()
        if sixth>config.SIXTH_SENSE_GATE:
            conviction=clamp(conviction*(1+0.20*sixth+0.15*st_rich),0,1)
            reasons.append("👁️‍🗨️ حس ششم فعال")
        if st_rich>0.5: reasons.append("🌀 هندسه فضازمانی غنی")
        if (side==1 and math_prob>0.55) or (side==-1 and math_prob<0.45):
            conviction=clamp(conviction*(1+0.25*math_conf*self.math_mind.iq_factor),0,1)
            reasons.append(f"🧠 تحلیل ریاضی ({math_prob*100:.0f}٪)")
        vol=max(an["volatility"],price*0.0004)
        tp_mult=mp["tp_mult"]; sl_mult=mp["sl_mult"]
        if side>0: tp=price+vol*tp_mult*(0.8+0.4*energy); sl=price-vol*sl_mult
        else: tp=price-vol*tp_mult*(0.8+0.4*energy); sl=price+vol*sl_mult
        max_sl_dist=price*(0.8/mp["leverage"])
        if abs(sl-price)>max_sl_dist: sl=price-side*max_sl_dist
        if sniper: reasons.append("🎯 نقطه هندسی فیبوناچی")
        if en_score>0.7: reasons.append("🕉️ اکنون ابدی منسجم")
        ...
        return {"symbol":coin,"side":side,"price":price,"spread":spread,
                "spread_pct":tk["spread_pct"],"tp":tp,"sl":sl,"conviction":conviction,
                "signal":sig,"volatility":vol,"change24h":tk["change24h"],
                "turnover":tk["turnover"],"reasons":reasons,"sniper":sniper,
                "lesson_adj":lesson_adj,"alignment":alignment,
                "live":live,"numerical":num}
    def _is_rational(self,sig,mp):
        ...
        cost_margin=mp.get("cost_margin",1.15)
        ev_margin=mp.get("ev_margin",0.20)
        if tp_dist_pct<cost_pct*cost_margin: return False
        if ev<=0: return False
        if mp.get("cost_aware") and ev<cost_pct*ev_margin: return False
        return True
    def _roll_daily(self):
        ...
        if today != self.daily_date:
            ...
            self.daily_start_equity=self.engine.equity(0)
            self.loss_cooldown=0
    def _daily_ok(self):
        eq=self.engine.equity(self._unrealized(self._last_tickers))
        dd=safe_div(self.daily_start_equity-eq,self.daily_start_equity)*100
        return dd < config.DAILY_LOSS_LIMIT_PCT
    def _live_entry_gate(self,sig,tk,mp):
        if self.loss_cooldown > 0: return False
        if not self._daily_ok(): return False
        live=sig.get("live",{})
        num=sig.get("numerical",{})
        regime=live.get("regime",{})
        antic=live.get("anticipated",{})
        side=sig["side"]
        max_spread=config.MAX_SPREAD_PCT.get(self.mode,0.003)
        if tk.get("spread_pct",1.0) > max_spread: return False
        if abs(live.get("signal",0)) < config.MIN_SENSE_ABS: return False
        if live.get("coherence",0) < config.MIN_SENSE_COHERENCE: return False
        if regime.get("bias",0) != side or regime.get("confidence",0) < config.MIN_REGIME_CONF: return False
        if antic.get("dir",0) != side or antic.get("strength",0) < config.MIN_ANTICIPATION: return False
        if not num.get("ready"): return False
        if num.get("direction",0) != side: return False
        if abs(num.get("score",0)) < config.MIN_NUMERIC_SCORE: return False
        cc=self.cardiac_coherence()
        min_cc=config.MIN_CARDIAC_SCALP if self.mode=="scalp" else config.MIN_CARDIAC_SWING
        if cc < min_cc: return False
        if sig.get("alignment",0) < config.CONSENSUS_MIN: return False
        if self.market_sync.sync_score < 0.35 and abs(sig.get("signal",0)) < 0.8: return False
        return True
    def _trading_step(self,scanner_snapshot):
        if self._reset_cooldown>0:
            self._reset_cooldown-=1
            return
        if self.loss_cooldown>0: self.loss_cooldown-=1
        mp=self.mode_params
        ...
        for coin,d in deep.items():
            an=d.get("analysis")
            if an and an.get("ready"):
                prelim_signals.append({"signal":an["signal"],"conviction":an["conviction"]})
        self.market_sync.perceive(prelim_signals)
        signals=[]
        for coin,d in deep.items():
            tk=tickers.get(coin)
            if not tk: continue
            ...
            sig=self._make_signal(coin,tk,an,geom,d["klines"],energy,mp,attention,prelim_signals)
            if sig:
                sig["geometry"]=geom
                signals.append(sig)
        ...
        st_rich=self.spacetime.richness()
        en_score=self.eternal_now.eternal_now_score
        awareness=clamp(0.55+0.55*(self.cons.level/100.0),0.55,1.20)
        msync=self.market_sync.sync_score
        presence_scale=((0.5+0.5*en_score)*(0.6+0.4*cc)*(0.7+0.3*sixth)
                        *(0.8+0.2*st_rich)*awareness*(0.75+0.25*msync))
        for sig in signals:
            coin=sig["symbol"]
            tk=tickers.get(coin)
            if not tk: continue
            if coin in self.engine.open_positions: continue
            if sig["conviction"]<mp["conviction_threshold"]+conv_mod: continue
            if not self._is_rational(sig,mp): continue
            if not self._live_entry_gate(sig,tk,mp): continue
            if self.engine.slot_of(coin) is None: self.engine.assign_coin(coin)
            avail=self.engine.available_margin(coin)
            if avail<5: continue
            margin=min(avail*mp["margin_fraction"]*size_mult*presence_scale,avail)
            ...
            pos=self.engine.open_position(coin,sig["side"],sig["price"],sig["spread"],
                                          ...
                                          trail_start_r=mp.get("trail_start_r"),
                                          trail_keep_ratio=mp.get("trail_keep_ratio"))
            if pos:
                self.focus_symbol=coin
                self._trade_thought_open(pos)
    ...
    def _on_closed(self,pos,net,reason,exit_price):
        ...
        if net>0:
            ...
            self.genome.epigenome.adapt("intuition",-0.0003)
            ...
            self._reward=0.0
        # 🛡️ کاهش ضرر: بعد از ضرر، ارگانیسم صبر می‌کند
        if net < 0: self.loss_cooldown = config.LOSS_COOLDOWN_TICKS
        else: self.loss_cooldown = max(0, self.loss_cooldown // 2)
    ...
    def _trade_thought_open(self,pos):
        ...
        t=Thought(self.mind.next_id,self.tick,"trade",text,pos["conviction"],
                  ["NUMERICAL_CORTEX","BASAL_GANGLIA","SPACETIME_CORTEX"])
    ...
    def step(self):
        ...
        out=self.brain.step(sensory,neuro,q_events,self.body.fatigue,
                            ...
                            heart_coherence=self.cardiac_coherence(),
                            market_sync=self.market_sync.sync_score)
        ...
        level=self.cons.update(out["activities"],out["activities"].get("BRAINSTEM",0),
                               ...
                               attention=attention,market_sync=self.market_sync.sync_score)
    ...
    def snapshot(self):
        ...
        return {
            ...
            "market_sync":self.market_sync.sync_score,
            "market_coherence":self.market_sync.coherence,
            "market_breadth":self.market_sync.breadth,
            "market_breadth_n":self.market_breadth_n,
            "sense_power":float(config.SENSE_POWER),
...
# 🧬 فهرست کامل ۱۲۰۰۰ حس زنده برای نمایش در تب زنده (ساخت یک‌بار در شروع)
SENSE_TABLE_DATA=[
    {"#":i+1,"حس":str(name),"قدرت":int(config.SENSE_POWER),"وضعیت":"فعال"}
    for i,name in enumerate(ORGANISM.xeno_array.sense_directory)
]
sense_directory_panel=html.Div(className="panel s12",children=[
    html.H3([html.Span(className="dot-gold"),
             f"🧬 فهرست کامل {ORGANISM.xeno_array.total} حس زنده — قدرت ۱۰۰٪ | حواس جمع ۱۰۰٪"]),
    html.Div(f"۵ حس پایه × {config.SENSORS_PER_SENSE} + {config.N_XENO_SENSES} حس فرازمینی = "
             f"{ORGANISM.xeno_array.total} حس فعال روی لایو مارکت",
             style={"fontSize":11.5,"color":"#94a3b8","marginBottom":8}),
    dash_table.DataTable(
        data=SENSE_TABLE_DATA,
        columns=[
            {"name":"#","id":"#"},
            {"name":"حس","id":"حس"},
            {"name":"قدرت","id":"قدرت"},
        ...
        page_size=50,
        page_action="native",
        style_table={"maxHeight":"380px","overflowY":"auto","overflowX":"auto"},
        style_cell={
            "backgroundColor":"#0f172a","color":"#e2e8f0",
            "fontFamily":"Vazirmatn","fontSize":"11px","textAlign":"right",
            "border":"1px solid rgba(148,163,184,.08)",
        },
        style_header={"backgroundColor":"#1e293b","color":"#c7d2fe","fontWeight":"700"},
        style_as_list_view=True,
    ),
])

# ═══════════════════════ ۲۹) چارت زنده با تریل‌استاپ ═══════════════════════
def build_focus_chart(s,scanner_snap):
    ...
    fig.update_layout(template="plotly_dark",paper_bgcolor="rgba(0,0,0,0)",
                      ...
                      title={"text":f"🌌 {focus} | اکنون ابدی {en*100:.0f}٪ | توجه {attn*100:.0f}٪ | {mode_fa}",
...
main_tab=dcc.Tab(label="🧠 داشبورد زنده",value="tab-main",className="tab",
    ...
    # 🧬 فهرست کامل ۱۲۰۰۰ حس زنده در تب زنده
    html.Div(className="grid",children=[sense_directory_panel]),
])
...
signals_tab=dcc.Tab(label="📊 سیگنال‌ها و معاملات",value="tab-signals",className="tab",
    ...
    html.Div(className="grid",children=[
        ...
        html.Div(className="panel s5",children=[
            html.H3([html.Span(className="dot-gold"),"🎯 سیگنال‌های فعال"]),
...
def render_sensory_panel(s):
    ...
    sixth_box=html.Div(className="sixth-box",children=[
        ...
        html.Div(f"فعال‌سازی کل {xeno_total} حس زنده: {xeno_act*100:.0f}٪ | قدرت: {config.SENSE_POWER:.0f}٪",
...
def render_emotion_panel(s):
    ...
    vit=s.get("vitality",0.9)
    cc=s.get("cardiac_coherence",0.0)
    state_color="#4ade80" if "آرام" in state else ("#fbbf24" if "هوشیار" in state else "#f87171")
    cc_color="#4ade80" if cc>0.7 else ("#fbbf24" if cc>0.5 else "#f87171")
    return html.Div([
        html.Div(f"وضعیت: {state}",
                 style={"fontWeight":"bold","color":state_color,"fontSize":13,"marginBottom":8}),
        ...
        html.Div(className="emo-meter",children=[
            html.Div(f"🫀 انسجام قلب: {cc*100:.0f}٪",style={"color":cc_color,"marginBottom":2}),
            html.Div(className="bar-bg",children=[
                html.Div(className="bar-fg",style={"width":f"{cc*100:.0f}%","background":cc_color})])]),
        html.Div(f"⚡ سوپرهیومن: ×{s.get('superhuman_factor',1):.2f} | 🧠 IQ: {s.get('iq_display',200)} | "
                 f"🎯 حواس جمع: {s.get('attention_level',1)*100:.0f}٪",
...
def render_mode_info(s):
    ...
    children=[
        ...
        html.Div(f"آستانه: {int(mp['conviction_threshold']*100)}٪ | مارجین اسلات: {config.SLOT_MARGIN:.0f}$"),
    ...
    if s.get("loss_cooldown",0)>0:
        children.append(html.Div(f"🛡️ مکث پس از ضرر ({s['loss_cooldown']} تیک)",
                                 style={"color":"#f87171","marginTop":4}))
    return html.Div(className="mode-card",children=children)

def render_signals(signals):
    if not signals:
        return html.Div("در حال اسکن… منتظر اجماع ۱۲۰۰۰ حس",style={"fontSize":12,"color":"#94a3b8"})
    cards=[]
    for s in signals:
        ...
        extra=[]
        if regime:
            extra.append(html.Span(
                f"🌊 {regime.get('name','')} | {regime.get('confidence',0)*100:.0f}٪",
                className="reason-chip",
                style={"background":"rgba(167,139,250,.15)","color":"#c084fc",
                       "border":"1px solid rgba(167,139,250,.3)"}))
        if antic:
            extra.append(html.Span(
                f"🔮 {antic.get('pattern','')} | {antic.get('strength',0)*100:.0f}٪",
                className="reason-chip",
                style={"background":"rgba(34,211,238,.15)","color":"#22d3ee",
                       "border":"1px solid rgba(34,211,238,.3)"}))
        if num.get("ready"):
            extra.append(html.Span(
                f"🔢 {num.get('reason','')} | {abs(num.get('score',0))*100:.0f}٪",
                className="reason-chip",
        ...
        cards.append(html.Div(className="sig-card",
            ...
            html.Div(className="sig-head",children=[
                ...
                html.Span(f"اطمینان {int(s['conviction']*100)}٪ | اجماع {s.get('alignment',0)*100:.0f}٪",
            ...
            html.Div(reasons+extra,style={"marginTop":4}),
        ]))
...
def _update_impl(n,mode):
    ...
    chips=[
        ...
        html.Span(f"🧬 {s.get('xeno_total',0)} حس زنده | قدرت {int(config.SENSE_POWER)}٪",className="chip chip-gold"),
        html.Span(f"🎯 حواس جمع {s.get('attention_level',1)*100:.0f}٪",className="chip chip-ok"),
        html.Span(f"🫀 انسجام قلب {s.get('cardiac_coherence',0)*100:.0f}٪",className="chip chip-purple"),
        html.Span(f"🌐 همگامی مارکت {s.get('market_sync',0)*100:.0f}٪ ({s.get('market_breadth_n',0)} ارز)",
    ...
    scanner_status=html.Div(
        ...
        f"کارمزد: {config.TAKER_FEE*100:.3f}٪ | 🧬 {s.get('xeno_total',0)} حس زنده",
        style={"color":"#94a3b8"})
...
def interact(*args):
    ...
    if trig=="btn-rebirth":
        ORGANISM.rebirth()
        return html.Span("🌌 موجود جدید با ژنوم تریدر هوشمند متولد شد.",style={"color":"#a78bfa"})
...
if __name__=="__main__":
    ...
    print(f"🧬 {config.SENSORS_PER_SENSE} حسگر/حس + {config.N_XENO_SENSES} حس فرازمینی = "
          f"{config.SENSORS_PER_SENSE*5+config.N_XENO_SENSES} حس زنده")
