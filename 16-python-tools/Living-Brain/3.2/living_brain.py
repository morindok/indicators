...
# -*- coding: utf-8 -*-
"""
🧠 مغز زنده — نسخه 0.19 (تک‌فایلی کامل)
تریل‌استاپ واضح | قلب عصبی | ذهن ریاضیدان IQ 200 | ادراک واقعی
...
config.PROJECT_NAME="مغز زنده"; config.VERSION="0.19-trail"
config.SUBTITLE="تریل‌استاپ واضح | قلب عصبی | ذهن ریاضیدان IQ 200"
...
config.SENSORS_PER_SENSE=max(10,config.HW.cores*4)
config.N_XENO_SENSES=180
config.SPACETIME_HISTORY=256
# 🔥 تریل‌استاپ: شروع از ۱R، فاصله تریل ۶۰٪ فاصله اولیه
...
class MarketSyncPerception:
    """🌐 همگام‌سازی با کل مارکت: انسجام جهت + وسعت (breadth) میان همه‌ی ارزهای اسکن‌شده."""
    def __init__(self):
        self.sync_score=0.35; self.coherence=0.0; self.breadth=0.0; self.n_seen=0
    def perceive(self,prelim_signals):
        if not prelim_signals:
            self.sync_score=ema(self.sync_score,0.30,0.10)
...
class MathematicalMind:
    ...
    def learn(self,predicted_side,was_profitable):
        ...
        if self.accuracy>0.55:
            self.growth=clamp(self.growth+0.02,0,1.0)
        elif self.accuracy<0.45:
            self.growth=clamp(self.growth-0.01,0,1.0)
        self.iq_display=int(200+self.growth*100)
...
# ═══════════════════════ ۱۴) مدیر لحظه‌ای با تریل‌استاپ واضح ═══════════════════════
class ActivePositionManager:
    """مدیر لحظه‌ای معاملات با تریل‌استاپ واضح و قابل‌مشاهده."""
    ...
    def manage(self,pos,tk,regulator,analysis=None):
        ...
        # 🔥 تریل‌استاپ: مخصوص هر حالت (اسکالپ زودتر از سویینگ قفل می‌کند)
...
class Organism:
    ...
    def _perceive_spacetime_and_senses(self,deep,tickers,focus):
        ...
        # 🌌 حس فراگیر مارکت: شاخص ترکیبیِ نرمال‌شده از همه‌ی ارزهای اسکن‌شده،
        # نه فقط سکه‌ی کانونی — ارگانیسم با تمام حواسش کل بازار را حس می‌کند
    ...
    def _make_signal(self,coin,tk,an,geom,energy,mp,attention,prelim_signals=None):
        ...
        conviction=clamp(abs(sig)*(0.5+0.5*energy)*(1+0.3*self.phenotype.intuition)
                         *(0.8+0.2*shf)*(0.8+0.3*attention),0,1)
        ...
        if en_dir!=0 and np.sign(sig)==en_dir:
            conviction=clamp(conviction*(1+0.25*en_score),0,1)
        ...
        if st_rich>0.5:
            reasons.append("🌀 هندسه فضازمانی غنی")
        ...
        if (side==1 and math_prob>0.55) or (side==-1 and math_prob<0.45):
            conviction=clamp(conviction*(1+0.3*math_conf*self.math_mind.iq_factor),0,1)
            reasons.append(f"🧠 تحلیل ریاضی (احتمال {math_prob*100:.0f}٪)")
    ...
    def _trading_step(self,scanner_snapshot):
        ...
        for coin,d in deep.items():
            ...
            sig=self._make_signal(coin,tk,an,geom,energy,mp,attention,prelim_signals)
            if sig:
                sig["geometry"]=geom
                signals.append(sig)
        ...
        presence_scale=(0.5+0.5*en_score)*(0.6+0.4*cc)*(0.7+0.3*sixth)*(0.8+0.2*st_rich)*awareness*(0.75+0.25*msync)
        for sig in signals:
            coin=sig["symbol"]
...
def build_focus_chart(s,scanner_snap):
    ...
    if pos:
        ...
        # 🔥 نمایش حد ضرر تریل‌شده با رنگ متفاوت
    ...
    fig.update_layout(template="plotly_dark",paper_bgcolor="rgba(0,0,0,0)",
                      ...
                      title={"text":f"🌌 {focus} | اکنون ابدی {en*100:.0f}٪ | توجه {attn*100:.0f}٪ | {mode_fa}","x":0.5,
