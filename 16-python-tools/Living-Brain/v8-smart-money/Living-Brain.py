...
# -*- coding: utf-8 -*-
"""
🏆 مغز زنده — نسخه ۵.۰ «Elite Trader» (تک‌فایلی کامل)
بزرگترین تریدر جهان | Smart Money Concepts | Order Flow | Multi-TF Confluence
...
def _benchmark(seconds=0.12):
    try:
        ...
        while time.perf_counter() - t0 < seconds: a @ b; iters += 1
...
class HardwareProfile:
    ...
    ticks: int; parallel: bool; workers: int
    @property
    def label(self):
        return f"CPU {self.cores} | {self.gflops:.0f}GF | رم {self.memory:.0f}G | {self.tier}"
...
def profile_hardware():
    ...
    parallel = cores >= 8
    p = HardwareProfile(cores, phys, mem, gf, tier, scale, ticks, parallel, workers)
    print("🖥 سخت‌افزار →", p.label); return p

# ═══════════ ۳) پیکربندی ═══════════
class _Config: pass
config = _Config()
config.HW = profile_hardware()
config.PROJECT_NAME = "مغز زنده"; config.VERSION = "5.0-elite-trader"
config.SUBTITLE = "بزرگترین تریدر جهان | SMC + Order Flow + Multi-TF"
config.TICKS_PER_SECOND = config.HW.ticks
config.DT = 1.0 / config.TICKS_PER_SECOND
config.NEURON_SCALE = config.HW.scale * (2.0 + min(config.HW.cores, 16) * 0.20)
config.MAX_NEURONS = 120000 if config.HW.parallel else 60000
...
config.SCAN_SYMBOL_LIMIT = 60
config.SCAN_DEEP_LIMIT = 5
config.SCAN_PERIOD = 5.0
config.SCAN_RESCAN_SEC = 10
config.SIM_RATE = 6
config.SIXTH_SENSE_GATE = 0.25
config.SENSORS_PER_SENSE = max(6, config.HW.cores * 2)
config.N_XENO_SENSES = 80
...
# 🔬 تحلیل اتمی: تایم‌فریم‌های چندگانه
config.TFS = {"15": "15m", "60": "1h", "240": "4h"}
config.KLINE_LIMIT = 200
config.MIN_RR = 2.5           # حداقل Risk:Reward
config.KELLY_FRACTION = 0.25  # محافظه‌کارانه
config.MIN_CONVICTION = 0.55  # آستانه بالای ورود

# ⚔️ دروازه‌های سخت‌گیر ورود
config.GATE_HTF_ALIGN = 0.30    # هم‌راستایی H4
config.GATE_MTF_ALIGN = 0.20    # هم‌راستایی H1
config.GATE_OB_SCORE = 0.35     # Order Block معتبر
config.GATE_FVG_SCORE = 0.25    # Fair Value Gap
config.GATE_LIQ_SWEEP = 0.20    # Liquidity Sweep
config.GATE_STRUCTURE = 0.30    # BOS/CHoCH تازه
config.GATE_VOLUME = 0.20       # تأیید حجم
config.GATE_PREMIUM = 0.25      # در discount برای لانگ، premium برای شورت

# 📐 تریل‌استاپ هوشمند
config.TRAIL_START_R = 1.0
config.TRAIL_KEEP_RATIO = 0.55
config.PARTIAL_TP_1R = 0.30   # ۳۰٪ در ۱R
config.PARTIAL_TP_2R = 0.40   # ۴۰٪ در ۲R
config.TIME_STOP_MIN = 90     # ۹۰ دقیقه بی‌ثمر = خروج
config.BREAK_EVEN_R = 1.0     # BE بعد از ۱R
config.DAILY_LOSS_LIMIT = 0.05  # حداکثر ۵٪ ضرر روزانه
config.SYMBOL_BAN_TRADES = 3

TRADING_MODES = {
    "elite": {"label": "🏆 Elite Trader (لوریج ۱۰)", "leverage": 10, "tp_mult": 3.5, "sl_mult": 1.4,
              "conviction_threshold": 0.55, "decision_interval": 3,
              "margin_fraction": 0.5, "max_daily": 8, "cost_aware": True},
    "scalp": {"label": "⚡ اسکلپ دقیق (لوریج ۲۰)", "leverage": 20, "tp_mult": 2.0, "sl_mult": 1.0,
              "conviction_threshold": 0.60, "decision_interval": 2,
              "margin_fraction": 0.3, "max_daily": 20, "cost_aware": True},
}
...
class TradingDB:
    ...
    def _init(self):
        with self.lock:
            ...
            c.execute("""CREATE TABLE IF NOT EXISTS positions (
                ...
                exit_reason TEXT, net_pnl REAL, close_time TEXT, peak REAL, trailed INTEGER)""")
    ...
    def save_open_position(self, engine, pos):
        with self.lock:
            ...
            c.execute("""INSERT INTO positions (engine,coin,side,entry,size,margin,leverage,notional,
                tp,sl,conviction,reasons,commission,spread_cost,open_time,open_ts,open_tick,slot_id,status,peak,trailed)
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?, 'open', ?, ?)""",
                      (engine, pos["coin"], pos["side"], pos["entry"], pos["size"], pos["margin"],
                       ...
                       pos["open_time"], pos["open_ts"], pos["open_tick"], pos["slot_id"],
                       pos.get("peak", pos["entry"]), int(pos.get("trailed", False))))
            conn.commit(); conn.close()
    ...
    def load_positions(self, engine):
        with self.lock:
            ...
            cols = [d[0] for d in c.description]; rows = [dict(zip(cols, r)) for r in c.fetchall()]
            conn.close()
        open_pos = {}; closed = []
        for r in rows:
            try: r["reasons"] = json.loads(r["reasons"]) if r["reasons"] else []
            except Exception: r["reasons"] = []
            r["trailed"] = bool(r.get("trailed", False))
...
# ═══════════ ۶) 🔬 تحلیل‌گر اتمی (SMC + Order Flow) ═══════════
class AtomicAnalyzer:
    """تحلیل‌گر حرفه‌ای بر پایه Smart Money Concepts.
    خروجی: Market Structure, Order Blocks, FVG, Liquidity, Volume Profile"""
    def __init__(self): self.cache = {}

    def _atr(self, h, l, c, p=14):
        n = len(c); out = np.full(n, 1e-9); tr = np.empty(n)
        tr[0] = h[0] - l[0]
        for i in range(1, n):
    ...
    def _ema_arr(self, a, p):
        n = len(a); out = np.empty(n); k = 2.0/(p+1)
        out[0] = a[0]
        for i in range(1, n): out[i] = a[i]*k + out[i-1]*(1-k)
        return out

    def _swing_points(self, h, l, window=3):
        """شناسایی swing highs و swing lows با فیلتر قوی."""
        n = len(h); highs = []; lows = []
        for i in range(window, n-window):
            if h[i] > np.max(h[i-window:i]) and h[i] > np.max(h[i+1:i+window+1]):
                highs.append((i, float(h[i])))
            if l[i] < np.min(l[i-window:i]) and l[i] < np.min(l[i+1:i+window+1]):
                lows.append((i, float(l[i])))
        return highs, lows

    def _market_structure(self, highs, lows, current_price):
        """BOS (Break of Structure) و CHoCH (Change of Character)."""
        result = {"trend": 0, "bos": 0.0, "choch": 0.0, "last_bos_type": None, "fresh": False}
        if len(highs) < 2 or len(lows) < 2: return result
        # شناسایی HH/HL یا LH/LL
        hh = highs[-1][1] > highs[-2][1]
        hl = lows[-1][1] > lows[-2][1]
        lh = highs[-1][1] < highs[-2][1]
        ll = lows[-1][1] < lows[-2][1]
        if hh and hl:
            result["trend"] = 1   # Uptrend
            # BOS: شکست آخرین swing high
            if current_price > highs[-1][1]:
                result["bos"] = 1.0
                result["last_bos_type"] = "bullish"
                result["fresh"] = True  # تازه است
            result["choch"] = 0.0
        elif lh and ll:
            result["trend"] = -1  # Downtrend
            if current_price < lows[-1][1]:
                result["bos"] = 1.0
                result["last_bos_type"] = "bearish"
                result["fresh"] = True
            result["choch"] = 0.0
        else:
            # CHoCH: تغییر ساختار
            if hh and ll:
                # Bullish CHoCH اگر آخرین low بالاتر از قبلی باشد
                result["choch"] = 0.8 if hl else 0.3
                result["trend"] = 1 if result["choch"] > 0.5 else 0
                result["last_bos_type"] = "choch_bull"
            elif lh and hl:
                result["choch"] = 0.8 if ll else 0.3
                result["trend"] = -1 if result["choch"] > 0.5 else 0
                result["last_bos_type"] = "choch_bear"
        return result

    def _order_blocks(self, o, h, l, c, highs, lows, trend):
        ...
        if trend >= 0:
            for i in range(max(1, len(lows)-1)-1, max(0, len(lows)-3)-1, -1):
                if i < 0 or i >= len(lows): continue
                li = lows[i][0]
                # پیدا کردن کندل نزولی قبل از این low
                for j in range(li, max(0, li-5), -1):
                    if j < 1 or j >= n: continue
                    if c[j-1] < o[j-1]:  # کندل نزولی
                        strength = abs(c[j-1]-o[j-1]) / max(h[j-1]-l[j-1], 1e-9)
                        if strength > 0.5:
                            obs.append({"type": "bullish", "high": float(o[j-1]), "low": float(l[j-1]),
                                        "index": j-1, "strength": strength})
                            break
                if obs: break
        # Bearish OB
        if trend <= 0:
            for i in range(max(1, len(highs)-1)-1, max(0, len(highs)-3)-1, -1):
                if i < 0 or i >= len(highs): continue
                hi = highs[i][0]
                for j in range(hi, max(0, hi-5), -1):
                    if j < 1 or j >= n: continue
                    if c[j-1] > o[j-1]:  # کندل صعودی
                        strength = abs(c[j-1]-o[j-1]) / max(h[j-1]-l[j-1], 1e-9)
                        if strength > 0.5:
                            obs.append({"type": "bearish", "high": float(h[j-1]), "low": float(o[j-1]),
                                        "index": j-1, "strength": strength})
                            break
                if obs: break
        return obs

    def _fair_value_gaps(self, o, h, l, c):
        """Fair Value Gaps: عدم تعادل ۳ کندلی."""
        fvgs = []; n = len(c)
        for i in range(2, n):
            # Bullish FVG: low[i] > high[i-2]
            if l[i] > h[i-2]:
                size = (l[i] - h[i-2]) / max(h[i]-l[i], 1e-9)
                if size > 0.15:
                    fvgs.append({"type": "bullish", "top": float(l[i]), "bottom": float(h[i-2]),
                                 "size": size, "index": i, "filled": False})
            # Bearish FVG: high[i] < low[i-2]
            if h[i] < l[i-2]:
                size = (l[i-2] - h[i]) / max(h[i]-l[i], 1e-9)
                if size > 0.15:
                    fvgs.append({"type": "bearish", "top": float(l[i-2]), "bottom": float(h[i]),
                                 "size": size, "index": i, "filled": False})
        # فقط FVG های تازه (پر نشده)
        fresh = []
        price = c[-1]
        for f in fvgs[-8:]:
            if f["type"] == "bullish" and price < f["bottom"]:
                f["fresh"] = True; fresh.append(f)
    ...
    def _liquidity_pools(self, highs, lows, current_price, atr):
        """شناسایی Equal Highs/Lows که استاپ‌ها آنجا هستند."""
        pools = []; tolerance = atr * 0.3
        # Equal Highs
        for i in range(len(highs)):
            for j in range(i+1, min(i+5, len(highs))):
                if abs(highs[i][1] - highs[j][1]) < tolerance:
                    pools.append({"type": "equal_highs", "price": float((highs[i][1]+highs[j][1])/2),
                                  "side": "above", "strength": 0.8})
                    break
        # Equal Lows
        for i in range(len(lows)):
            for j in range(i+1, min(i+5, len(lows))):
                if abs(lows[i][1] - lows[j][1]) < tolerance:
                    pools.append({"type": "equal_lows", "price": float((lows[i][1]+lows[j][1])/2),
                                  "side": "below", "strength": 0.8})
                    break
        return pools[:4]

    def _liquidity_sweep(self, highs, lows, h, l, c, atr):
        """آیا اخیراً استاپ‌هانت شده؟"""
        n = len(c); score = 0.0; sweep_type = None
        if n < 20 or not highs or not lows: return 0.0, None
        recent_high = max(hh[1] for hh in highs[-3:])
        recent_low = min(ll[1] for ll in lows[-3:])
        # Bullish sweep: low اخیر زیر حمایت قبلی اما close بالای آن
        if l[-1] < recent_low - atr*0.3 and c[-1] > recent_low:
            score = 0.9; sweep_type = "bullish_sweep"
        # Bearish sweep
        if h[-1] > recent_high + atr*0.3 and c[-1] < recent_high:
            score = 0.9; sweep_type = "bearish_sweep"
        return score, sweep_type

    def _volume_profile(self, h, l, c, v, n_bins=20):
        """POC (Point of Control), VAH, VAL."""
        if len(c) < 30 or v.sum() < 1e-9: return {"poc": 0.0, "vah": 0.0, "val": 0.0, "premium": 0.5}
        price_min = l[-60:].min() if len(l) >= 60 else l.min()
        price_max = h[-60:].max() if len(h) >= 60 else h.max()
        if price_max - price_min < 1e-9: return {"poc": 0.0, "vah": 0.0, "val": 0.0, "premium": 0.5}
        bins = np.linspace(price_min, price_max, n_bins+1)
        vol_profile = np.zeros(n_bins)
        for i in range(len(c)):
            if i >= len(v): break
            mid = (h[i] + l[i] + c[i]) / 3
            idx = int((mid - price_min) / (price_max - price_min) * n_bins)
        ...
        for idx in sorted_idx:
            cum += vol_profile[idx]; included.add(idx)
            if cum >= target: break
        vah_idx = max(included); val_idx = min(included)
        vah = (bins[vah_idx] + bins[vah_idx+1]) / 2
        val = (bins[val_idx] + bins[val_idx+1]) / 2
        cur = c[-1]
        premium = (cur - val) / max(vah - val, 1e-9)
        return {"poc": poc, "vah": vah, "val": val, "premium": clamp(premium, 0, 1)}

    def _volume_confirm(self, v, c, direction, lookback=10):
        """آیا حجم حرکت فعلی را تأیید می‌کند؟"""
        n = len(c)
        if n < lookback + 1: return 0.0
        recent_vol = np.mean(v[-lookback:])
        current_vol = v[-1]
        vol_ratio = current_vol / max(recent_vol, 1e-9)
        # حرکت هم‌جهت با حجم بالا
        if direction > 0 and c[-1] > c[-2] and vol_ratio > 1.3:
            return clamp(vol_ratio / 2.5, 0, 1)
        if direction < 0 and c[-1] < c[-2] and vol_ratio > 1.3:
            return clamp(vol_ratio / 2.5, 0, 1)
        return clamp((vol_ratio - 1.0) / 2.0, 0, 1) * 0.3

    def _premium_discount(self, h, l, c, lookback=60):
        """قیمت در premium یا discount نسبت به range اخیر."""
        if len(c) < lookback: return 0.5
        hi = h[-lookback:].max(); lo = l[-lookback:].min()
        cur = c[-1]; rng = hi - lo
        if rng < 1e-9: return 0.5
        return clamp((cur - lo) / rng, 0, 1)  # 0=discount, 1=premium
    ...
    def _rsi(self, c, p=14):
        ...
        au, ad = up[0], dn[0]
        for i in range(1, n):
            au = (au*(p-1)+up[i])/p; ad = (ad*(p-1)+dn[i])/p
            out[i] = 100 - 100 / (1 + au / max(ad, 1e-12))
        return out

    def analyze(self, o, h, l, c, v, tf="15"):
        """تحلیل اتمی کامل یک تایم‌فریم."""
        if c is None or len(c) < 80 or v is None or len(v) != len(c):
            return {"ready": False, "tf": tf}
        n = len(c); price = float(c[-1])
        atr = self._atr(h, l, c, 14)
        atr_val = float(atr[-1]) if len(atr) > 0 else price * 0.005
        highs, lows = self._swing_points(h, l, 3)
        structure = self._market_structure(highs, lows, price)
        order_blocks = self._order_blocks(o, h, l, c, highs, lows, structure["trend"])
        fvgs = self._fair_value_gaps(o, h, l, c)
        liquidity_pools = self._liquidity_pools(highs, lows, price, atr_val)
        liq_sweep, sweep_type = self._liquidity_sweep(highs, lows, h, l, c, atr_val)
        vp = self._volume_profile(h, l, c, v)
        vol_confirm = self._volume_confirm(v, c, structure["trend"])
        premium = self._premium_discount(h, l, c)
        rsi = self._rsi(c, 14)[-1]
        return {
            "ready": True, "tf": tf, "price": price,
            "atr": atr_val, "highs": highs, "lows": lows,
            "structure": structure,
            "order_blocks": order_blocks, "fvgs": fvgs,
            "liquidity_pools": liquidity_pools,
            "liq_sweep": liq_sweep, "sweep_type": sweep_type,
            "volume_profile": vp, "vol_confirm": vol_confirm,
            "premium": premium, "rsi": float(rsi),
            "trend": structure["trend"], "bos": structure["bos"],
            "choch": structure["choch"], "fresh": structure["fresh"],
        }

class MultiTFAnalyzer:
    """تحلیل‌گر چند تایم‌فریمی. H4 برای جهت، H1 برای تأیید، M15 برای ورود."""
    def __init__(self):
        self.atomic = AtomicAnalyzer()

    def confluence_score(self, an_15, an_60, an_240, direction):
        """امتیاز هم‌راستایی چندتایم‌فریمی (۰ تا ۱)."""
        score = 0.0; reasons = []
        if not (an_15.get("ready") and an_60.get("ready") and an_240.get("ready")):
            return 0.0, reasons

        # H4 Trend (سنگین‌ترین)
        if an_240["trend"] == direction:
            score += 0.35
            reasons.append(f"📊 H4 هم‌جهت ({'↑' if direction > 0 else '↓'})")
        elif an_240["trend"] == 0:
            score += 0.10
            reasons.append("📊 H4 رنج")

        # H1 Alignment
        if an_60["trend"] == direction:
            score += 0.25
            reasons.append(f"📈 H1 هم‌جهت")
        if an_60.get("bos", 0) > 0.5:
            score += 0.10
            reasons.append("🎯 H1 BOS تازه")

        # M15 Entry
        if an_15["trend"] == direction:
            score += 0.15
            reasons.append(f"🔥 M15 هم‌جهت")
        if an_15.get("fresh", False) and an_15.get("bos", 0) > 0.5:
            score += 0.10
            reasons.append("💥 M15 BOS/CHoCH تازه")

        return clamp(score, 0, 1), reasons

    def order_block_score(self, an_15, direction, price):
        """آیا قیمت در Order Block معتبر قرار دارد؟"""
        obs = an_15.get("order_blocks", [])
        if not obs: return 0.0, None
        best_ob = None; best_score = 0.0
        for ob in obs:
            if (direction > 0 and ob["type"] == "bullish") or \
               (direction < 0 and ob["type"] == "bearish"):
                # قیمت باید در محدوده OB باشد یا نزدیک آن
                if ob["low"] <= price <= ob["high"]:
                    s = ob["strength"] * 1.2
                elif direction > 0 and abs(price - ob["high"]) < an_15["atr"] * 0.3:
                    s = ob["strength"] * 0.8
                elif direction < 0 and abs(price - ob["low"]) < an_15["atr"] * 0.3:
                    s = ob["strength"] * 0.8
                else:
                    s = 0.0
                if s > best_score:
                    best_score = s; best_ob = ob
        return clamp(best_score, 0, 1), best_ob

    def fvg_score(self, an_15, direction, price):
        """آیا قیمت نزدیک FVG معتبر است؟"""
        fvgs = an_15.get("fvgs", [])
        if not fvgs: return 0.0, None
        best = None; best_score = 0.0
        for fvg in fvgs:
            if (direction > 0 and fvg["type"] == "bullish") or \
               (direction < 0 and fvg["type"] == "bearish"):
                if direction > 0 and fvg["bottom"] <= price <= fvg["top"]:
                    s = min(1.0, fvg["size"] * 2)
                elif direction < 0 and fvg["bottom"] <= price <= fvg["top"]:
                    s = min(1.0, fvg["size"] * 2)
                else:
                    dist = abs(price - (fvg["top"] + fvg["bottom"]) / 2)
                    if dist < an_15["atr"] * 0.5:
                        s = min(1.0, fvg["size"]) * (1 - dist / (an_15["atr"] * 0.5))
                    else:
                        s = 0.0
                if s > best_score:
                    best_score = s; best = fvg
        return clamp(best_score, 0, 1), best

    def liquidity_score(self, an_15, direction):
        """امتیاز liquidity sweep."""
        sweep = an_15.get("liq_sweep", 0)
        sweep_type = an_15.get("sweep_type")
        if sweep > 0.5:
            if (direction > 0 and "bullish" in str(sweep_type)) or \
               (direction < 0 and "bearish" in str(sweep_type)):
                return sweep, "🎯 استاپ‌هانت هم‌جهت"
        return 0.0, None

    def premium_discount_score(self, an_15, direction):
        """Premium/Discount مناسب؟"""
        premium = an_15.get("premium", 0.5)
        if direction > 0 and premium < 0.5:
            return clamp((0.5 - premium) * 2, 0, 1), f"📉 در Discount ({premium*100:.0f}٪)"
        if direction < 0 and premium > 0.5:
            return clamp((premium - 0.5) * 2, 0, 1), f"📈 در Premium ({premium*100:.0f}٪)"
        return 0.0, None

# ═══════════ ۷) هندسه‌دان فیبوناچی ═══════════
class GeometryAnalyzer:
    ...
    def fib_levels(self, high, low):
        diff = high - low
        return {"0.382": high - diff * 0.382, "0.5": high - diff * 0.5,
                "0.618": high - diff * self.GOLDEN, "1.0(Low)": low}
    def analyze(self, o, h, l, c):
        if c is None or len(c) < 20: return {"ready": False, "levels": {}, "trend": 0}
        highs, lows = self.find_swings(h, l, window=3)
        if not highs or not lows: return {"ready": False, "levels": {}, "trend": 0}
        levels = self.fib_levels(max(highs[-3:]), min(lows[-3:]))
        trend = 0
        if len(highs) >= 2 and len(lows) >= 2:
            if highs[-1] > highs[-2] and lows[-1] > lows[-2]: trend = 1
            elif highs[-1] < highs[-2] and lows[-1] < lows[-2]: trend = -1
...
class EternalNowPerception:
    ...
    def perceive(self, closes, ratios=(1, 4, 16, 32)):
        ...
        for r in ratios:
            ...
            d = short - long; scale = max(abs(long), 1e-9)
            directions.append(np.sign(d))
            momentums.append(clamp(d / scale * 20, -1, 1))
        if not directions:
            return {"coherence": 0.5, "direction": 0, "eternal_now": 0.5, "n_scales": 0}
        coherence = abs(float(np.mean(directions)))
        unified_dir = int(np.sign(np.mean(directions)))
        mom_consensus = abs(float(np.mean(momentums)))
        self.coherence = coherence; self.eternal_now_score = 0.6*coherence + 0.4*mom_consensus
        self.last_direction = unified_dir; self.n_scales = len(directions)
        return {"coherence": coherence, "direction": unified_dir,
                "eternal_now": self.eternal_now_score, "n_scales": len(directions)}
...
class AdvancedNumericalPerception:
    ...
    def perceive(self, price):
        ...
        return {"velocity": d["velocity"], "acceleration": d["acceleration"], "jerk": d["jerk"],
                ...
                "harmonic": harm, "depth": self.numerical_iq * self.shf}
...
class VisualPerception:
    ...
    def feed(self, o, h, l, c): self.candles.append((float(o), float(h), float(l), float(c)))
    def detect_patterns(self):
        ...
        return {"pattern": patterns[-1] if patterns else "none", "strength": clamp(strength, 0, 1), "bias": bias}
    def perceive(self, o, h, l, c):
        self.feed(o, h, l, c); return self.detect_patterns()

# ═══════════ ۱۰) آرایه حسی فرازمینی + فضازمان ═══════════
XENO_SENSE_TYPES = ["بینایی 👁️", "شنوایی 👂", "لامسه ✋", "بویایی 👃", "چشایی 👅"]
XENO_ALIEN_SENSE_NAMES = ["حس میدان", "حس خمیدگی", "حس آنتروپی", "حس رتبه", "حس شتاب",
                          "حس تشدید", "حس همبستگی", "حس شکاف", "حس پژواک", "حس سایه",
                          "حس رزونانس", "حس چگالی", "حس شار", "حس گرداب", "حس انبساط",
                          "حس انقباض", "حس فاز", "حس تداخل", "حس هارمونیک", "حس آشوب"]
class XenoSensor:
...
class SpacetimePerception:
    ...
    def feed(self, price, tick): self.field.append((float(tick), float(price)))
    def compute_geometry(self):
        if len(self.field) < 10:
            g = {"arc_length": 0.0, "curvature": 0.0, "slope": 0.0, "expansion": 0.0,
        ...
        ds = np.sqrt(dt**2 + dp**2); arc_length = float(np.sum(ds))
...
# ═══════════ ۱۱) آگاهی بدن + حواس جمع ═══════════
class InteroceptionSystem:
    def __init__(self):
        self.body_awareness = 0.5; self.signals = {}
...
class AttentionSystem:
    def __init__(self):
        self.focus_level = 0.5; self.attention_target = None
...
# ═══════════ ۱۲) ذهن ریاضیدان IQ 200 ═══════════
class MathematicalMind:
    def __init__(self, iq_factor=2.0):
        self.iq_factor = iq_factor; self.growth = 0.0
        self.prediction_history = deque(maxlen=200); self.accuracy = 0.5; self.iq_display = 200
    def analyze_from_signals(self, signals, current_price):
        ...
        dirs = [s["signal"] for s in signals]; convs = [s["conviction"] for s in signals]
        total_conv = sum(abs(c) for c in convs) + 1e-9
        weighted_dir = sum(d*abs(c) for d, c in zip(dirs, convs)) / total_conv
        prob_up = 0.5 + 0.5 * math.tanh(weighted_dir * self.iq_factor)
        ...
        self.iq_display = int(200 + self.growth * 100)
...
# ═══════════ ۱۴) 🏆 مدیر پوزیشن Elite ═══════════
class ElitePositionManager:
    """مدیر پوزیشن با Trailing Stop هوشمند + Partial Profits + Time Stop + Break Even."""
    def __init__(self, discipline, patience):
        self.discipline = discipline; self.patience = patience
    def manage(self, pos, tk, regulator, analysis=None, now_ms=0):
        side = pos["side"]; entry = pos["entry"]
        cur = tk["bid"] if side > 0 else tk["ask"]
        initial_risk = abs(entry - pos["sl"])
        if initial_risk <= 0: return None
        r_mult = (cur - entry) * side / initial_risk

        # ۱. Break Even بعد از ۱R
        if r_mult >= config.BREAK_EVEN_R and not pos.get("be_set"):
            pos["sl"] = entry
            pos["be_set"] = True

        # ۲. Partial Profits
        if r_mult >= 1.0 and not pos.get("partial_1r"):
            pos["partial_1r"] = True
            # ۳۰٪ سود در ۱R
            pos["closed_partial"] = pos.get("closed_partial", 0.0) + 0.30
            return ("partial", f"🏆 ۳۰٪ سود در ۱R قفل شد")
        if r_mult >= 2.0 and not pos.get("partial_2r"):
            pos["partial_2r"] = True
            pos["closed_partial"] = pos.get("closed_partial", 0.30) + 0.40
            return ("partial", f"🏆 ۴۰٪ سود در ۲R قفل شد")

        # ۳. Trailing Stop هوشمند: بر اساس swing points
        ...
        # ۴. Time Stop: خروج بعد از X دقیقه بی‌ثمر
        if now_ms and pos.get("open_ts"):
            age = (now_ms - pos["open_ts"]) / 60000.0
            if age > config.TIME_STOP_MIN and r_mult < 0.3:
                return ("close", f"⏰ توقف زمان ({age:.0f} دقیقه بی‌ثمر)")

        # ۵. قفل سود در اوج طمع
        greed_threshold = 3.0 - regulator.greed * 0.5
        if r_mult >= greed_threshold and momentum_aligned < -0.2:
            return ("close", "🔒 قفل سود (ضد طمع)")

        # ۶. خروج اضطراری
        if r_mult <= -0.95:
            return ("close", "🚨 خروج اضطراری")
...
class TradingEngine:
    ...
    def _init_fresh(self):
        ...
        self.daily_trades = 0; self.daily_pnl = 0.0
        self.daily_date = time.strftime("%Y-%m-%d"); self.lesson_stats = {}
    def _roll_day(self):
        today = time.strftime("%Y-%m-%d")
        if today != self.daily_date:
            self.daily_date = today; self.daily_trades = 0; self.daily_pnl = 0.0
    def trading_allowed(self):
        self._roll_day()
        return self.daily_pnl > -config.DAILY_LOSS_LIMIT * self.initial_capital
    ...
    def top_lessons(self, n=5):
        ...
        items.sort(key=lambda x: -x[3]); return items[:n]
    ...
    def available_margin(self, coin):
        ...
        s = self.slots[sid]; return max(0.0, s["capital"] - s["margin_used"])
    def total_margin_used(self): return sum(s["margin_used"] for s in self.slots)

    def open_position(self, coin, side, price, spread, margin, leverage, tp, sl, conviction, reasons, tick, mode):
        with self.lock:
            self._roll_day()
            ...
            pos = {"coin": coin, "side": side, "entry": entry, "size": size, "margin": margin,
                   ...
                   "peak": entry, "trailed": False, "trail_amount": 0.0,
                   "be_set": False, "partial_1r": False, "partial_2r": False, "closed_partial": 0.0}
            self.open_positions[coin] = pos
            self.db.save_open_position(self.name, pos); self._persist_state()
            return pos

    def close_partial(self, coin, fraction):
        """بستن بخشی از پوزیشن."""
        with self.lock:
            if coin not in self.open_positions: return None
            pos = self.open_positions[coin]
            side = pos["side"]; size = pos["size"] * fraction
            cur = pos.get("peak", pos["entry"])
            gross = (cur - pos["entry"]) * side * side  # side = 1 or -1
            # تقریب: gross = (cur - entry) * side * (size)
            gross = (cur - pos["entry"]) * side * (pos["size"] * fraction)
            exit_comm = cur * size * self.fee_rate
            net = gross - exit_comm
            self.realized_pnl += net; self.total_commission += exit_comm
            if net > 0: self.wins += 0.5 * fraction  # weighted win
            return net

    def live_pnl(self, pos, bid, ask):
        side = pos["side"]; cur = bid if side > 0 else ask
        remaining_frac = 1.0 - pos.get("closed_partial", 0.0)
        size = pos["size"] * remaining_frac
        gross = (cur - pos["entry"]) * side * size
        exit_comm = cur * size * self.fee_rate
        net = gross - pos["commission"] - exit_comm - pos["spread_cost"] * remaining_frac
        roi = net / max(pos["margin"], 1e-9) * 100
        return {"gross": gross, "net": net, "roi": roi, "cur": cur,
    ...
    def close_position(self, coin, exit_price, reason):
        with self.lock:
            ...
            remaining_frac = 1.0 - pos.get("closed_partial", 0.0)
            size = pos["size"] * remaining_frac
            gross = (exit_price - pos["entry"]) * side * size
            exit_comm = exit_price * size * self.fee_rate
            net = gross - pos["commission"] - exit_comm - pos["spread_cost"] * remaining_frac
            self.realized_pnl += net; self.total_commission += exit_comm
            self.daily_pnl += net
...
class MultiSymbolScanner:
    def __init__(self, http, symbol_limit, deep_limit, kline_limit):
        self.http = http; self.symbol_limit = symbol_limit
        self.deep_limit = deep_limit; self.kline_limit = kline_limit
        self.lock = threading.RLock()
    ...
    def fetch_klines(self, symbol, interval):
        d = self.http.get("/v5/market/kline", {"category": "linear", "symbol": symbol,
                                                "interval": interval, "limit": self.kline_limit})
    ...
    def scan_cycle(self):
        ...
        now = time.time()
        candidates = [s for s in scored if now - self.deep_last_scan.get(s, 0) >= config.SCAN_RESCAN_SEC][:self.deep_limit]
        for s in candidates:
            klines = {}
            for tf in config.TFS.keys():
                kl = self.fetch_klines(s, tf)
                if kl: klines[tf] = kl
            if "15" not in klines: continue
            analyses = {}
            for tf, kl in klines.items():
                analyses[tf] = self.atomic.analyze(kl["o"], kl["h"], kl["l"], kl["c"], kl["v"], tf)
            with self.lock:
                self.deep[s] = {"klines": klines, "analyses": analyses, "ts": now}
                self.deep_last_scan[s] = now
        self.scan_cycles += 1
    def background(self, period):
        print(f"🔍 اسکنر Elite: {self.symbol_limit} ارز | عمیق {self.deep_limit} | هر {period}s")
        while True:
            try: self.scan_cycle()
...
GENE_CATALOG = [
    ...
    ("GEN-DISC", "انضباط آهنین", "discipline", 0.92),
    ("GEN-RISKMG", "مدیریت ریسک برتر", "risk_management", 0.92),
    ("GEN-EMOCTL", "کنترل احساسات", "emotional_control", 0.92),
    ("GEN-PATIENCE", "صبر شکارچی", "patience", 0.88),
    ("GEN-PRECISION", "دقت اسنایپر", "precision", 0.92),
    ("GEN-NUMIQ", "هوش عددی برتر", "numerical_iq", 0.95),
    ("GEN-VISIQ", "هوش بصری برتر", "visual_iq", 0.95),
    ("GEN-ADAPT", "سازگاری سریع", "adaptability", 0.88),
    ("GEN-CALM", "آرامش تحت فشار", "calm", 0.92),
    ("GEN-FORESIGHT", "آینده‌نگری", "foresight", 0.88),
    ("GEN-PRES", "ادراک اکنون ابدی", "presence", 0.92),
    ("GEN-SYN", "یکپارچگی حسی", "synesthesia", 0.95),
    ("GEN-XENO", "حسگرهای فرازمینی", "xeno_sense", 0.95),
    ("GEN-SPACETIME", "ادراک فضازمانی", "spacetime", 0.95),
    ("GEN-INTERO", "آگاهی بدن", "interoception", 0.95),
    ("GEN-ATTN", "حواس جمع", "attention", 0.95),
    ("GEN-CARDNEURAL", "شبکه عصبی قلب", "cardiac_neural", 0.95),
    ("GEN-INTCORE", "هسته شهود", "intuition_core", 0.95),
    ("GEN-BINARY", "جریان تفکر باینری", "binary_cognition", 0.95),
    ("GEN-OBS", "چشم مشاهده‌گر", "observer_eye", 0.95),
]
...
TRADER_GENE_KEYS = ["discipline", "risk_management", "emotional_control", "patience", "precision",
                    ...
                    "intuition_core", "binary_cognition", "observer_eye"]
...
class Phenotype:
    ...
    intuition_core: float = 0.7; binary_cognition: float = 0.7; observer_eye: float = 0.7
...
class Genome:
    ...
    def build_phenotype(self):
        ...
        for key in TRADER_GENE_KEYS: setattr(p, key, 0.20 + 0.80 * E(key))
        apex = [getattr(p, k) for k in TRADER_GENE_KEYS]
        p.superhuman_factor = clamp(1.3 + (sum(apex) / len(apex)) * 1.4, 1.0, 3.0)
        return p
    def describe(self):
    ...
    def step(self, dt, base_rate, act_rate, activity):
        ...
        for i, reg in enumerate(self.registers):
            ...
            if self.rng.random() < base_rate + act_rate * float(act):
                ...
                reg["psi"] = psi_new
                events.append(QuantumEvent(0, i, format(idx, f"0{reg['n']}b"),
                                           idx / max(1, reg["dim"] - 1)))
...
REG_ORDER = ["BRAINSTEM", "THALAMUS", "AMYGDALA", "HIPPOCAMPUS",
             ...
             "INTUITION_CORTEX", "OBSERVATORY_CORTEX", "BINARY_GATE",
...
REGION_FA = {"BRAINSTEM": "ساقه‌ی مغز", "THALAMUS": "تالاموس", "AMYGDALA": "آمیگدال",
             ...
             "OBSERVATORY_CORTEX": "قشر مشاهده‌گر 🔭", "BINARY_GATE": "دروازه باینری ⚡",
...
PATHWAYS = [
    ...
    ("THALAMUS", "OBSERVATORY_CORTEX", 0.09, 1.8, True),
    ("OBSERVATORY_CORTEX", "INTUITION_CORTEX", 0.10, 2.0, True),
    ("OBSERVATORY_CORTEX", "CORTEX", 0.07, 1.6, True),
...
class Region:
    def __init__(self, name, n, rng):
        ...
        self.spiked_now = np.zeros(self.n, dtype=bool)
        self.spike_buffer = [np.zeros(self.n, dtype=bool) for _ in range(MAX_DELAY)]
        self.buf_idx = 0; self.activity = 0.0
    def reset_current(self, noise=0.35): self.I = self.rng.normal(1.6, noise, self.n)
    def add_current(self, idx_or_all, amount):
        if idx_or_all is None: self.I += amount
        else: self.I[idx_or_all] += amount
    def step(self, substeps=2, dt_ms=1.0):
        spikes = np.zeros(self.n, dtype=bool)
        I_eff = np.clip(self.I, -40.0, 50.0)
...
class Brain:
    def __init__(self, phenotype, rng, scale=None):
        ...
        counts = {"BRAINSTEM": 100*scale, "THALAMUS": 200*scale, "AMYGDALA": 130*scale,
                  "HIPPOCAMPUS": 220*scale,
                  "VISUAL_CORTEX": (60 + 200*phenotype.visual_iq*shf)*scale,
                  "NUMERICAL_CORTEX": (50 + 220*phenotype.number_sense*shf)*scale,
                  "SPACETIME_CORTEX": (60 + 260*phenotype.spacetime*shf)*scale,
                  "INTUITION_CORTEX": (120 + 420*phenotype.intuition_core*shf)*scale,
                  "OBSERVATORY_CORTEX": (120 + 420*phenotype.observer_eye*shf)*scale,
                  "BINARY_GATE": (60 + 240*phenotype.binary_cognition*shf)*scale,
                  "BASAL_GANGLIA": 150*scale,
                  "CORTEX": 500*phenotype.cortical_expansion*scale}
        ...
        print(f"🧠 مغز Elite: {sum(counts.values())} نورون | موازی: {config.PARALLEL_BRAIN}")
    def _build_connectome(self):
        for src, dst, p, strength, plast in PATHWAYS:
            ...
            for d in range(1, MAX_DELAY):
                ...
                self.syn[dst].append({"src": src, "pre": pre[m], "post": post[m], "w": w[m],
    ...
    def _gather_target(self, dst):
        ...
        for b in self.syn[dst]:
            ...
            if not sp.any(): continue
            idx = b["post"][sp]
            gain = self._exc_gain if b["w"][0] >= 0 else self._inh_gain
            I += np.bincount(idx, weights=b["w"][sp] * gain, minlength=n)
    def step(self, sensory, neurochem, quantum_events, fatigue, plasticity_factor=1.0,
             ...
             intuition_level=0.0, evidence_abs=0.0, binary_pulse=0.0, fib_lock=0.0):
        ...
        regs["BRAINSTEM"].add_current(None, 5.0 + 5.0*arousal + 3.0*fatigue + 8.0*sensory.get("pain", 0)
                                      + 2.0*body_awareness + 3.0*cardiac_afferent)
        ...
        regs["OBSERVATORY_CORTEX"].add_current(None, 6.0*evidence_abs + 3.0*attention + 2.0*sixth_sense)
        regs["INTUITION_CORTEX"].add_current(None, 6.0*intuition_level + 4.0*sixth_sense
                                             + 3.0*presence + 4.0*fib_lock + 2.0*cardiac_afferent)
        ...
        for dst in ("CORTEX", "HIPPOCAMPUS", "NUMERICAL_CORTEX", "VISUAL_CORTEX",
                    "SPACETIME_CORTEX", "INTUITION_CORTEX", "OBSERVATORY_CORTEX"):
            Rt = regs[dst]
            for b in self.syn[dst]:
                if not b["plastic"] or len(b["w"]) < 8: continue
                Rs = regs[b["src"]]
                k = min(96, len(b["w"]))
                idx = self.rng.integers(0, len(b["w"]), k)
                mask = Rs.spiked_now[b["pre"][idx]] & Rt.spiked_now[b["post"][idx]]
                if mask.any():
                    sel = idx[mask]
                    b["w"][sel] += lr * np.sign(b["w"][sel])
                    np.clip(b["w"], -4.0, 4.0, out=b["w"])
                b["w"] *= 0.99985
        acts = {k: float(r.activity) for k, r in regs.items()}
        self.eeg = float(np.mean(regs["CORTEX"].v))
...
class CardiacNervousSystem:
    def __init__(self, rng, n_neurons):
        self.rng = rng; self.n = max(20, int(n_neurons))
        self.a = np.full(self.n, 0.02); self.b = np.full(self.n, 0.20)
        self.c = np.full(self.n, -65.0); self.d = np.full(self.n, 8.0)
        n_inh = max(2, int(self.n * 0.25))
        self.a[:n_inh] = 0.10; self.b[:n_inh] = 0.25; self.d[:n_inh] = 2.0
        self.v = np.full(self.n, -65.0); self.u = self.b * self.v.copy()
        self.I = np.zeros(self.n); self.activity = 0.0; self.coherence = 0.5
...
class CardiacSystem:
    ...
    def step(self, dt, sympathetic, parasympathetic, stretch, o2, market_signal=0.0, market_vol=0.0):
        ...
        target_hr = clamp(target_hr, 40, 200); self.last_hr = target_hr
    ...
    def rhythm_coherence(self):
        ...
        intervals = np.array(self.recent_intervals); mean_iv = np.mean(intervals)
        if mean_iv < 1e-9: return 0.6
        cv = np.std(intervals) / mean_iv
        return clamp(1.0 - cv * 3, 0, 1)
    def coherence(self, emotional_calm=1.0):
        rhythm = self.rhythm_coherence(); neural_coh = self.neural.coherence
...
class Body:
    def __init__(self, phenotype, rng):
        ...
        self.sensory = {}
        self.cardiac = CardiacSystem(rng, phenotype.heart_base, neural_scale=phenotype.cardiac_neural * 2.0)
    ...
    def feed_information(self, novelty):
        ...
        self.last_info_feed = feed; return feed
...
class ConsciousnessMeter:
    def __init__(self, n_regions, window=160):
        self.window = window; self.history = deque(maxlen=window)
    ...
    def update(self, activities, brainstem, body_ok_factor, quantum_coh, info_level=0.5,
               ...
               heart_vitality=0.6, body_awareness=0.5, attention=0.5,
               intuition_level=0.0, evidence_abs=0.0, fib_lock=0.0):
        ...
        x = (0.28*activities.get("CORTEX", 0) + 0.15*activities.get("THALAMUS", 0)
             + 0.10*activities.get("HIPPOCAMPUS", 0) + 0.10*activities.get("BASAL_GANGLIA", 0)
             + 0.12*activities.get("SPACETIME_CORTEX", 0)
             + 0.13*activities.get("INTUITION_CORTEX", 0)
             + 0.08*activities.get("OBSERVATORY_CORTEX", 0) + 0.07*attention)
        self.workspace = ema(self.workspace, 1.0 / (1.0 + math.exp(-40.0 * (x - 0.05))), 0.10)
        self.workspace = max(self.workspace, 0.40)
        raw = 100.0 * clamp(0.22*self.workspace + 0.20*self.arousal + 0.11*self.phi
                            + 0.07*self.integration + 0.07*presence + 0.09*sixth_sense
                            + 0.05*spacetime_richness + 0.07*heart_vitality
                            + 0.05*body_awareness + 0.05*attention
                            + 0.08*intuition_level + 0.06*fib_lock + 0.04*evidence_abs)
...
# ═══════════ ۲۴) ذهن ═══════════
CATEGORY_FA = {"market": "تحلیل مارکت", "trade": "معامله‌گری", "insight": "بینش",
...
CATEGORY_COLORS = {"market": "#38bdf8", "trade": "#4ade80", "insight": "#fde047", "sensory": "#2dd4bf",
                   ...
                   "observe": "#2dd4bf"}
T = {"market": ["📊 ساختار بازار را با دقت Elite تحلیل می‌کنم؛ {obs}",
                "📈 BOS/CHoCH را در تایم‌فریم‌های چندگانه بررسی می‌کنم؛ {obs}"],
     ...
     "trade": ["🏆 ستاپ A+ را ارزیابی می‌کنم؛ R:R بالای ۱:۲.۵."],
     "insight": ["💡 Order Block و FVG هم‌راستا با H4 یافته‌ام."],
     "memory": ["💾 آموخته‌هایم را مرور می‌کنم؛ {lesson}"],
     "emotion": ["🧘 احساساتم را تماشا می‌کنم؛ اجازه نمی‌دهم تصمیم بگیرند."],
     "presence": ["🕉️ در اکنون ابدی حضور دارم."],
     "self": ["من Elite Trader هستم و مارکت را با دقت اتمی می‌بینم."],
     "quantum": ["جرقه‌ای کوانتومی در ذهنم فرو افتاد: «{bits}»."],
     "heart": ["قلبم با آهنگ فیبوناچی می‌تپد."],
     "observe": ["🔭 شواهد اتمی را تحلیل می‌کنم؛ هم‌رایی {agree}٪."]}
...
class StreamOfConsciousness:
    ...
    def _market_obs(self, market_signal):
        if market_signal > 0.4: return "فشار خرید قوی و BOS صعودی"
        elif market_signal < -0.4: return "فشار فروش قوی و BOS نزولی"
        elif market_signal > 0.15: return "تمایل ملایم به صعود"
        elif market_signal < -0.15: return "تمایل ملایم به نزول"
        else: return "بازار در تعادل و رنج"
    def step(self, tick, cons, activities, neurochem, body, sensory, quantum_events,
        ...
        weights = {
            ...
            "observe": 3.0 if obs_active else 0.5,
        }
        keys = list(weights); vals = np.array([max(1e-4, weights[k]) for k in keys])
        ...
        text = self._compose(cat, quantum_events, market_signal, n_scanned, top_lesson,
                             sensory_array, spacetime, body, evidence, agreement)
    ...
    def _compose(self, cat, qevents, market_signal, n_scanned, top_lesson, sensory_array,
        ...
        if cat == "body":
            ...
            return tmpl.format(hr=int(body.heart_rate), energy=f"{body.energy*100:.0f}٪")
        ...
        if cat == "observe":
            return T["observe"][0].format(agree=int(agreement * 100))
        if cat in T: return T[cat][int(self.rng.integers(0, len(T[cat])))]
        return "…"
...
class Environment:
    ...
    def step(self):
        ...
        if self.rng.random() < 0.03:
            kind = self.rng.choice(["light", "sound", "touch", "pain", "danger"],
                                   p=[0.35, 0.28, 0.17, 0.10, 0.10])
...
class Organism:
    def __init__(self, seed=None, scanner=None, memory_db=None, trading_db=None, http=None):
        ...
        self.mtf = MultiTFAnalyzer()
        self.mode = "elite"; self.mode_params = TRADING_MODES["elite"]
    ...
    def rebirth(self, seed=None):
        ...
        self.math_mind = MathematicalMind(iq_factor=2.0)
        self.regulator = EmotionalRegulator(self.phenotype.discipline,
                                            self.phenotype.emotional_control,
                                            self.phenotype.calm)
        self.active_manager = ElitePositionManager(self.phenotype.discipline, self.phenotype.patience)
        ...
        self.swing_engine = TradingEngine("elite", config.INITIAL_CAPITAL, config.N_SLOTS,
                                          config.TAKER_FEE, self.trading_db)
        self.scalp_engine = TradingEngine("scalp", config.INITIAL_CAPITAL, config.N_SLOTS,
                                          config.TAKER_FEE, self.trading_db)
        self.engines = {"elite": self.swing_engine, "scalp": self.scalp_engine}
        ...
        self.last_evidence = 0.0; self.last_agreement = 0.5
        self.math_prob = 0.5
        self.symbol_memory = {}  # برای banned symbols
        self.milestones, self._ms_flags = [], set()
        self._add_milestone(f"🏆 متولد شد — ژنوم {self.genome.signature()} | "
                            f"سوپرهیومن ×{shf:.2f} | Elite Trader")
    ...
    def reset_all_trading_memory(self):
        ...
        self.peak_equity = config.INITIAL_CAPITAL
        self.symbol_memory = {}
        self._add_milestone("🗑️ حافظه معاملات ریست شد")

    def _symbol_banned(self, coin):
        mem = self.symbol_memory.get(coin)
        return bool(mem and mem["n"] >= config.SYMBOL_BAN_TRADES and mem["pnl"] < 0)
    ...
    def _make_signal(self, coin, tk, an_15, an_60, an_240, geom, energy, mp, attention):
        """🏆 ساخت سیگنال با منطق Elite Trader (Multi-TF + SMC)."""
        if not an_15.get("ready") or not an_60.get("ready") or not an_240.get("ready"):
            return None
        price = tk["price"]; spread = tk["spread"]

        # ۱. تعیین جهت از H4
        htf_dir = an_240.get("trend", 0)
        if htf_dir == 0: return None  # بازار رنج، ترید نمی‌کنیم

        direction = 1 if htf_dir > 0 else -1

        # ۲. امتیاز هم‌راستایی چند تایم‌فریمی
        mtf_score, mtf_reasons = self.mtf.confluence_score(an_15, an_60, an_240, direction)
        if mtf_score < config.GATE_HTF_ALIGN + config.GATE_MTF_ALIGN:
            return None  # هم‌راستایی کافی نیست

        # ۳. امتیاز Order Block
        ob_score, ob_detail = self.mtf.order_block_score(an_15, direction, price)

        # ۴. امتیاز Fair Value Gap
        fvg_score, fvg_detail = self.mtf.fvg_score(an_15, direction, price)

        # ۵. Liquidity Sweep
        liq_score, liq_reason = self.mtf.liquidity_score(an_15, direction)

        # ۶. Premium/Discount
        pd_score, pd_reason = self.mtf.premium_discount_score(an_15, direction)

        # ۷. Market Structure (BOS/CHoCH)
        struct_score = an_15.get("bos", 0) * 0.7 + an_15.get("choch", 0) * 0.5
        if an_15.get("fresh", False): struct_score *= 1.5

        # ۸. Volume Confirmation
        vol_score = an_15.get("vol_confirm", 0)

        # 🔢 محاسبه conviction نهایی
        conviction = 0.0
        conviction += mtf_score * 0.25
        conviction += ob_score * 0.20
        conviction += fvg_score * 0.15
        conviction += liq_score * 0.10
        conviction += pd_score * 0.10
        conviction += struct_score * 0.15
        conviction += vol_score * 0.05
        conviction = clamp(conviction, 0, 1)

        # تقویت با شهود Elite
        shf = self.phenotype.superhuman_factor
        conviction = clamp(conviction * (1 + 0.25*self.phenotype.intuition) * (0.8 + 0.2*shf), 0, 1)

        # تقویت با توجه و انرژی
        conviction = clamp(conviction * (0.8 + 0.3*attention) * (0.8 + 0.3*energy), 0, 1)

        reasons = list(mtf_reasons)
        if ob_score > config.GATE_OB_SCORE:
            reasons.append(f"🏦 Order Block ({ob_score*100:.0f}٪)")
        if fvg_score > config.GATE_FVG_SCORE:
            reasons.append(f"📊 FVG تازه ({fvg_score*100:.0f}٪)")
        if liq_score > config.GATE_LIQ_SWEEP:
            reasons.append(f"🎯 Liquidity Sweep ({liq_reason})")
        if pd_score > config.GATE_PREMIUM:
            reasons.append(pd_reason)
        if struct_score > config.GATE_STRUCTURE:
            reasons.append(f"💥 BOS/CHoCH تازه ({struct_score*100:.0f}٪)")
        if vol_score > config.GATE_VOLUME:
            reasons.append(f"📊 تأیید حجم ({vol_score*100:.0f}٪)")

        # 🧮 محاسبه TP/SL بر اساس SMC
        atr = an_15["atr"]
        # SL: پشت swing اخیر یا OB
        if direction > 0:
            if an_15["lows"]:
                sl = min(ll[1] for ll in an_15["lows"][-2:]) - atr * 0.3
            else:
                sl = price - atr * 1.5
        else:
            if an_15["highs"]:
                sl = max(hh[1] for hh in an_15["highs"][-2:]) + atr * 0.3
            else:
                sl = price + atr * 1.5

        sl_dist = abs(price - sl)
        if sl_dist < atr * 0.5: sl_dist = atr * 1.5
        if sl_dist > price * 0.05: sl_dist = price * 0.05  # حداکثر ۵٪

        # TP: حداقل R:R = 2.5
        tp_dist = sl_dist * mp["tp_mult"] / mp["sl_mult"]  # tp_mult/sl_mult = R:R
        ...
        # بررسی منطقی R:R
        actual_rr = tp_dist / max(sl_dist, 1e-9)
        if actual_rr < config.MIN_RR:
            return None  # R:R کافی نیست

        # تقویت با هندسه فیبوناچی
        sniper = False
        if geom.get("ready") and geom.get("levels"):
            for lvl_name, lvl_price in geom["levels"].items():
                dist = abs(price - lvl_price) / price
                if dist < 0.004 and geom.get("trend") == direction:
                    conviction = clamp(conviction + 0.08, 0, 1)
                    sniper = True
                    reasons.append(f"🎯 فیبوناچی {lvl_name}")
                    break

        # تقویت با ریاضی
        math_analysis = self.math_mind.analyze_from_signals(
            self.current_signals if self.current_signals else
            [{"signal": direction, "conviction": conviction}], price)
        math_prob = math_analysis["probability"]
        math_conf = math_analysis["confidence"]
        if (direction == 1 and math_prob > 0.55) or (direction == -1 and math_prob < 0.45):
            conviction = clamp(conviction * (1 + 0.2 * math_conf), 0, 1)
            reasons.append(f"🧠 احتمال ریاضی {math_prob*100:.0f}٪")

        # تطبیق با یادگیری
        lesson_adj = self.engine.lesson_adjustment(reasons)
        conviction = clamp(conviction + lesson_adj * (0.5 + 0.5 * self.phenotype.learning), 0, 1)
        ...
        return {"symbol": coin, "side": direction, "price": price, "spread": spread,
                "spread_pct": tk["spread_pct"], "tp": tp, "sl": sl, "conviction": conviction,
                "signal": direction, "volatility": atr, "change24h": tk["change24h"],
                "turnover": tk["turnover"], "reasons": reasons, "sniper": sniper,
                "rr": actual_rr, "mtf_score": mtf_score, "ob_score": ob_score,
                "fvg_score": fvg_score, "liq_score": liq_score, "pd_score": pd_score,
                "struct_score": struct_score, "vol_score": vol_score,
                "geometry": geom, "lesson_adj": lesson_adj}
    ...
    def _is_rational(self, sig, mp):
        ...
        # بررسی R:R حداقل
        rr = tp_dist_pct / max(sl_dist_pct, 1e-9)
        if rr < config.MIN_RR: return False
        return True

    def _kelly_size(self, conviction, rr):
        """Kelly Criterion برای سایز پوزیشن."""
        p = conviction  # احتمال برد
        q = 1 - p
        kelly = (p * rr - q) / rr
        kelly = max(0, min(kelly, config.KELLY_FRACTION))
        return kelly

    def _trading_step(self, scanner_snapshot):
        if self._reset_cooldown > 0:
            self._reset_cooldown -= 1; return
        mp = self.mode_params
        ...
        for coin in list(self.engine.open_positions.keys()):
            ...
            if reason:
                ...
                continue
            action = self.active_manager.manage(pos, tk, self.regulator, None, now_ms=now_ms)
            if action:
                close_type, why = action
                if close_type == "partial":
                    # بستن بخشی از پوزیشن (بدون حذف از لیست)
                    fraction = 0.30 if "1R" in why else 0.40
                    self.engine.close_partial(coin, fraction)
                    continue
        ...
        for coin, d in deep.items():
            tk = tickers.get(coin)
            if not tk: continue
            an_dict = analyses.get(coin, {})
            an_15 = an_dict.get("15", {})
            an_60 = an_dict.get("60", {})
            an_240 = an_dict.get("240", {})
            if not an_15.get("ready"): continue
            kl = d["klines"].get("15")
            if not kl: continue
            geom = self.geometry.analyze(kl["o"], kl["h"], kl["l"], kl["c"])
            sig = self._make_signal(coin, tk, an_15, an_60, an_240, geom, energy, mp, attention)
            if sig: signals.append(sig)

        # مرتب‌سازی بر اساس conviction و R:R
        signals.sort(key=lambda s: s["conviction"] * (1 + 0.1 * s["rr"]), reverse=True)
        self.current_signals = signals[:12]
        if signals:
            self.focus_geometry = signals[0].get("geometry", {})
            self.last_evidence = signals[0]["mtf_score"]
            self.last_agreement = signals[0]["ob_score"]

        # تصمیم ورود
        if self.tick % mp["decision_interval"] != 0: return
        if self.engine.daily_trades >= mp["max_daily"]: return
        if not self.engine.trading_allowed(): return
        if len(self.engine.open_positions) >= config.MAX_CONCURRENT_POSITIONS: return
        cc = self.cardiac_coherence()
        if self.mode == "scalp" and cc < 0.10: return
        ...
        for sig in signals:
            ...
            if sig["conviction"] < max(mp["conviction_threshold"], config.MIN_CONVICTION) + conv_mod: continue
            if not self._is_rational(sig, mp): continue
            if self._symbol_banned(coin): continue

            # Kelly sizing
            kelly = self._kelly_size(sig["conviction"], sig["rr"])
            if kelly < 0.05: continue  # سایز خیلی کوچک

            if self.engine.slot_of(coin) is None: self.engine.assign_coin(coin)
            avail = self.engine.available_margin(coin)
            if avail < 5: continue

            margin = min(avail * mp["margin_fraction"] * size_mult * kelly * 2, avail)
            ...
            if pos:
                ...
                self._trade_thought_open(pos, sig)
                self._add_milestone(f"🏆 [{self.mode}] شکار Elite {coin} "
                                    f"{'🟢' if pos['side'] > 0 else '🔴'} | R:R={sig['rr']:.1f} | "
                                    f"MTF={sig['mtf_score']*100:.0f}٪ | لوریج{mp['leverage']}")
                break
    ...
    def _on_closed(self, pos, net, reason, exit_price):
        ...
        mem = self.symbol_memory.setdefault(pos["coin"], {"pnl": 0.0, "n": 0})
        mem["pnl"] += net; mem["n"] += 1
        ...
        if net > 0:
            ...
            self.genome.epigenome.adapt("observer_eye", -0.0004)
            self.genome.epigenome.adapt("precision", -0.0003)
    ...
    def _trade_thought_open(self, pos, sig):
        side_fa = "خرید 🟢" if pos["side"] > 0 else "فروش 🔴"
        mode_fa = "Elite" if pos["mode"] == "elite" else "اسکالپ"
        text = (f"🏆 {mode_fa} {side_fa} {pos['coin']} در {pos['entry']:.4f} | "
                f"TP: {pos['tp']:.4f} | SL: {pos['sl']:.4f} | R:R={sig.get('rr', 0):.1f} | "
                f"مارجین {pos['margin']:.0f}$ | اطمینان {int(pos['conviction']*100)}٪")
        t = Thought(self.mind.next_id, self.tick, "trade", text, pos["conviction"],
                    ["OBSERVATORY_CORTEX", "INTUITION_CORTEX", "BASAL_GANGLIA"])
    ...
    def step(self):
        ...
        neuro = self.neurochem.step(rates, self.body.hormones, pain=self.body.pain,
                                    ...
                                    info_feed=info_feed)
        self.math_prob = 0.5 + 0.3 * clamp(self.last_evidence, -1, 1)
        ...
        out = self.brain.step(sensory, neuro, q_events, self.body.fatigue,
                              ...
                              intuition_level=self.phenotype.intuition_core,
                              evidence_abs=abs(self.last_evidence))
        ...
        thought = self.mind.step(self.tick, self.cons, out["activities"], self.neurochem,
                                 ...
                                 evidence=self.last_evidence, agreement=self.last_agreement)
        self.hist_equity.push(eq); self.hist_cons.push(level); self.hist_eeg.push(out["eeg"])
    ...
    def _check_milestones(self, q_events, thought):
        ...
        if self.cons.level > 90: once("c90", "🏆 فراآگاهی Elite فعال شد")
        ...
        if st["wins"] == 1: once("w1", "🎉 نخستین شکار Elite موفق!")
...
def build_focus_chart(s, scanner_snap):
    ...
    if not focus or focus not in deep:
        ...
        fig.update_layout(template="plotly_dark", paper_bgcolor="rgba(0,0,0,0)",
                          ...
                          title={"text": "🏆 در حال جست‌وجوی ستاپ A+…", "x": 0.5})
        return fig
    kl = deep[focus]["klines"].get("15")
    if not kl:
        fig = go.Figure()
        fig.update_layout(template="plotly_dark", title={"text": "🏆 در انتظار داده…", "x": 0.5})
        return fig
    ...
    mode_fa = "Elite" if s.get("mode") == "elite" else "اسکالپ"
    en = s.get("eternal_now", 0); attn = s.get("attention_level", 0)
    ...
    fig.update_layout(template="plotly_dark", paper_bgcolor="rgba(0,0,0,0)",
                      ...
                      title={"text": f"🏆 {focus} | اکنون ابدی {en*100:.0f}٪ | MTF {evidence*100:.0f}٪ | {mode_fa}",
                             "x": 0.5, "font": {"color": "#a78bfa", "size": 13}})
    return fig
...
.title { font-size: 26px; font-weight: 800; background: linear-gradient(90deg, #fbbf24, #a78bfa, #38bdf8);
...
@keyframes glow { from{box-shadow:0 0 12px rgba(167, 139, 250, .12);} to{box-shadow:0 0 26px rgba(251, 191, 36, .30);} }
    ...
    return html.Div(className="header", children=[
        ...
        html.Div(className="controls", children=[
            dcc.Dropdown(id="mode-selector", options=[
                {"label": "🏆 Elite Trader (لوریج ۱۰)", "value": "elite"},
                {"label": "⚡ اسکلپ دقیق (لوریج ۲۰)", "value": "scalp"},
            ], value="elite", clearable=False, style={"width": 180, "fontSize": 12}),
...
main_tab = dcc.Tab(label="🧠 داشبورد زنده", value="tab-main", className="tab",
    ...
    html.Div(className="grid", children=[
        ...
        html.Div(className="panel s5", children=[
            html.H3([html.Span(className="dot-purple"), "🌌 حسگرهای فرازمینی + حس ششم"]),
    ...
    html.Div(className="grid", children=[
        html.Div(className="panel s5", children=[
            html.H3([html.Span(className="dot"), "جریان تفکرات Elite"]),
            html.Div(id="thoughts", className="thoughts"),
        ]),
        html.Div(className="panel s4", children=[
            html.H3([html.Span(className="dot"), "روند آگاهی"]),
    ...
    html.Div(className="grid", children=[
        html.Div(className="panel s4", children=[
            html.H3([html.Span(className="dot"), "بدن — علائم حیاتی"]),
        ...
        html.Div(className="panel s4", children=[
            ...
            dcc.Graph(id="ecg", config={"displayModeBar": False}, style={"height": 150}),
            html.Div(id="fib-heart"),
            dcc.Graph(id="eeg", config={"displayModeBar": False}, style={"height": 90}),
        ]),
        html.Div(className="panel s4", children=[
...
signals_tab = dcc.Tab(label="🏆 سیگنال‌ها و معاملات Elite", value="tab-signals", className="tab",
                      selected_className="tab--selected", children=[
    html.Div(className="grid", children=[
        html.Div(className="panel glow s8", children=[
            html.H3([html.Span(className="dot-green"), "🏆 چارت زنده + SMC + تریل‌استاپ"]),
            dcc.Graph(id="focus-chart", config={"displayModeBar": False}, style={"height": 380}),
        ]),
        html.Div(className="panel s4", children=[
            html.H3([html.Span(className="dot-green"), "🎯 حالت و حساب Elite"]),
    ...
    html.Div(className="grid", children=[
        html.Div(className="panel s7", children=[
            html.H3([html.Span(className="dot-green"), "💼 پوزیشن‌های باز + Trailing Stop + Partials"]),
            html.Div(id="positions-table", style={"maxHeight": 320, "overflowY": "auto"}),
        ]),
        html.Div(className="panel s5", children=[
            html.H3([html.Span(className="dot-gold"), "🏆 ستاپ‌های A+ فعال"]),
            html.Div(id="scanner-status", style={"fontSize": 12, "color": "#94a3b8", "marginBottom": 8}),
            html.Div(id="signals-list", style={"maxHeight": 320, "overflowY": "auto"}),
        ]),
    ], style={"marginBottom": 14}),
    html.Div(className="grid", children=[
        html.Div(className="panel s7", children=[
            html.H3([html.Span(className="dot-gold"), "📜 معاملات بسته‌شده"]),
            html.Div(id="closed-table", style={"maxHeight": 260, "overflowY": "auto"}),
        ]),
        html.Div(className="panel s5", children=[
            html.H3([html.Span(className="dot-gold"), "📚 آموخته‌های Elite"]),
            html.Div(id="lessons-panel", style={"maxHeight": 260, "overflowY": "auto"}),
        ]),
    ]),
])
...
def render_sensory_panel(s):
    ...
    xeno_total = s.get("xeno_total", 0); xeno_act = s.get("xeno_activation", 0)
    st = s.get("spacetime", {}); st_rich = s.get("spacetime_richness", 0)
    body_aw = s.get("body_awareness", 0); intero = s.get("interoception_signals", {})
    attn = s.get("attention_level", 0); cardiac_neurons = s.get("cardiac_neurons", 0)
...
def render_mode_info(s):
    mp = s["mode_params"]; mode = s["mode"]; st = s["trade_stats"]
    mode_fa = "🏆 Elite" if mode == "elite" else "⚡ اسکلپ"
    children = [
        html.Div(f"حالت فعال: {mode_fa} (موتور «{s['engine_name']}」)",
                 style={"fontWeight": "bold", "color": "#a78bfa", "fontSize": 14}),
        html.Div(f"لوریج: {mp['leverage']} | TP×{mp['tp_mult']} | SL×{mp['sl_mult']} | Min R:R={config.MIN_RR}",
                 style={"marginTop": 4}),
        html.Div(f"آستانه: {int(max(mp['conviction_threshold'], config.MIN_CONVICTION)*100)}٪ | "
                 f"مارجین اسلات: {config.SLOT_MARGIN:.0f}$"),
        html.Div(f"معاملات امروز: {st['daily_trades']}/{mp['max_daily']} | "
                 f"مارجین درگیر: {fmt_num(st['margin_used'])}$"),
    ]
...
def render_signals(signals):
    if not signals:
        return html.Div("در حال اسکن… منتظر ستاپ A+ با R:R بالای ۱:۲.۵",
    ...
    for s in signals:
        ...
        cards.append(html.Div(className="sig-card",
            ...
            html.Div(className="sig-head", children=[
                ...
                html.Span(f"🏆 اطمینان {int(s['conviction']*100)}٪ | R:R={s.get('rr', 0):.1f}",
...
def render_positions(positions):
    if not positions:
        return html.Div("— پوزیشن بازی نیست —",
                        style={"fontSize": 12, "color": "#94a3b8", "textAlign": "center", "padding": 14})
    header_row = html.Tr([html.Th("نماد"), html.Th("جهت"), html.Th("لوریج"), html.Th("ورود"), html.Th("فعلی"),
                          html.Th("TP"), html.Th("SL تریل"), html.Th("قله"), html.Th("PnL زنده"), html.Th("ROI٪")])
    rows = []
    for p in positions:
        ...
        peak_txt = f"{fmt_num(p.get('peak', p['entry']), 4)}" if p.get("trailed") else "—"
        rows.append(html.Tr([
            ...
            html.Td(peak_txt, style={"color": "#a78bfa"}),
...
def render_closed(closed):
    if not closed:
        return html.Div("— هنوز معامله‌ای بسته نشده —",
                        style={"fontSize": 12, "color": "#94a3b8", "textAlign": "center", "padding": 14})
    ...
    for t in closed:
        ...
        mode_fa = "Elite" if t.get("mode") == "elite" else "اسکالپ"
...
def _update_impl(n, mode):
    ...
    gauge = go.Figure(go.Indicator(
        ...
        gauge={"axis": {"range": [0, 100], "tickcolor": "#94a3b8"}, "bar": {"color": "#fbbf24"},
    ...
    trend.add_scatter(y=s["hist_consciousness"], line=dict(color="#a78bfa", width=2),
                      fill="tozeroy", fillcolor="rgba(167,139,250,.15)")
    ...
    style_fig(regions)
    regions.update_layout(xaxis=dict(range=[0, 100]), margin=dict(l=110, r=34, t=20, b=28))
    ...
    vitals_children = [
        ...
        vital_card("گلوکز", v["گلوکز"], "mg/dL", v["گلوکز"] / 1.6, "#fbbf24"),
        vital_card("دما", v["دما"], "°C", (v["دما"] - 34) / 7 * 100, "#fb923c"),
        vital_card("انرژی", v["انرژی"], "٪", v["انرژی"], "#34d399"),
        vital_card("🍽️ سیری اطلاعاتی", f"{s['information_level']*100:.0f}", "٪",
                   s["information_level"] * 100, "#fbbf24"),
    ]
    st = s["trade_stats"]
    chips = [
        ...
        html.Span(f"💵 {fmt_num(st['equity'])}$/۱۰۰۰$", className="chip chip-gold"),
        html.Span(f"📊 وین‌ریت {st['win_rate']:.0f}٪", className="chip " + ("chip-ok" if st["win_rate"] >= 50 else "chip-bad")),
        html.Span(f"🫀 {s.get('cardiac_neurons', 0)} نورون قلب", className="chip chip-gold"),
    ...
    scanner_status = html.Div(
        f"🔍 اسکن Elite: {config.SCAN_SYMBOL_LIMIT} ارز | عمیق {config.SCAN_DEEP_LIMIT} | دور: {s['scan_cycles']} | "
        f"کارمزد: {config.TAKER_FEE*100:.3f}٪ | Min R:R={config.MIN_RR}",
        style={"color": "#94a3b8"})
    ...
    return [gauge, trend, eeg, regions, hormones, thoughts_children,
            ...
            ecg_fig, fib_heart, emotion_panel, sensory_panel,
...
def reset_memory(n, mode):
    if not n: return ""
    try:
        ORGANISM.set_mode(mode or "elite")
        ORGANISM.reset_all_trading_memory()
        return "🗑️ حافظه معاملات ریست شد"
    except Exception as e:
        import traceback; traceback.print_exc()
...
def interact(*args):
    ...
    if trig == "btn-rebirth":
        ORGANISM.rebirth()
        return html.Span("🏆 موجود Elite Trader جدید با ژنوم برتر متولد شد.", style={"color": "#a78bfa"})
...
if __name__ == "__main__":
    ...
    print(f"🧠 نورون‌ها با مقیاس ×{config.NEURON_SCALE:.2f} | موازی: {config.PARALLEL_BRAIN}")
    print(f"🌌 {config.SENSORS_PER_SENSE} حسگر/حس + {config.N_XENO_SENSES} حس فرازمینی")
    print(f"💵 هر موتور: {config.INITIAL_CAPITAL}$ بین {config.N_SLOTS} ارز | Min R:R={config.MIN_RR}")
    print(f"🏆 استراتژی: SMC + Order Flow + Multi-TF Confluence + Kelly Sizing")
