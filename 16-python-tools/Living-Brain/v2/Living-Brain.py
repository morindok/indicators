...
# -*- coding: utf-8 -*-
"""
🧠 مغز زنده — نسخه 0.17 (تک‌فایلی کامل)
قلب با سیستم عصبی مستقل | ذهن ریاضیدان IQ 200 | ادراک واقعی | رشد یادگیری
...
config.PROJECT_NAME="مغز زنده"; config.VERSION="0.17-alive"
config.SUBTITLE="قلب عصبی | ذهن ریاضیدان IQ 200 | ادراک واقعی"
...
config.SIM_RATE=6
config.ETERNAL_NOW_GATE=0.20
config.SCALP_CARDIAC_GATE=0.18
...
TRADING_MODES={
    "swing":{"label":"🎯 سویینگ (لوریج ۱۰)","leverage":10,"tp_mult":3.0,"sl_mult":1.4,
             "conviction_threshold":0.52,"decision_interval":5,"use_geometry":True,
             "margin_fraction":0.6,"max_daily":12,"cost_aware":True},
    "scalp":{"label":"⚡ اسکالپ (لوریج ۲۰)","leverage":20,"tp_mult":1.3,"sl_mult":0.9,
             ...
             "margin_fraction":0.4,"max_daily":80,"cost_aware":True},
}
...
class MathematicalMind:
    """🧠 ذهن ریاضیدان فرا بشری (IQ 200+) با رشد یادگیری."""
...
class CardiacNervousSystem:
    """🫀 سیستم عصبی ذاتی قلب (ICNS) — مغز کوچک قلب با نورون‌های مستقل."""
...
class CardiacSystem:
    """قلب زنده با سیستم عصبی مستقل به بزرگی مغز."""
    def __init__(self,rng,base_hr,neural_scale=1.0):
        ...
        # 🫀 سیستم عصبی ذاتی قلب با نورون‌های زیاد
    ...
    def step(self,dt,sympathetic,parasympathetic,stretch,o2,market_signal=0.0,market_vol=0.0):
        ...
        # 🫀 سیستم عصبی قلب مستقل پردازش می‌کند
...
class Body:
    def __init__(self,phenotype,rng):
        ...
        self.sensory={}
        # 🫀 قلب با سیستم عصبی مستقل به بزرگی مغز
...
class Organism:
    ...
    def _make_signal(self,coin,tk,an,geom,energy,mp,attention):
        ...
        elif (side==1 and math_prob<0.4) or (side==-1 and math_prob>0.6):
            conviction*=0.5
            reasons.append("⚠️ تضاد با تحلیل ریاضی")
    ...
    def _trading_step(self,scanner_snapshot):
        ...
        en_score=self.eternal_now.eternal_now_score
        if en_score<config.ETERNAL_NOW_GATE: return
