...
# -*- coding: utf-8 -*-
"""
🧠 مغز زنده — نسخه ۲.۰ «شکارچی کوانتومی» (تک‌فایلی کامل)
================================================================
🔮 شهود کوانتومی: هر کاوشگر=کیوبیت با احتمال واقعی | ۷ اندازه‌گیری | ۳ دور پایداری
⚡ ورود فقط با اجماع ≥۷۰٪ + پایداری ≥۸۰٪ + رژیم روندی — نه تصمیم سرسری
🛡️ تریل سودقطعی | توقف زمان | قطع‌کن ضرر روزانه | حافظه درد نماد
...
class GPUBackend:
    def __init__(self):
        ...
        try:
            ...
            if cp.cuda.runtime.getDeviceCount() > 0:
                ...
                nm = props.get("name", "GPU")
                self.name = nm.decode() if isinstance(nm, bytes) else str(nm)
                self.vram_gb = props.get("totalGlobalMem", 0)/(1024**3)
                a = cp.random.rand(512, 512).astype(cp.float32); _ = a @ a
        except Exception:
            self.gpu = False
    def asnumpy(self, a):
        if self.gpu:
    ...
    @property
    def label(self):
        return (f"🟢 GPU: {self.name} | VRAM {self.vram_gb:.0f}G" if self.gpu else "🔵 CPU: NumPy")
...
config.PROJECT_NAME = "مغز زنده"; config.VERSION = "2.0-quantum-hunter"
config.SUBTITLE = "شهود کوانتومی | آبشار باینری پایدار | شکار دقیق"
...
# 🔥 دروازه‌های دقیق نسخه کوانتومی
config.REGIME_GATE = 0.38            # حداقل کیفیت رژیم (هرگز پایین نمی‌آید)
config.INTUITION_GATE = 0.45
config.HUNGER_MAX = 0.10             # گرسنگی کوچک؛ فقط روی شهود، نه رژیم
config.HUNGER_RAMP_TICKS = 600
config.CASCADE_MIN_RATIO = 0.70      # اجماع باینری
config.CASCADE_MIN_STABILITY = 0.80  # پایداری بین دورهای اندازه‌گیری
config.CASCADE_MEASURES = 7          # اندازه‌گیری ضعیف هر کیوبیت
config.CASCADE_ROUNDS = 3            # دورهای کامل آبشار
config.TIME_STOP_MIN = 45            # توقف زمان (دقیقه)
config.TIME_STOP_R = 0.5
config.DAILY_LOSS_LIMIT = 0.03       # قطع‌کن ضرر روزانه ۳٪
config.SYMBOL_BAN_TRADES = 3         # حافظه درد نماد
TRADING_MODES = {
    ...
    "scalp": {"label": "⚡ اسکالپ (لوریج ۲۰)", "leverage": 20, "tp_mult": 1.3, "sl_mult": 0.9,
              "conviction_threshold": 0.34, "decision_interval": 2, "use_geometry": False,
...
# ═══════════════════════ ۷) تحلیلگر میدان غیرعلی + چندمقیاسی ═══════════════════════
class AcausalFieldAnalyzer:
    """تحلیل غیرسنتی + هم‌راستایی چندمقیاسی برای امتیاز جهت و کیفیت رژیم."""
    def __init__(self): self.prev_state = {}
    def _perm_entropy(self, x, order=4, delay=1):
        n = len(x); N = n-(order-1)*delay
        if N < 12: return 0.5
    ...
    def _spectral(self, x):
        ...
        dom_idx = int(np.argmax(P)); dom_cycle = float(1.0/f[dom_idx]) if f[dom_idx] > 1e-9 else 0.0
    ...
    def _multiscale(self, c):
        """هم‌راستایی جهت در مقیاس‌های ۱/۴/۱۶ → (align، coherence)."""
        dirs = []; moms = []
        for r in (1, 4, 16):
            if len(c) < r*20: continue
            agg = np.array([np.mean(c[j*r:(j+1)*r]) for j in range(len(c)//r)])
            if len(agg) < 20: continue
            s5 = np.mean(agg[-5:]); s20 = np.mean(agg[-20:])
            d = s5-s20; scale = max(abs(s20), 1e-9)
            dirs.append(np.sign(d)); moms.append(clamp(d/scale*20, -1, 1))
        if not dirs: return 0.0, 0.5
        align = float(np.mean(dirs))
        return align, abs(align)
    def analyze(self, o, h, l, c, key=""):
        ...
        ms_align, ms_coh = self._multiscale(c)
        recent = min(20, n)
        ...
        if abs(ms_align) > 0.6: reasons.append("🧭 هم‌راستایی چندمقیاسی")
        if abs(curv) > 0.3: reasons.append("🌀 خمیدگی میدان")
        prev = self.prev_state.get(key, {})
        if (prev.get("pe", pe)-pe) > 0.03: reasons.append("📉 کاهش آنتروپی (نظم‌یابی)")
        self.prev_state[key] = {"pe": pe, "centroid": spec["centroid"]}
        return {"ready": True, "price": price, "signal": float(field_dir), "conviction": float(conviction),
                ...
                "bull_rejection": False, "bear_rejection": False, "reasons": reasons,
                "features": {"perm_entropy": pe, "complexity": comp,
                             ...
                             "ms_align": ms_align, "ms_coherence": ms_coh}}

# ═══════════════════════ ۸) حس اقتصادی ═══════════════════════
class EconomicSense:
    def __init__(self):
        self.turnover_hist = {}; self.spread_hist = {}
    def analyze(self, tk, features=None):
        sym = tk["symbol"]
        to = tk.get("turnover", 0); sp = tk.get("spread_pct", 0)
        ...
        heat = clamp((t_arr[-1]-t_mu)/(t_sd*2)+0.5, 0, 1)
        s_mu = s_arr.mean()
        vacuum = clamp((s_mu-s_arr[-1])/(s_mu+1e-9), 0, 1)
...
class EternalNowPerception:
    ...
    def perceive(self, closes, ratios=(1, 4, 16, 32)):
        ...
        if len(closes) < 40:
            self.coherence = 0.5; self.eternal_now_score = 0.5; self.last_direction = 0; self.n_scales = 0
        ...
        for r in ratios:
            ...
            directions.append(np.sign(d)); momentums.append(clamp(d/scale*20, -1, 1))
        if not directions:
            self.coherence = 0.5; self.eternal_now_score = 0.5; self.last_direction = 0; self.n_scales = 0
            return {"coherence": 0.5, "direction": 0, "eternal_now": 0.5, "n_scales": 0}
        ...
        return {"coherence": coherence, "direction": unified_dir, "eternal_now": eternal_now, "n_scales": len(directions)}

# ═══════════════════════ ۱۱) درک عددی/بصری فوق‌بشری ═══════════════════════
...
class VisualPerception:
    ...
    def detect_patterns(self):
        ...
        if len(cs) >= 3:
            if all(cs[i][3] > cs[i][1] for i in range(-3, 0)): patterns.append("three_soldiers"); bias += 2
            if all(cs[i][3] < cs[i][1] for i in range(-3, 0)): patterns.append("three_crows"); bias -= 2
...
XENO_ALIEN_SENSE_NAMES = ["حس میدان", "حس خمیدگی", "حس آنتروپی", "حس رتبه", "حس شتاب",
                          ...
                          "حس شار", "حس گرداب", "حس انبساط", "حس انقباض", "حس فاز", "حس تداخل", "حس هارمونیک", "حس آشوش"]
...
class SpacetimePerception:
    def __init__(self, history_size=256):
        self.field = deque(maxlen=history_size); self.metric_history = deque(maxlen=128)
    ...
    def _fractal_dimension(self, p):
        try:
            ...
            return float(clamp(abs(float(np.polyfit(x, y, 1)[0])), 1.0, 2.0))
...
class AttentionSystem:
    def __init__(self): self.focus_level = 0.5; self.attention_target = None
    def focus(self, signals, consciousness):
        if not signals:
            self.focus_level = ema(self.focus_level, 0.35, 0.1); return self.focus_level
...
class HyperMathematicalMind:
    def __init__(self, iq_factor=2.5):
        ...
        self.accuracy = 0.5; self.iq_display = 200
        self.bayes_prob = 0.5; self.inference_depth = 0
    def bayes_update(self, evidence_dir, evidence_strength):
        prior = self.bayes_prob
        lr = math.exp(evidence_dir*evidence_strength*self.iq_factor)
        odds = (prior/(1-prior+1e-9))*lr
        self.bayes_prob = clamp(odds/(1+odds), 0.01, 0.99)
    ...
    def analyze_from_signals(self, signals, current_price):
        ...
        return {"probability": prob_up, "confidence": confidence, "iq_score": iq_score,
                "insight": insight, "weighted_dir": weighted_dir}
...
# ═══════════════════════ ۱۵) هسته شهود (امتیاز جهت + رژیم) ═══════════════════════
class IntuitionCore:
    """🔮 شهود = |امتیاز جهت هم‌وزن| × کیفیت رژیم + تعدیل‌گرهای حسی کوچک."""
    def __init__(self, phenotype):
        self.p = phenotype; self.last_intuition = 0.0; self.last_direction = 0
    def dir_score(self, flow, ms_align, curv, econ_pressure):
        return clamp(0.40*flow+0.35*ms_align+0.15*clamp(curv, -1, 1)
                     +0.10*clamp(econ_pressure, -1, 1), -1, 1)
    def regime(self, features):
        hurst = features.get("hurst", 0.5); pe = features.get("perm_entropy", 0.5)
        coh = features.get("ms_coherence", 0.5)
        return clamp(clamp((hurst-0.5)*2.0, 0, 1)*0.45+(1.0-pe)*0.30+coh*0.25, 0, 1)
    def fuse(self, dir_score, regime, features, econ, sixth, cardiac_coh, quantum_coh, math_prob, direction):
        base = abs(dir_score)*(0.35+0.65*regime)
        mods = {
            "order": (0.75-features.get("perm_entropy", 0.7)),
            "memory": (features.get("hurst", 0.5)-0.5)*1.5,
            "xeno": (sixth-0.25),
            "heart": (cardiac_coh-0.4)*0.5,
            "quantum": (quantum_coh-0.2)*0.5,
            "math": (math_prob-0.5)*direction,
            "econ": econ.get("pressure", 0)*direction*0.3,
        }
        bonus = 0.15*sum(clamp(v, -1, 1) for v in mods.values())/len(mods)
        scale = 0.85+0.30*self.p.intuition_core*(0.5+0.5*self.p.superhuman_factor*0.5)
        intuition = clamp(base*scale+max(0.0, bonus)*0.5, 0, 1)
        self.last_intuition = intuition; self.last_direction = direction
        return {"intuition": float(intuition),
                "components": {k: float(clamp(0.5+0.5*v, 0, 1)) for k, v in mods.items()},
                "active_senses": [k for k, v in mods.items() if v > 0.2]}

# ═══════════════════════ ۱۶) آبشار باینری کوانتومی (دقیق) ═══════════════════════
class QuantumIntuitionCascade:
    """⚡ هر کاوشگر یک کیوبیت است: |ψ⟩=√p|1⟩+√(1−p)|0⟩ با p واقعی از آمار بازار.
    هر کیوبیت M بار اندازه‌گیری ضعیف → رأی اکثریتی؛ کل آبشار R دور؛
    ورود فقط با اجماع ≥۷۰٪ و پایداری ≥۸۰٪ بین دورها = تصمیم دقیق، نه رندوم."""
    def __init__(self, rng, n_measure=None, n_rounds=None):
        self.rng = rng
        self.M = n_measure or config.CASCADE_MEASURES
        self.R = n_rounds or config.CASCADE_ROUNDS
        self.history = deque(maxlen=16)
        self.total_cascades = 0; self.total_triggers = 0
    @staticmethod
    def sig(x):
        try: return 1.0/(1.0+math.exp(-max(-20, min(20, x))))
        except Exception: return 0.5
    def probe_probs(self, direction, ctx):
        f = ctx.get("features", {}); econ = ctx.get("econ", {})
        s = self.sig
        return [
            ("شار اطلاعاتی هم‌سو", s(ctx.get("field_dir", 0)*direction*6)),
            ("نظم الگویی", s((0.75-f.get("perm_entropy", 0.7))*4)),
            ("حافظه هرست", s((f.get("hurst", 0.5)-0.5)*6)),
            ("چرخه طیفی", s((0.85-f.get("spectral_entropy", 0.7))*4)),
            ("ساختار فشرده", s((0.85-f.get("complexity", 0.7))*3)),
            ("خمیدگی هم‌جهت", s(f.get("curvature", 0)*direction*3)),
            ("فشار اقتصادی", s(econ.get("pressure", 0)*direction*3)),
            ("گرمای نقدینگی", s((econ.get("heat", 0)-0.4)*3)),
            ("حس ششم", s((ctx.get("sixth", 0)-0.2)*4)),
            ("فضازمان غنی", s((ctx.get("st_rich", 0)-0.25)*4)),
            ("هم‌راستایی چندمقیاسی", s(ctx.get("ms_align", 0)*direction*5)),
            ("همدلی قلب", s((ctx.get("cardiac_coh", 0)-0.3)*4)),
            ("انسجام کوانتومی", s((ctx.get("quantum_coh", 0)-0.15)*4)),
            ("احتمال ریاضی", s((ctx.get("math_prob", 0.5)-0.5)*direction*5)),
        ]
    def cascade(self, direction, ctx, min_ratio=None, min_stability=None):
        t0 = time.perf_counter()
        min_ratio = min_ratio or config.CASCADE_MIN_RATIO
        min_stability = min_stability or config.CASCADE_MIN_STABILITY
        probs = self.probe_probs(direction, ctx)
        rounds = []
        for _ in range(self.R):
            bits = ""
            for name, p in probs:
                votes = sum(1 for _ in range(self.M) if self.rng.random() < p)
                bits += "1" if votes*2 > self.M else "0"
            rounds.append(bits)
        agree = 0; tot = 0
        for i in range(self.R):
            for j in range(i+1, self.R):
                tot += len(rounds[i])
                agree += sum(1 for a, b in zip(rounds[i], rounds[j]) if a == b)
        stability = agree/max(1, tot)
        ratios = [b.count("1")/len(b) for b in rounds]
        ratio = sum(ratios)/len(ratios)
        bits = rounds[-1]
        passed = ratio >= min_ratio and stability >= min_stability
        ...
        rec = {"tick": ctx.get("tick", 0), "symbol": ctx.get("symbol", "?"), "bits": bits,
               "ratio": ratio, "stability": stability, "passed": passed, "decision": decision,
               "latency_us": latency_us, "rounds": rounds}
...
class ActivePositionManager:
    ...
    def manage(self, pos, tk, regulator, analysis=None, now_ms=0):
        ...
        if r_mult >= config.TRAIL_START_R:
            ...
            trail_r = max(trail_r, 0.25)  # حداقل سود قفل‌شده
            old_sl = pos["sl"]
            if side > 0:
                new_sl = entry+trail_r*initial_risk
                if new_sl > pos["sl"]: pos["sl"] = new_sl; pos["trailed"] = True; pos["trail_amount"] = new_sl-old_sl
            else:
                new_sl = entry-trail_r*initial_risk
                if new_sl < pos["sl"]: pos["sl"] = new_sl; pos["trailed"] = True; pos["trail_amount"] = old_sl-new_sl
        # ⏱️ توقف زمان: معامله‌ای که رشد نمی‌کند، سرمایه را آزاد کن
        if now_ms and pos.get("open_ts"):
            age_min = (now_ms-pos["open_ts"])/60000.0
            if age_min > config.TIME_STOP_MIN and r_mult < config.TIME_STOP_R:
                return ("close", f"توقف زمان ({age_min:.0f} دقیقه بی‌ثمر)")
        greed_threshold = 2.5-regulator.greed*0.5
        if r_mult >= greed_threshold and momentum_aligned < 0: return ("close", "قفل سود (ضد طمع)")
...
# ═══════════════════════ ۲۰) موتور معاملاتی + قطع‌کن روزانه ═══════════════════════
class TradingEngine:
    ...
    def settle_on_reconnect(self, http):
        ...
        for coin in list(self.open_positions.keys()):
            ...
            settlement = self._determine_settlement(pos, klines) if klines else None
...
class Region:
    ...
    def add_current(self, idx_or_all, amount):
        ...
        else: self.I[self.xpb.asarray(np.asarray(idx_or_all))] += amount
...
class Brain:
    ...
    def _build_connectome(self):
        for src, dst, p, strength, plast in PATHWAYS:
            ...
            for d in range(1, MAX_DELAY):
                ...
                self.syn[dst].append({"src": src, "pre": self.xpb.asarray(pre[m]),
                                      "post": self.xpb.asarray(post[m]),
                                      "w": self.xpb.asarray(w[m]),
                                      "w_sign": float(np.sign(w[m][0])) if len(w[m]) else 1.0,
    ...
    def _gather_target(self, dst):
        ...
        for b in self.syn[dst]:
            ...
            idx = b["post"][sp]
            gain = self._exc_gain if b["w_sign"] >= 0 else self._inh_gain
    ...
    def step(self, sensory, neurochem, quantum_events, fatigue, plasticity_factor=1.0,
        ...
        regs["BRAINSTEM"].add_current(None, 5.0+5.0*arousal+3.0*fatigue+8.0*sensory.get("pain", 0)+2.0*body_awareness)
        ...
        regs["SPACETIME_CORTEX"].add_current(None, 4.0*spacetime_richness+3.0*sixth_sense+2.0*abs(market_signal))
        ...
        if numeric_input: regs["NUMERICAL_CORTEX"].add_current(None, 2.2*numeric_input.get("conviction", 0))
        ...
        for dst in ("CORTEX", "HIPPOCAMPUS", "NUMERICAL_CORTEX", "VISUAL_CORTEX",
            ...
            for b in self.syn[dst]:
                ...
                pre_np = GPU.asnumpy(b["pre"][idx_np]); post_np = GPU.asnumpy(b["post"][idx_np])
...
class CardiacSystem:
    ...
    def step(self, dt, sympathetic, parasympathetic, stretch, o2, market_signal=0.0, market_vol=0.0):
        self.market_excitement = ema(self.market_excitement, clamp(abs(market_signal)+market_vol*2, 0, 1), 0.1)
        self.icns_activity = self.neural.step(sympathetic, parasympathetic, self.market_excitement, o2)
...
class Body:
    ...
    def regulate(self, autonomic, env_state, tick, market_signal=0.0, market_vol=0.0):
        ...
        self.energy = clamp(0.4*self.glycogen+0.3*(self.glucose/120)
                            +0.3*min(1.0, Hh["heart"]+Hh["liver"])/1.5-0.20*self.sickness+info_boost)
...
T = {"market": ["📊 میدان اطلاعاتی را اسکن می‌کنم؛ {obs}",
     ...
     "binary": ["⚡ آبشار کوانتومی: {bits} ⟶ {decision} | پایداری {stab}٪",
                "⚡ {rounds} دور اندازه‌گیری؛ اجماع {ratio}٪ — تصمیم دقیق."],
...
class StreamOfConsciousness:
    ...
    def step(self, tick, cons, activities, neurochem, body, sensory, quantum_events,
        ...
        weights = {"market": 2.5+abs(market_signal)*3.0,
    ...
    def _compose(self, cat, qevents, market_signal, n_scanned, top_lesson, sensory_array,
        ...
        if cat == "spacetime" and spacetime:
            ...
            return tmpl.format(curv=f"{g.get('curvature', 0):.3f}", arc=f"{g.get('arc_length', 0):.2f}",
        ...
        if cat == "binary" and last_binary:
            ...
            return tmpl.format(bits=last_binary.get("bits", ""), decision=last_binary.get("decision", "—"),
                               ...
                               stab=int(last_binary.get("stability", 0)*100),
                               rounds=config.CASCADE_ROUNDS)
        if cat == "intuition":
...
class Organism:
    ...
    def rebirth(self, seed=None):
        ...
        self.binary_stream = QuantumIntuitionCascade(self.rng)
        self.regulator = EmotionalRegulator(self.phenotype.discipline,
                                            self.phenotype.emotional_control, self.phenotype.calm)
        ...
        self.ticks_since_trade = 0; self._hunger = 0.0
        self.symbol_memory = {}
        self._last_regime = 0.0
        ...
        self.last_binary = None; self._binary_pulse = 0.0
    ...
    def _make_intuition_signal(self, coin, tk, an, geom, energy, mp, attention):
        ...
        flow = features.get("info_flow", 0.0)
        ms_align = features.get("ms_align", 0.0)
        curv = clamp(features.get("curvature", 0.0), -1, 1)
        dir_score = self.intuition_core.dir_score(flow, ms_align, curv, econ.get("pressure", 0))
        regime = self.intuition_core.regime(features)
        ...
        fusion = self.intuition_core.fuse(dir_score, regime, features, econ, sixth,
        ...
        if regime > 0.55: reasons.append("🌊 رژیم روندی فعال")
        ...
        return {"symbol": coin, "side": direction, "price": price, "spread": spread,
                ...
                "signal": dir_score, "regime": regime, "volatility": vol,
                "change24h": tk["change24h"], "turnover": tk["turnover"], "reasons": reasons,
                "sniper": sniper, "lesson_adj": lesson_adj, "features": features, "econ": econ,
    ...
    def _trading_step(self, scanner_snapshot):
        ...
        mp = self.mode_params
        if not scanner_snapshot.get("connected"):
            self._hunger = 0.0; return
        ...
        for coin in list(self.engine.open_positions.keys()):
            ...
            if reason:
                ...
                continue
            action = self.active_manager.manage(pos, tk, self.regulator, analyses.get(coin), now_ms=now_ms)
            if action:
        ...
        signals.sort(key=lambda s: s["conviction"]*(0.7+0.3*s["regime"]), reverse=True)
        self.current_signals = signals[:12]
        self._last_regime = signals[0]["regime"] if signals else 0.0
        if signals: self.focus_geometry = signals[0].get("geometry", {})
        if len(self.engine.open_positions) == 0:
            self.ticks_since_trade += 1
        else:
            self.ticks_since_trade = 0
        self._hunger = clamp(self.ticks_since_trade/config.HUNGER_RAMP_TICKS, 0, 1)*config.HUNGER_MAX
        ...
        eff_gate = max(mp["conviction_threshold"], config.INTUITION_GATE-self._hunger)
        sixth = self.xeno_array.activation_history[-1] if self.xeno_array.activation_history else 0.0
        ...
        for sig in signals:
            ...
            # 🌊 فیلتر رژیم: در بازار رنج هرگز وارد نمی‌شویم
            if sig["regime"] < config.REGIME_GATE: continue
            if sig["conviction"] < eff_gate+conv_mod: continue
            if not self._is_rational(sig, mp): continue
            if self._symbol_banned(coin): continue
            # ⚡🔮 آبشار باینری کوانتومی: اجماع + پایداری
            ctx = {"symbol": coin, "tick": self.tick, "features": sig["features"], "econ": sig["econ"],
                   "field_dir": sig["signal"], "sixth": sixth, "st_rich": st_rich,
                   "ms_align": sig["features"].get("ms_align", 0),
                   "cardiac_coh": cc, "quantum_coh": self.quantum.avg_coherence,
            ...
            if avail < 5: continue
            conf_scale = 0.5+0.5*sig["conviction"]**2
            margin = min(avail*mp["margin_fraction"]*size_mult*presence_scale*conf_scale
                         *(0.8+0.2*cascade["ratio"]), avail)
            ...
            if pos:
                ...
                self._add_milestone(f"🌌 [{self.mode}] شکار دقیق {coin} "
                                    f"{'🟢' if pos['side'] > 0 else '🔴'} لوریج{mp['leverage']} | "
                                    f"اجماع {int(cascade['ratio']*100)}٪ | پایداری {int(cascade['stability']*100)}٪")
                break

    def _binary_thought(self, cascade):
        text = (f"⚡ {cascade['bits']} ⟶ {cascade['decision']} | اجماع {int(cascade['ratio']*100)}٪ | "
                f"پایداری {int(cascade['stability']*100)}٪ | {cascade['latency_us']:.0f}μs")
    ...
    def _on_closed(self, pos, net, reason, exit_price):
        ...
        mem["pnl"] += net; mem["n"] += 1
        if self._symbol_banned(pos["coin"]):
            self._add_milestone(f"🚫 حافظه درد: {pos['coin']} موقتاً ممنوع شد ({mem['pnl']:+.2f}$)")
    ...
    def _check_milestones(self, q_events, thought):
        ...
        if self.binary_stream.total_triggers == 1: once("bin", "⚡ نخستین تصمیم باینری پایدار صادر شد")
    ...
    def snapshot(self):
        ...
        banned = [c for c, m in self.symbol_memory.items()
                  if m["n"] >= config.SYMBOL_BAN_TRADES and m["pnl"] < 0]
        return {
            ...
            "hunger": self._hunger,
            "regime_level": self._last_regime,
...
def build_focus_chart(s, scanner_snap):
    ...
    if not focus:
        focus = s["signals"][0]["symbol"] if s.get("signals") else None
    ...
    en = s.get("eternal_now", 0); intuition = s.get("intuition_level", 0)
    regime = s.get("regime_level", 0)
    fig.update_layout(template="plotly_dark", paper_bgcolor="rgba(0,0,0,0)",
                      plot_bgcolor="rgba(2,6,23,0.35)",
                      ...
                      title={"text": f"🌌 {focus} | شهود {intuition*100:.0f}٪ | رژیم {regime*100:.0f}٪ | {mode_fa}",
...
.vital { flex:1 1 16%; background:rgba(2,6,23,.55); border-radius:12px; padding:10px 12px;
         border:1px solid rgba(148,163,184,.15);}
.vital .num { font-size:20px; font-weight:700;}
...
main_tab = dcc.Tab(label="🧠 داشبورد زنده", value="tab-main", className="tab",
    ...
    html.Div(className="grid", children=[
        ...
        html.Div(className="panel s5", children=[
            html.H3([html.Span(className="dot-gold"), "🔮 هسته شهود + ⚡ آبشار کوانتومی"]),
    ...
    html.Div(className="grid", children=[
        html.Div(className="panel s12", children=[
            html.H3([html.Span(className="dot"), "🫀 علائم حیاتی + 🧪 نوروشیمی"]),
...
signals_tab = dcc.Tab(label="📊 سیگنال‌ها و معاملات", value="tab-signals", className="tab",
    ...
    html.Div(className="grid", children=[
        ...
        html.Div(className="panel s5", children=[
            html.H3([html.Span(className="dot-gold"), "⚡ آبشارهای کوانتومی اخیر"]),
...
def render_intuition_panel(s):
    ...
    box = html.Div(className="intuition-box", children=[
        ...
        html.Div(f"{intuition*100:.0f}٪ — آستانه {int(config.INTUITION_GATE*100)}٪ "
                 f"(با گرسنگی: {int(max(config.INTUITION_GATE-hunger, 0)*100)}٪)",
        ...
        html.Div(f"🌊 رژیم بازار: {regime*100:.0f}٪ (حداقل {int(config.REGIME_GATE*100)}٪) ",
                 style={"fontSize": 12, "color": reg_color, "marginTop": 6}),
        html.Div(f"🧠 بِیزی: {s.get('bayes_prob', 0.5)*100:.0f}٪ | IQ: {s.get('iq_display', 200)} | "
                 f"🍽️ گرسنگی: {hunger*100:.0f}٪ | 🚫 ممنوع: {len(banned)}",
    ...
    if last_b:
        ...
        b = html.Div([
            html.Div(f"⚡ آخرین استنتاج کوانتومی — {last_b['symbol']}",
                     style={"fontWeight": "bold", "color": "#fbbf24", "fontSize": 13}),
            bits_html,
            html.Div(f"تصمیم: {last_b['decision']} | اجماع {int(last_b['ratio']*100)}٪ | "
                     f"پایداری {int(last_b['stability']*100)}٪ | {last_b['latency_us']:.0f}μs",
                     style={"fontSize": 12, "color": dec_color, "textAlign": "center"}),
        ])
    else:
        b = html.Div("⚡ هنوز آبشار کوانتومی اجرا نشده…", style={"fontSize": 12, "color": "#94a3b8"})
...
def render_binary_panel(s):
    ...
    for rec in hist[:8]:
        ...
        cards.append(html.Div(className="binary-card", children=[
            ...
            bits,
            html.Div(f"اجماع {int(rec['ratio']*100)}٪ | پایداری {int(rec['stability']*100)}٪ | "
                     f"{'✅ عبور دقیق' if rec['passed'] else '⛔ رد'} | {rec['latency_us']:.0f}μs",
                     style={"color": "#94a3b8"}),
        ]))
    summary = html.Div(f"📊 کل آبشارها: {s.get('binary_cascades', 0)} | تصمیم‌های دقیق: {s.get('binary_triggers', 0)}",
...
def render_sensory_panel(s):
    ...
    for sense, val in xeno_summary.items():
        ...
        rows.append(html.Div(className="sense-row", children=[
            ...
            html.Div(className="sense-bar-bg", children=[
                html.Div(className="sense-bar-fg", style={"width": f"{val*100:.0f}%", "background": color})]),
    ...
    sixth_box = html.Div(className="sixth-box", children=[
        ...
        html.Div(f"فعال‌سازی کل {xeno_total} حسگر: {xeno_act*100:.0f}٪",
                 style={"fontSize": 11, "color": "#94a3b8", "marginTop": 4}),
        html.Div(className="bar-bg", children=[
            html.Div(className="sense-bar-fg", style={"width": f"{xeno_act*100:.0f}%", "background": sixth_color})],
                 style={"marginTop": 6}),
    ])
    st_box = html.Div(className="st-box", children=[
        ...
        html.Div(f"غنای هندسی: {st_rich*100:.0f}٪", style={"fontSize": 12, "color": "#f472b6", "marginTop": 4}),
    ])
    ...
    top_html = html.Div([html.Div(f"✨ {name}: {act:.2f}", style={"fontSize": 10, "color": "#7dd3fc"})
                         for name, act in top_active])
...
def render_mode_info(s):
    ...
    children = [
        ...
        html.Div(f"آستانه شهود: {int(config.INTUITION_GATE*100)}٪ | رژیم: {int(config.REGIME_GATE*100)}٪ | "
                 f"مارجین اسلات: {config.SLOT_MARGIN:.0f}$"),
        html.Div(f"معاملات امروز: {st['daily_trades']}/{mp['max_daily']} | PnL امروز: {st.get('daily_pnl', 0):+.2f}$ "
                 f"(قطع‌کن {int(config.DAILY_LOSS_LIMIT*100)}٪-)"),
    ]
    ...
    for s in signals:
        ...
        cards.append(html.Div(className="sig-card",
            ...
            html.Div(className="sig-head", children=[
                ...
                html.Span(f"شهود {int(s['conviction']*100)}٪ | رژیم {int(s.get('regime', 0)*100)}٪",
...
def _update_impl(n, mode):
    ...
    vitals_children = [
        ...
        vital_card("سیری اطلاعاتی", f"{s['information_level']*100:.0f}", "٪",
                   s["information_level"]*100, "#a78bfa"),
    ]
    st = s["trade_stats"]
    regime = s.get("regime_level", 0)
    chips = [
        html.Span(f"🕐 تیک {s['tick']}", className="chip"),
        html.Span(f"{'🟢 GPU' if s.get('gpu_active') else '🔵 CPU'}",
        ...
        html.Span(f"🌊 رژیم {regime*100:.0f}٪", className="chip "+("chip-ok" if regime > config.REGIME_GATE else "chip-bad")),
...
def reset_memory(n, mode):
    ...
    try:
        ...
        return "🗑️ حافظه معاملات هر دو موتور ریست شد"
    except Exception as e:
        import traceback; traceback.print_exc()
...
def interact(*args):
    ...
    if trig == "btn-rebirth":
        ORGANISM.rebirth()
        return html.Span("🌌 موجود جدید با ژنوم شکارچی کوانتومی متولد شد.", style={"color": "#a78bfa"})
...
if __name__ == "__main__":
    print(f"🌌 {config.PROJECT_NAME} v{config.VERSION} — شکارچی کوانتومی")
    print(f"🖥 {GPU.label}")
    print(f"🧠 نورون‌ها با مقیاس ×{config.NEURON_SCALE:.2f} | سقف {config.MAX_NEURONS:,} | موازی: {config.PARALLEL_BRAIN}")
    print(f"🔮 شهود ≥{int(config.INTUITION_GATE*100)}٪ | 🌊 رژیم ≥{int(config.REGIME_GATE*100)}٪ | "
          f"⚡ اجماع ≥{int(config.CASCADE_MIN_RATIO*100)}٪ + پایداری ≥{int(config.CASCADE_MIN_STABILITY*100)}٪")
    print(f"🛡️ قطع‌کن روزانه {int(config.DAILY_LOSS_LIMIT*100)}٪ | توقف زمان {config.TIME_STOP_MIN} دقیقه | حافظه درد نماد")
