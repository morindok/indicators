# -*- coding: utf-8 -*-
"""
🌌 Golden Spiral Fractal LIVE Trader v5.2
✅ رفع گیر کردن PnL: قیمت لحظه‌ای برای همه معاملات باز (حتی خارج از لیست تایید)
✅ اسکنر همیشه نمادهای دارای معامله باز را ردیابی می‌کند
✅ [جدید] یکسان‌سازی کامل منطق بک‌تست و اسکنر زنده (یک تابع مشترک تشخیص سیگنال)
✅ [جدید] پنجره محاسبه سطوح در اسکنر زنده دقیقاً مثل بک‌تست (window غلتان، نه کل تاریخچه)
✅ [جدید] فیلترهای غیرسنتی افزایش وین‌ریت: confluence (تعداد برخورد)، تایید بسته‌شدن کندل، بایاس جهت اسپیرال
✅ [جدید] فعال‌سازی واقعی کنترل‌های «قدرت» و «لمس%» در پنل تنظیمات (قبلاً تعریف شده بودند ولی استفاده نمی‌شدند)
"""
import numpy as np
import pandas as pd
import requests
...
QUAL_RECALC = 1
QUAL_MIN_STR = 100
QUAL_MIN_RETURN = 2000
LIVE_MIN_STR = 100  # سقف پیش‌فرض (fallback) - مقدار واقعی مورد استفاده از CFG['min_strength'] می‌آید
...
# ⭐⭐⭐ پارامترهای جدید فیلتر غیرسنتی برای افزایش وین‌ریت ⭐⭐⭐
MIN_TOUCHES = 3            # حداقل تعداد برخورد قیمت با سطح (confluence) تا سیگنال معتبر شود
BIAS_LOOKBACK = 20         # تعداد کندل اخیر برای تشخیص جهت غالب اسپیرال (بایاس روند)
BIAS_RATIO = 1.3           # نسبت غلبه کندل صعودی/نزولی لازم برای فعال شدن فیلتر بایاس
CLOSE_CONFIRM_BUFFER = 0.0004  # بافر تایید بسته شدن کندل در سمت درست سطح (رد شدن قیمت از سطح)
        ...
        with self._lock, self._conn() as c:
            c.execute('''INSERT INTO signals (timestamp,symbol,signal_type,entry_price,tp_price,sl_price,
                ...
                (datetime.now().isoformat(), kw['symbol'], kw['signal_type'], kw['entry_price'], kw['tp_price'],
                 ...
                 kw['margin_used'], kw['position_size'], kw['pos_value'], kw['entry_comm'], kw['rr_ratio'], kw.get('notes', '')))
...
def sr_from_arrays(ho, hc, ref, lo2=None, lc2=None, bins=60, touch_pct=0.0025):
    """
    شناسایی سطوح حمایت/مقاومت با تراکم وزنی (histogram density) + محاسبه
    تعداد برخورد واقعی (confluence) هر سطح که برای فیلتر جدید استفاده می‌شود.
    """
    if len(ho) < 30: return []
    ...
    # ⭐ تعداد برخورد واقعی هر سطح: چند کندل بدنه‌اش به سطح نزدیک شده (confluence)
    body_lo = np.minimum(ho, hc); body_hi = np.maximum(ho, hc)
    out = []
    for p, v in peaks[:10]:
        tol = p * touch_pct
        touches = int(np.sum((body_lo - tol <= p) & (p <= body_hi + tol)))
        out.append({'price': float(p), 'strength': int(v / ms * 100),
                     'type': 'support' if p < ref else 'resistance', 'touches': touches})
    return out

def spiral_bias(ho, hc, lookback=BIAS_LOOKBACK, ratio=BIAS_RATIO):
    """
    ⭐ بایاس جهت غالب اسپیرال (بدون اندیکاتور سنتی مثل RSI/MACD):
    نسبت کندل‌های صعودی به نزولی در پنجره اخیر را می‌سنجد تا جهت غالب بازار
    مشخص شود؛ سیگنال مخالفِ بایاس غالب فیلتر می‌شود تا وین‌ریت بهبود یابد.
    خروجی: 1=بایاس صعودی، -1=بایاس نزولی، 0=خنثی (بدون فیلتر جهت)
    """
    n = len(ho)
    if n == 0: return 0
    lb = min(lookback, n)
    ro, rc = ho[-lb:], hc[-lb:]
    bulls = int(np.sum(rc >= ro))
    bears = lb - bulls
    if bears == 0 or bulls / max(bears, 1) >= ratio: return 1
    if bulls == 0 or bears / max(bulls, 1) >= ratio: return -1
    return 0

def find_entry_signal(sr_levels, cur_high, cur_low, cur_close, min_str, slp, bias=0, min_touches=MIN_TOUCHES):
    """
    ⭐⭐⭐ منطق واحد و مشترک تشخیص سیگنال ورود ⭐⭐⭐
    این تابع دقیقاً هم در بک‌تست (run_backtest) و هم در اسکنر زنده (_scan_symbol)
    فراخوانی می‌شود تا نتیجه بک‌تست و رفتار زنده ۱۰۰٪ مطابقت داشته باشند
    (قبلاً این دو بخش دو تکه کد مجزا و کمی متفاوت بودند).

    فیلترهای جدید نسبت به نسخه قبل (برای افزایش وین‌ریت، بدون استفاده از
    اندیکاتورهای سنتی مثل RSI/MACD/Bollinger):
      1) Confluence: سطح باید حداقل min_touches بار توسط بدنه کندل‌ها لمس شده باشد
      2) Rejection confirmation: کندل باید در سمت درستِ سطح بسته شده باشد
         (یعنی واقعاً از سطح رد شده، نه فقط لمس گذرا)
      3) هم‌راستایی با بایاس جهت غالب اسپیرال (اگر بایاس فعال باشد)
    """
    for lv in sr_levels:
        if lv['strength'] < min_str: continue
        if lv.get('touches', 0) < min_touches: continue
        lp = lv['price']
        if not (cur_low <= lp <= cur_high): continue
        if abs(cur_close - lp) / lp > slp * 2: continue
        direction = 'LONG' if lv['type'] == 'support' else 'SHORT'
        buf = lp * CLOSE_CONFIRM_BUFFER
        if direction == 'LONG' and cur_close < lp - buf: continue
        if direction == 'SHORT' and cur_close > lp + buf: continue
...
def run_backtest(df_htf, df_ltf=None, lookback=10000, window=150, recalc=1,
                 min_str=100, cap0=500, lev=10, risk=0.20, rr=2.0, slp=0.008, touch_pct=0.0025):
    ...
    for i in range(start_i, len(df_bt)):
        ...
        if open_t is not None:
            ...
            eq.append(capital)
            continue
        d, lv = find_entry_signal(sr, ch, cl2, cc, min_str, slp, bias=bias)
        if d:
            lp = lv["price"]
            if d == "LONG":
                entry = lp * (1 + SPREAD_PCT); sld = lp * slp; sl = entry - sld; tp = entry + sld * rr
            else:
                entry = lp * (1 - SPREAD_PCT); sld = lp * slp; sl = entry + sld; tp = entry - sld * rr
            margin = capital * risk
            if margin > 0:
...
class SpiralScanner:
    ...
    def _scan_all(self):
        # ⭐ همیشه نمادهای دارای معامله باز را هم ردیابی کن (حتی خارج از لیست تایید)
    ...
    def _scan_symbol(self, sym):
        try:
            ...
            # ⭐⭐⭐ رفع ناهماهنگی اصلی بک‌تست/زنده ⭐⭐⭐
            # قبلاً اینجا از کل «done» (تا 500 کندل) برای محاسبه سطوح استفاده می‌شد،
            # در حالی که بک‌تست فقط از یک پنجره غلتان به اندازه CFG['window'] (پیش‌فرض 150)
            # استفاده می‌کند. همین اختلاف باعث می‌شد سطوح شناسایی‌شده در حالت زنده با
            # سطوحی که در بک‌تست باعث ورود می‌شدند فرق کنند. اینجا دقیقاً همان پنجره را
            # می‌سازیم تا رفتار زنده = رفتار بک‌تست باشد.
            ws = max(0, len(done) - CFG['window'])
            win_df = done.iloc[ws:]

            ltf_tf = {'240': '60', '60': '15', '15': '5'}.get(CFG['interval'], '15')
            df_l = bybit.get_klines(sym, ltf_tf, limit=300)
            if len(df_l) > 0 and len(win_df) > 0:
                t0, t1 = win_df['ts_num'].iloc[0], win_df['ts_num'].iloc[-1]
                df_l = df_l[(df_l['ts_num'] >= t0) & (df_l['ts_num'] <= t1)]
            lo2 = df_l['open'].values if len(df_l) > 0 else None
            lc2 = df_l['close'].values if len(df_l) > 0 else None

            levels = sr_from_arrays(win_df['open'].values, win_df['close'].values, price, lo2, lc2,
                                    touch_pct=CFG['touch_pct'])
            ...
            last = done.iloc[-1]
            ch, cl2, cc = last['high'], last['low'], last['close']
            bias = spiral_bias(win_df['open'].values, win_df['close'].values)

            open_syms = set(db.get_open()['symbol'].tolist())
            if sym not in open_syms and len(open_syms) < CFG['max_open']:
                lt_ = db.last_signal_time(sym)
                cooldown_ok = not (lt_ and (datetime.now() - lt_).total_seconds() < COOLDOWN_SEC)
                fm, _ = db.free_margin()
                margin_ok = fm >= CFG['capital'] * CFG['risk_pct'] * 0.5
                if cooldown_ok and margin_ok:
                    # ⭐ همان تابع مشترکی که بک‌تست استفاده می‌کند - تضمین مطابقت کامل
                    direction, lv = find_entry_signal(levels, ch, cl2, cc, CFG['min_strength'], CFG['sl_pct'], bias=bias)
                    if direction:
                        p = compute_params(direction, price, lv['price'], lv['strength'])
                        db.insert(symbol=sym, signal_type=direction, entry_price=p['entry'], tp_price=p['tp'],
                                  ...
                                  level_price=lv['price'], risk_usd=p['risk'], margin_used=p['margin'],
                                  position_size=p['pos_size'], pos_value=p['pos_value'],
                                  entry_comm=p['entry_comm'], rr_ratio=CFG['rr'],
                                  notes=f"candle-cross L{lv['strength']} T{lv.get('touches',0)} B{bias}")
...
app.layout = html.Div([
    html.Div([
        ...
        html.P(f"100+ ارز بایبیت | آستانه: {QUAL_MIN_RETURN:.0f}% | ریسک 20% | لوریج 10x | PnL زنده | "
               f"فیلتر Confluence≥{MIN_TOUCHES} | بایاس اسپیرال | بک‌تست=زنده",
    ...
    html.Div(id='tab-scanner', style={'display': 'none'}, children=[
        ...
        html.Div([html.Div([html.H3("📡 ارزهای تایید شده (بازده > 500%)")], className='section-title'),
...
def save_cfg(n, cap, lev, risk, rr, sl, win, st, touch, tf):
    ...
    set_msg(f"تنظیمات ذخیره شد: L:{CFG['leverage']}x ریسک:{CFG['risk_pct']*100:.0f}% "
            f"قدرت≥{CFG['min_strength']} لمس%:{CFG['touch_pct']*100:.2f}")
    return f"✅ L:{CFG['leverage']}x | ریسک:{CFG['risk_pct']*100:.0f}% | قدرت≥{CFG['min_strength']}"
...
def update_qual(n, remove_clicks):
    ...
    status = html.Div([
        ...
        html.Span(f"✅ تایید (>500%): {len(qualified)}", style={'color': '#00ff88', 'fontWeight': '900', 'marginLeft': '14px'}),
...
def update_spiral(n, sym):
    ...
    levels = sr_from_arrays(done['open'].values, done['close'].values, price, touch_pct=CFG['touch_pct'])
    ...
    for lv in levels:
        ...
        rows.append(html.Tr([
            ...
            html.Td(f"{lv.get('touches', 0)}", style={'color': '#bb86fc', 'fontWeight': '700'}),
            html.Td(f"{(lv['price'] - price) / price * 100:+.2f}%"),
        ]))
    tbl = html.Table([html.Thead(html.Tr([html.Th('قیمت'), html.Th('نوع'), html.Th('قدرت'), html.Th('برخورد'), html.Th('فاصله')])),
...
def update_scanner(n):
    ...
    # ⭐ قیمت لحظه‌ای واقعی برای همه معاملات باز (حتی خارج از لیست تایید)
    ot_open = db.get_open()
    ...
    cards = []
    tracked = list(set(qualified) | set(ot['symbol'].tolist()) if not ot.empty else set(qualified))
    if not tracked:
...
def open_browser():
    try: webbrowser.open_new("http://127.0.0.1:8071")
    except: pass

if __name__ == '__main__':
    print("=" * 60)
    print("🚀 Golden Spiral LIVE Trader v5.2")
    print("✅ PnL زنده برای همه معاملات باز (حتی خارج از لیست تایید)")
    print("✅ اسکنر همیشه نمادهای دارای معامله باز را ردیابی می‌کند")
    print("✅ بک‌تست و اسکنر زنده اکنون از یک منطق مشترک استفاده می‌کنند")
    print(f"✅ فیلترهای جدید: Confluence≥{MIN_TOUCHES} | تایید بسته‌شدن کندل | بایاس اسپیرال")
    ok, p = bybit.test_connection()
    print(f"✅ اتصال: BTC ${p:,.2f}" if ok else "⚠️ خطا در اتصال")
    print("=" * 60)
    if not os.environ.get('WERKZEUG_RUN_MAIN'):
        Timer(1.5, open_browser).start()
    app.run(debug=True, host='127.0.0.1', port=8071, use_reloader=False)