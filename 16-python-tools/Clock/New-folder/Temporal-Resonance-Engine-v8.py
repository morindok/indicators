# -*- coding: utf-8 -*-
"""
🌌 Temporal Resonance Engine v17
=========================================================
{Morindok}

✓ تب جدید: بک‌تست تاریخی بدون دیتابیس
✓ دوره قابل انتخاب (۱ ماه تا ۱ سال - پیش‌فرض ۳ ماه)
✓ منحنی رشد سرمایه + آمار کامل
"""
...
def get_total_open_margin(db):
    ...
    return sum(sig.get("margin_used", 0) for sig in open_signals)
...
# ==============================================================================
# ★ موتور بک‌تست تاریخی (جدید)
# ==============================================================================
def run_historical_backtest(symbol, interval, period_days, balance, trading_mode, threshold):
    """اجرای بک‌تست تاریخی روی داده‌های گذشته بدون نیاز به دیتابیس."""
    interval_min = get_interval_minutes(interval)
    candles_per_day = (24 * 60) / interval_min
    required_candles = int(period_days * candles_per_day) + BACKTEST_MIN_HISTORY
    required_candles = min(required_candles, 1000)

    df = get_klines(symbol, interval, limit=required_candles)

    if df.empty or len(df) < BACKTEST_MIN_HISTORY + 10:
        return None, None, {"error": "داده کافی نیست. از تایم‌فریم کوچک‌تر استفاده کنید."}
    ...
    while i < n - 1:
        ...
        try:
            ...
            cycle_engine.build_phase_return_maps(df_slice, interval_min)

            current_time = pd.to_datetime(df_slice["ts"].iloc[-1]).to_pydatetime().replace(tzinfo=timezone.utc)
        ...
        position = risk_engine.calculate_position(
            current_balance, current_price, tp_sl["sl_distance"], sig_data["confidence"]
        )
        ...
        # کارمزد ۰.۱۲٪ (۰.۰۶٪ ورود + ۰.۰۶٪ خروج)
        ...
        trades.append({
            ...
            "direction": "Long" if sig_data["signal"] == 1 else "Short",
        ...
        equity_curve.append({
            "time": timestamps[exit_idx],
            "balance": current_balance,
        })

        i = exit_idx + 1

    stats = calculate_backtest_stats(trades, equity_curve, balance)
    return trades, equity_curve, stats


def calculate_backtest_stats(trades, equity_curve, initial_balance):
    if not trades:
        return {
            ...
            "avg_confidence": 0.0, "long_trades": 0, "short_trades": 0,
            "long_win_rate": 0.0, "short_win_rate": 0.0,
            "tp_count": 0, "sl_count": 0, "horizon_count": 0,
    ...
    if len(pnl_pcts) > 1:
        ...
        std_ret = (sum((r - avg_ret) ** 2 for r in pnl_pcts) / len(pnl_pcts)) ** 0.5
        sharpe = (avg_ret / std_ret * (len(pnl_pcts) ** 0.5)) if std_ret > 0 else 0
    else:
        sharpe = 0
    ...
    horizon_count = sum(1 for t in trades if t["exit_reason"] == "Horizon")
...
# ==============================================================================
# ★ توابع UI برای بک‌تست
# ==============================================================================
def build_backtest_kpis(stats, initial_balance):
    if stats.get("error"):
        return html.Div(stats["error"], style={"color": DN, "textAlign": "center"})

    if stats["total_trades"] == 0:
        return html.Div("📭 هیچ معامله‌ای در دوره انتخاب‌شده ثبت نشد.", 
                       style={"color": MUT, "textAlign": "center", "padding": "20px"})

    pnl_color = UP if stats["total_pnl"] >= 0 else DN
    win_color = UP if stats["win_rate"] >= 50 else (GOLD if stats["win_rate"] >= 35 else DN)
    sharpe_color = UP if stats["sharpe_ratio"] >= 1 else (GOLD if stats["sharpe_ratio"] >= 0.5 else MUT)
    pf_color = UP if stats["profit_factor"] >= 1.5 else (GOLD if stats["profit_factor"] >= 1 else DN)
    dd_color = UP if stats["max_drawdown_pct"] < 10 else (GOLD if stats["max_drawdown_pct"] < 20 else DN)

    return dbc.Row([
        dbc.Col(html.Div([
            html.Div("💰 سود/ضرر کل", className="stat-label"),
        ...
        dbc.Col(html.Div([
            html.Div("📈 بازده کل", className="stat-label"),
            html.Div(f"{stats['total_return_pct']:+.2f}%", className="stat-value", style={"color": pnl_color, "fontSize": "16px"}),
        ...
        dbc.Col(html.Div([
            ...
            html.Div(f"{stats['sharpe_ratio']:.2f}", className="stat-value", style={"color": sharpe_color, "fontSize": "16px"}),
        ], className="glass-card", style={"padding": "10px", "textAlign": "center"}), md=2),
        dbc.Col(html.Div([
            html.Div("⚡ Profit Factor", className="stat-label"),
            html.Div(f"{stats['profit_factor']:.2f}", className="stat-value", style={"color": pf_color, "fontSize": "16px"}),
        ], className="glass-card", style={"padding": "10px", "textAlign": "center"}), md=2),
        dbc.Col(html.Div([
            html.Div("📉 Max Drawdown", className="stat-label"),
            html.Div(f"{stats['max_drawdown_pct']:.2f}%", className="stat-value", style={"color": dd_color, "fontSize": "16px"}),
        ], className="glass-card", style={"padding": "10px", "textAlign": "center"}), md=2),
    ])


def build_backtest_detail_stats(stats):
    if stats.get("error") or stats["total_trades"] == 0:
        return html.Div()

    return dbc.Row([
        dbc.Col(html.Div([
            html.Div("تعداد معاملات", className="stat-label"),
            html.Div(f"{stats['total_trades']}", className="stat-value", style={"color": TXT, "fontSize": "14px"}),
        ], className="glass-card", style={"padding": "8px", "textAlign": "center"}), md=2),
        dbc.Col(html.Div([
            html.Div("🟢 Long / 🔴 Short", className="stat-label"),
            html.Div(f"{stats['long_trades']} / {stats['short_trades']}", className="stat-value", style={"color": TXT, "fontSize": "14px"}),
        ], className="glass-card", style={"padding": "8px", "textAlign": "center"}), md=2),
        dbc.Col(html.Div([
        ...
        dbc.Col(html.Div([
            html.Div("🎯 TP / 🛑 SL / ⏰ Horizon", className="stat-label"),
            html.Div(f"{stats['tp_count']} / {stats['sl_count']} / {stats['horizon_count']}", 
                    className="stat-value", style={"color": TXT, "fontSize": "14px"}),
        ], className="glass-card", style={"padding": "8px", "textAlign": "center"}), md=2),
        dbc.Col(html.Div([
...
def build_equity_curve_chart(equity_curve, initial_balance, symbol):
    ...
    line_color = UP if final_balance >= initial_balance else DN

    fig = go.Figure()

    fig.add_trace(go.Scatter(
        x=times, y=balances, mode="lines",
        name="سرمایه",
        line=dict(color=line_color, width=3),
        fill="tozeroy",
        fillcolor="rgba(0,255,204,0.1)" if final_balance >= initial_balance else "rgba(255,34,102,0.1)",
    ))

    fig.add_hline(
        y=initial_balance,
        line=dict(color=MUT, dash="dot", width=1),
        annotation_text=f"سرمایه اولیه: ${initial_balance:,.0f}",
        annotation_font=dict(color=MUT, size=10),
    )

    fig.add_trace(go.Scatter(
        x=[times[0], times[-1]],
        y=[balances[0], balances[-1]],
        mode="markers",
        marker=dict(size=10, color=[GOLD, line_color], symbol="circle"),
        name="نقاط عطف",
    ))
    ...
    fig.update_layout(
        ...
        title=dict(
            text=f"📈 منحنی رشد سرمایه — {symbol}",
            x=0.5,
...
def build_backtest_trades_table(trades):
    if not trades:
        return html.Div("معامله‌ای در بک‌تست ثبت نشد.", style={"color": MUT, "textAlign": "center", "padding": "20px"})

    header = html.Tr([
        html.Th("#", style={"width": "3%"}),
        html.Th("زمان ورود", style={"width": "10%"}),
        ...
        html.Th("خروج", style={"width": "8%"}),
        html.Th("دلیل", style={"width": "6%"}),
    ...
    for idx, t in enumerate(trades, 1):
        ...
        pnl_color = UP if t["pnl"] > 0 else DN
        ...
        rows.append(html.Tr([
            html.Td(f"{idx}", style={"color": MUT, "fontSize": "9px"}),
            ...
            html.Td(reason_text, style={"color": reason_color, "fontSize": "10px"}),
            html.Td(f"${t['pnl']:+,.2f}", style={"color": pnl_color, "fontWeight": "bold", "fontSize": "10px"}),
            html.Td(f"{t['pnl_pct']:+.2f}%", style={"color": pnl_color, "fontSize": "10px"}),
...
app.layout = html.Div([
    html.Div([
        ...
        html.Div("موتور معاملاتی رزونانس چرخه‌های زمانی — بک‌تست تاریخی — Morindok", style={
    ...
    dbc.Tabs([
        ...
        # ★ تب جدید: بک‌تست تاریخی
        dbc.Tab(label="📊 بک‌تست تاریخی", tab_id="tab-backtest", children=[
            dbc.Card(dbc.CardBody([
                html.Div([
                    html.Div("📌 بک‌تست روی داده‌های تاریخی بدون نیاز به دیتابیس | نماد و تایم‌فریم از تنظیمات بالا",
                             style={"fontSize": 12, "color": MUT, "marginBottom": "10px"}),
                    dbc.Row([
                        dbc.Col([
                            html.Label("📅 دوره بک‌تست", style={"fontSize": 11, "color": MUT}),
                            dcc.Dropdown(id="backtest-period", value="3m", clearable=False, 
                                        options=BACKTEST_PERIOD_OPTIONS),
                        ], md=3),
                        dbc.Col([
                            dbc.Button("🚀 اجرای بک‌تست", id="run-backtest-btn", color="success",
            ...
            html.Div(id="backtest-kpis", style={"margin": "15px 0"}),
            html.Div(id="backtest-detail-stats", style={"margin": "10px 0"}),

            dbc.Card(dbc.CardBody([
                dcc.Graph(id="backtest-equity-chart", config={"displaylogo": False}),
            ]), className="glass-card", style={"margin": "15px 0"}),

            dbc.Card(dbc.CardBody([
                html.H5("📋 معاملات بک‌تست", style={
                    "color": GOLD, "fontSize": "14px", "marginBottom": "10px", "textAlign": "center",
                }),
    ...
    html.Div(
        ...
        "بک‌تست تاریخی برای بررسی عملکرد استراتژی در گذشته.",
...
# ★ Callback جدید: بک‌تست تاریخی
@app.callback(
    Output("backtest-kpis", "children"),
    Output("backtest-detail-stats", "children"),
...
def run_backtest(_click, symbol, interval, period, balance, trading_mode, threshold):
    if _click is None:
    ...
    period_days = BACKTEST_PERIODS.get(period, BACKTEST_PERIODS["3m"])["days"]

    trades, equity_curve, stats = run_historical_backtest(
        symbol, interval, period_days, balance, trading_mode, threshold
    )

    if stats.get("error"):
        return html.Div(stats["error"], style={"color": DN, "textAlign": "center"}), \
               html.Div(), empty_fig("داده کافی نیست."), html.Div()

    if stats["total_trades"] == 0:
        return build_backtest_kpis(stats, balance), html.Div(), \
               empty_fig("معامله‌ای ثبت نشد."), html.Div()

    kpis = build_backtest_kpis(stats, balance)
    detail = build_backtest_detail_stats(stats)
    equity_chart = build_equity_curve_chart(equity_curve, balance, symbol)
    trades_table = build_backtest_trades_table(trades)

    return kpis, detail, equity_chart, trades_table
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("🌌 Temporal Resonance Engine v17")
    print("   ✅ تب بک‌تست تاریخی")
    print("   📈 منحنی رشد سرمایه")
    print("   🥇 طلا (XAUUSDT)")
