...
def get_prev_daily_hlc(symbol, category, before_ts=None):
    """
    H/L/C آخرین کندل روزانه‌ی کاملاً بسته‌شده.
    اگر before_ts داده شود (حالت بک‌تست)، فقط از کندل‌های روزانه‌ای که قبل
    از آن لحظه بسته شده‌اند استفاده می‌شود تا نگاه‌به‌جلو رخ ندهد.
    """
    d = get_klines(symbol, "D", category, limit=200)
    if d.empty or len(d) < 2:
        return None

    if before_ts is not None:
        cutoff = pd.Timestamp(before_ts)
        ...
        row = d.iloc[-2]  # حالت زنده: آخرین روز کاملاً بسته‌شده (نه کندل در حال شکل‌گیری)
...
# ==============================================================================
# 8) موتور تحلیل بازار (مستقل از رسم) — قابل استفاده برای لحظه‌ی زنده یا بک‌تست
# ==============================================================================
def analyze_market(df, pivot_period, turns, now_utc, interval_min, prev_hlc,
                    forecast_hours, active_planets, active_methods):
    """
    تمام محاسبات (پیووت، اسپیرال، نجوم، فیبوناچی، پیوت، پروفایل حجم، Confluence)
    را روی دیتافریم داده‌شده انجام می‌دهد. `now_utc` می‌تواند لحظه‌ی واقعی حال
    باشد یا زمان یک کندل تاریخی (برای بک‌تست) — در حالت دوم فقط داده‌های تا
    همان لحظه باید در df باشند تا نگاه‌به‌جلو (look-ahead) رخ ندهد.
    """
    ...
    return {
        "df_closed": df_closed, "seg": seg, "prices": prices, "times": times, "npts": npts,
        "th_max": th_max, "th": th, "r": r, "R": R, "th_step": th_step, "th_end": th_end,
        "live_price": live_price, "pivot_idx": pivot_idx, "pivot_type": pivot_type,
        "pivot_price": pivot_price, "th_clock": th_clock, "astro_now": astro_now,
        "astro_hist": astro_hist, "astro_fut": astro_fut, "levels": levels, "atr_val": atr_val,
        "forecast_minutes": forecast_minutes, "last_closed_time": last_closed_time,
        "now_utc": now_utc, "forecast_hours": forecast_hours, "active_methods": active_methods,
        "active_planets": active_planets,
    }


# ==============================================================================
# 9) ساخت نمودار اسپیرال (رسم، با استفاده از نتیجه‌ی analyze_market)
# ==============================================================================
def build_figure(df, analysis):
    if analysis is None:
        return empty_fig("دادهٔ پس از پیووت کافی نیست.")

    prices = analysis["prices"]; times = analysis["times"]
    th_max = analysis["th_max"]; th = analysis["th"]; r = analysis["r"]; R = analysis["R"]
    live_price = analysis["live_price"]; pivot_type = analysis["pivot_type"]; pivot_price = analysis["pivot_price"]
    th_clock = analysis["th_clock"]; astro_now = analysis["astro_now"]
    astro_hist = analysis["astro_hist"]; astro_fut = analysis["astro_fut"]
    levels = analysis["levels"]; now_utc = analysis["now_utc"]
    forecast_hours = analysis["forecast_hours"]; active_methods = analysis["active_methods"]
    active_planets = analysis["active_planets"]
...
def build_backtest_chart(df_full, analysis, selected_idx):
    """
    نمودار کندلی بزرگ برای بک‌تست.
    خطوط Confluence از نقطه‌ی انتخاب‌شده به بعد رسم می‌شوند (چون فقط با
    داده‌ی تا همان لحظه محاسبه شده‌اند) تا واکنش واقعیِ *بعدیِ* قیمت به
    آن‌ها با چشم قابل ارزیابی باشد.
    """
    fig = go.Figure()

    fig.add_trace(go.Candlestick(
        x=df_full["ts"], open=df_full["open"], high=df_full["high"],
        low=df_full["low"], close=df_full["close"],
        increasing_line_color=UP, decreasing_line_color=DN,
        name="قیمت",
    ))

    sel_ts = df_full.iloc[selected_idx]["ts"]
    x_end = df_full["ts"].iloc[-1]

    if analysis is not None:
        for lvl in analysis["levels"]:
            color = UP if lvl["side"] == "support" else DN
            tag = "S" if lvl["side"] == "support" else "R"
            width = 1.2 + min(lvl["confluence"], 5) * 0.45
            fig.add_shape(
                type="line", x0=sel_ts, x1=x_end, y0=lvl["price"], y1=lvl["price"],
                line=dict(color=color, width=width, dash="dash"), opacity=0.85,
            )
            fig.add_annotation(
                x=x_end, y=lvl["price"], text=f"{tag} {lvl['price']:.2f} (×{lvl['confluence']})",
                showarrow=False, xanchor="left", font=dict(color=color, size=10),
                bgcolor="rgba(11,18,32,0.85)", bordercolor=color, borderwidth=1, borderpad=2,
            )

    fig.add_vline(x=sel_ts, line=dict(color=GOLD, width=2, dash="dot"))
    fig.add_annotation(
        x=sel_ts, y=1.0, yref="paper", text="⏱ نقطه بک‌تست", yanchor="bottom",
    ...
    fig.update_layout(
        ...
        dragmode="pan",
        title=dict(text="📈 چارت بک‌تست — روی هر کندل کلیک کنید تا آن لحظه «زنده» فرض شود",
                   x=0.5, font=dict(color=GOLD, size=13)),
        legend=dict(bgcolor="rgba(11,18,32,0.8)", font=dict(size=9)),
    )
    fig.update_xaxes(gridcolor=LINE, tickfont=dict(size=9, color=MUT))
    fig.update_yaxes(gridcolor=LINE, tickfont=dict(size=9, color=MUT), title_text="قیمت")
    return fig
...
app.layout = html.Div([
    ...
    dbc.Card(dbc.CardBody([
        dbc.Row([
            dbc.Col(html.Div(id="backtest-mode-label", children="🟢 حالت زنده (لحظه‌ی فعلی)",
                             style={"color": UP, "fontWeight": "bold", "fontSize": 13}), md=9),
            dbc.Col(dbc.Button("↩️ بازگشت به لحظه زنده", id="backtest-reset-btn",
                               color="secondary", size="sm", style={"width": "100%"}), md=3),
        ], align="center", className="mb-2"),
        dcc.Graph(id="backtest-chart", style={"height": "55vh"}, config={"displaylogo": False}),
        html.Div(
            "💡 روی هر کندل کلیک کنید تا زمان «لحظه‌ی زنده» همان کندل در نظر گرفته شود: "
            "اسپیرال، موقعیت نجومی و سطوح Confluence دقیقاً با داده‌ی موجود تا همان لحظه "
            "(بدون نگاه‌به‌جلو) بازسازی می‌شوند — سپس می‌توانید واکنش واقعی قیمت را بعد از آن نقطه ببینید.",
...
# ==============================================================================
# 10) کال‌بک انتخاب نقطه‌ی بک‌تست (کلیک روی کندل / دکمه‌ی بازگشت)
# ==============================================================================
@app.callback(
    Output("backtest-store", "data"),
    Input("backtest-chart", "clickData"),
    Input("backtest-reset-btn", "n_clicks"),
    prevent_initial_call=True,
)
def set_backtest_point(click_data, _reset_clicks):
    ctx = callback_context
    if not ctx.triggered:
        return dash.no_update
    trig_id = ctx.triggered[0]["prop_id"].split(".")[0]

    if trig_id == "backtest-reset-btn":
        return None  # بازگشت به حالت زنده

    if trig_id == "backtest-chart" and click_data:
        try:
...
# ==============================================================================
# 11) کال‌بک اصلی (پشتیبانی از حالت زنده + بک‌تست بدون نگاه‌به‌جلو)
# ==============================================================================
@app.callback(
    Output("spiral-plot", "figure"),
    Output("backtest-chart", "figure"),
    Output("backtest-mode-label", "children"),
...
def update(_n, _click, backtest_ts, symbol, interval, category, pivot_period, turns,
           forecast_hours, active_planets, active_methods):
    try:
    ...
    now_actual = get_server_time()
    df_full = get_klines(symbol, interval, category, limit=500)

    if df_full.empty:
        status = "🔴 قطع از بایبیت"
        err = html.Div("خطا در دریافت کندل از بایبیت.", style={"color": DN, "fontSize": 12})
        empty_bt = empty_fig("دریافت دیتا از بایبیت ناموفق بود.")
        return (empty_fig("دریافت دیتا از بایبیت ناموفق بود."), empty_bt,
                "🔴 قطع از بایبیت", [], err, "", status, 10)

    # --- تعیین حالت: زنده یا بک‌تست (کلیک روی یک کندل تاریخی) ---
    selected_idx = len(df_full) - 1
    if backtest_ts:
        try:
            sel_ts_naive = pd.Timestamp(backtest_ts)
            if sel_ts_naive.tzinfo is not None:
                sel_ts_naive = sel_ts_naive.tz_localize(None)
            diffs = (df_full["ts"] - sel_ts_naive).abs()
            candidate_idx = int(diffs.values.argmin())
            # حداقل تعداد کندل لازم برای پیووت/تحلیل معنادار
            if candidate_idx >= 2 * pivot_period + 5:
                selected_idx = candidate_idx
        except Exception:
            pass

    is_backtest = selected_idx < len(df_full) - 1
    df_used = df_full.iloc[:selected_idx + 1].reset_index(drop=True)

    if is_backtest:
        now_utc_used = df_full.iloc[selected_idx]["ts"].to_pydatetime().replace(tzinfo=timezone.utc)
        mode_label = html.Span([
            "⏱ حالت بک‌تست — کندل انتخابی: ",
            html.B(now_utc_used.strftime("%Y-%m-%d %H:%M UTC")),
        ], style={"color": GOLD})
    else:
        now_utc_used = now_actual
        mode_label = html.Span("🟢 حالت زنده (لحظه‌ی فعلی)", style={"color": UP})

    prev_hlc = get_prev_daily_hlc(symbol, category, before_ts=now_utc_used)
    interval_min = get_interval_minutes(interval)

    analysis = analyze_market(
        df=df_used, pivot_period=pivot_period, turns=turns, now_utc=now_utc_used,
        interval_min=interval_min, prev_hlc=prev_hlc, forecast_hours=forecast_hours,
        active_planets=active_planets, active_methods=active_methods,
    )

    fig = build_figure(df_used, analysis)
    backtest_fig = build_backtest_chart(df_full, analysis, selected_idx)

    if analysis is None:
        status = f"🟡 داده‌ی کافی پس از پیووت نیست | {now_actual.strftime('%H:%M:%S')} UTC"
        return fig, backtest_fig, mode_label, [], "داده ناکافی.", "", status, 10

    levels = analysis["levels"]
    astro_now = analysis["astro_now"]
    pivot_type = analysis["pivot_type"]
    pivot_price = analysis["pivot_price"]
    pivot_idx = analysis["pivot_idx"]
    live_price = analysis["live_price"]

    pivot_time = df_used.iloc[pivot_idx]["ts"].strftime("%Y-%m-%d %H:%M")
    pivot_label = {"low": "کف", "high": "سقف", "none": "نامشخص"}.get(pivot_type, "—")

    # --- کارت بازار/پیووت ---
    market_info = html.Div([
        html.Div(f"⏰ {now_utc_used.strftime('%Y-%m-%d %H:%M:%S')} UTC"
                 f"{'  (بک‌تست)' if is_backtest else ''}", style={"marginBottom": 4}),
        html.Div([html.Span("نماد: ", style={"color": MUT}), html.Span(f"{symbol} | {category} | {interval}")],
                 style={"marginBottom": 4}),
        html.Div([html.Span("قیمت در آن لحظه: " if is_backtest else "قیمت لایو: ", style={"color": MUT}),
    ...
    status = (f"🟡 بک‌تست فعال | {now_utc_used.strftime('%H:%M:%S')} UTC" if is_backtest
              else f"🟢 متصل به سرور | {now_actual.strftime('%H:%M:%S')} UTC")

    return fig, backtest_fig, mode_label, levels_data, market_info, astro_info, status, max(int(max_conf), 1)
