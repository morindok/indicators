# -*- coding: utf-8 -*-
"""
🌌 Temporal Resonance Engine v12 — PnL زنده واقعی
===================================================
{Morindok}

✓ تب سیگنال‌ها: معاملات باز واقعی از دیتابیس با PnL زنده
✓ نتایج اسکن جدا از معاملات نمایش داده می‌شوند
✓ PnL هر ۱۰ ثانیه آپدیت می‌شود
"""
...
class SignalDatabase:
    ...
    def _init_db(self):
        ...
        conn.commit()
        try:
            cursor.execute("ALTER TABLE signals ADD COLUMN leverage REAL DEFAULT 1.0")
        except sqlite3.OperationalError:
            pass
        try:
            cursor.execute("ALTER TABLE signals ADD COLUMN margin_used REAL DEFAULT 0.0")
        except sqlite3.OperationalError:
            pass
        conn.commit()
        conn.close()
...
def get_klines(symbol, interval, limit=1000):
    """دریافت کندل از Bybit. برای طلا ابتدا linear و سپس spot."""
    if symbol == "XAUUSDT":
...
class RiskEngine:
    ...
    def calculate_tp_sl(self, entry_price, signal, atr_value, confidence):
        ...
        return {
            ...
            "rr_ratio": rr_ratio, "sl_distance": atr_value * sl_mult,
    ...
    def calculate_position(self, balance, entry_price, sl_distance, confidence):
        if sl_distance <= 0 or entry_price <= 0:
            return {"dollar_amount": 0.0, "leverage": MIN_LEVERAGE, "margin_used": 0.0}
...
# 6) اسکنر
# ==============================================================================
def scan_all_symbols(interval="15", balance=DEFAULT_BALANCE, risk_pct=0.01, threshold=0.35):
...
def update_open_signals_results(db):
    """بررسی سیگنال‌های باز و بستن خودکار TP/SL/منقضی."""
...
def calculate_open_pnls_live(db):
    """محاسبه PnL زنده تمام معاملات باز بر اساس قیمت فعلی."""
    open_signals = db.get_open_signals()
    live_pnls = {}
...
# ==============================================================================
# ★ جدول معاملات باز با PnL زنده (مشکل اصلی حل شد)
# ==============================================================================
def build_open_positions_table(open_signals, live_pnls):
    """جدول معاملات باز واقعی از دیتابیس با PnL زنده."""
    if not open_signals:
        return html.Div(
    ...
    header = html.Tr([
        ...
        html.Th("مارجین", style={"width": "8%"}),
        html.Th("قیمت فعلی", style={"width": "9%"}),
        html.Th("PnL ($)", style={"width": "8%"}),
        html.Th("PnL (٪)", style={"width": "7%"}),
        html.Th("عملیات", style={"width": "9%"}),
    ...
    for sig in open_signals:
        ...
        rows.append(html.Tr([
            html.Td(f"#{sig_id}", style={"color": MUT, "fontSize": "11px"}),
            html.Td([
                html.Span(symbol_info["icon"], style={"marginRight": "3px"}),
                html.Span(symbol.replace("USDT", ""), style={"fontWeight": "bold", "fontSize": "11px"}),
            ], style={"color": TXT}),
            ...
            html.Td(f"${sl_price:,.4g}", style={"color": DN, "fontSize": "11px"}),
            html.Td(f"${tp_price:,.4g}", style={"color": UP, "fontSize": "11px"}),
            html.Td(f"${dollar_amount:,.2f}", style={"color": BLUE, "fontWeight": "bold", "fontSize": "11px"}),
            html.Td(f"{leverage:.1f}x", style={"color": lev_color, "fontWeight": "bold", "fontSize": "11px"}),
            html.Td(f"${margin_used:,.2f}", style={"color": MUT, "fontSize": "11px"}),
            html.Td(f"${current_price:,.4g}", style={"color": CYAN, "fontWeight": "bold", "fontSize": "11px"}),
            html.Td(f"${pnl:+,.2f}", style={"color": pnl_color, "fontWeight": "bold", "fontSize": "12px"}),
            html.Td(f"{pnl_pct:+.2f}%", style={"color": pnl_color, "fontWeight": "bold", "fontSize": "11px"}),
            html.Td(
                dbc.Button(
                    "❌ بستن",
...
# ==============================================================================
# ★ جدول نتایج اسکن (بدون PnL — فقط پیش‌بینی)
# ==============================================================================
def build_scan_results_table(signals_data):
    """جدول نتایج اسکن — فقط پیش‌بینی، بدون PnL."""
    if not signals_data:
    ...
    for rank, s in enumerate(signals_data, 1):
        ...
        rows.append(html.Tr([
            html.Td(f"#{rank}", style={"color": GOLD, "fontWeight": "bold", "fontSize": "10px"}),
            html.Td([
                html.Span(s["icon"], style={"marginRight": "3px"}),
            ...
            html.Td(f"{s['final_score']:.1f}", style={"color": score_color, "fontWeight": "bold", "fontSize": "10px"}),
            ...
            html.Td(sl_text, style={"color": DN, "fontSize": "10px"}),
            html.Td(tp_text, style={"color": UP, "fontSize": "10px"}),
            html.Td(rr_text, style={"color": GOLD, "fontWeight": "bold", "fontSize": "10px"}),
            html.Td(status_text, style={"color": status_color, "fontSize": "10px"}),
        ], style={"textAlign": "center", "background": row_bg}))
...
def build_signals_summary(signals_data, total_pnl=0.0, current_balance=DEFAULT_BALANCE, open_pnl=0.0):
    ...
    return dbc.Row([
        dbc.Col(html.Div([
            ...
            html.Div(f"{total}", className="stat-value", style={"color": TXT, "fontSize": "16px"}),
        ...
        dbc.Col(html.Div([
            ...
            html.Div(f"${total_dollar:,.2f}", className="stat-value", style={"color": BLUE, "fontSize": "16px"}),
        ], className="glass-card", style={"padding": "10px", "textAlign": "center"}), md=2),
        dbc.Col(html.Div([
...
app.layout = html.Div([
    html.Div([
        ...
        html.Div("موتور معاملاتی رزونانس چرخه‌های زمانی — طلا 🥇 (XAUUSDT) — Morindok", style={
    ...
    dbc.Tabs([
        ...
        # ===================================================================
        # ★ تب سیگنال‌ها — بازطراحی کامل
        # ===================================================================
        dbc.Tab(label="📡 سیگنال‌ها", tab_id="tab-signals", children=[
            html.Div([
                html.Div("📌 معاملات باز واقعی با PnL زنده (هر ۱۰ ثانیه آپدیت) | نتایج اسکن جداگانه",
    ...
    html.Div(
        "💡 تب سیگنال‌ها: معاملات باز واقعی از دیتابیس با PnL زنده. نتایج اسکن فقط پیش‌بینی هستند و PnL ندارند.",
...
# ==============================================================================
# ★ Callback اسکن — ذخیره در DB و نمایش معاملات باز
# ==============================================================================
@app.callback(
...
def scan_signals(_click, interval, balance, risk_pct, threshold):
    ...
    try:
        ...
        balance, risk_pct_val, threshold_val = DEFAULT_BALANCE, 0.01, 0.35
    ...
    if not signals_data:
        return [], html.Div("خطا در اسکن.", style={"color": DN}), \
    ...
    # ★ خواندن معاملات باز واقعی از دیتابیس
    open_signals = db.get_open_signals()
    live_pnls = calculate_open_pnls_live(db)
    ...
    # ساخت UI
    summary = build_signals_summary(signals_data, total_pnl_closed, current_balance, open_pnl_estimate)
...
# ==============================================================================
# ★ Callback آپدیت PnL زنده معاملات باز — هر ۱۰ ثانیه
# ==============================================================================
@app.callback(
...
def update_positions_pnl_live(_tick, balance, active_tab, scan_data):
    """آپدیت PnL زنده معاملات باز هر ۱۰ ثانیه."""
    ...
    # آپدیت نتایج (TP/SL/انقضا)
    update_open_signals_results(db)

    # خواندن معاملات باز از دیتابیس
    open_signals = db.get_open_signals()
    ...
    # ساخت جدول معاملات باز با PnL زنده
    positions_table = build_open_positions_table(open_signals, live_pnls)

    # ساخت خلاصه
    scan_data = scan_data or []
    summary = build_signals_summary(scan_data, total_pnl_closed, current_balance, open_pnl_estimate)
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("🌌 Temporal Resonance Engine v12")
    print("   ✅ PnL زنده واقعی در تب سیگنال‌ها")
    print("   🥇 طلا (XAUUSDT)")
