...
# -*- coding: utf-8 -*-
"""
🧠 مغز زنده — نسخه 0.20 (تک‌فایلی کامل)
تریدر نهادی فراآگاه | تحلیل فرابعدی | فقط معاملات سودده
...
config.PROJECT_NAME="مغز زنده"; config.VERSION="0.20-institutional"
config.SUBTITLE="تریدر نهادی فراآگاه | تحلیل فرابعدی | فقط معاملات سودده"
...
# 🎯 آستانه‌های بسیار سخت‌گیرانه برای معاملات سودده
config.MIN_CONVICTION=0.75
config.MIN_CONFIDENCE=0.70
config.RISK_REWARD_RATIO=3.0
config.MAX_TRADES_PER_DAY=1
TRADING_MODES={
    "swing":{"label":"🎯 سویینگ نهادی (لوریج ۵)","leverage":5,"tp_mult":3.5,"sl_mult":1.2,
             "conviction_threshold":0.75,"decision_interval":8,"use_geometry":True,
             "margin_fraction":0.3,"max_daily":1,"cost_aware":True},
    "scalp":{"label":"⚡ اسکالپ نهادی (لوریج ۱۰)","leverage":10,"tp_mult":2.0,"sl_mult":0.8,
             "conviction_threshold":0.70,"decision_interval":4,"use_geometry":False,
             "margin_fraction":0.2,"max_daily":2,"cost_aware":True},
}
...
class MultiDimensionalAnalyzer:
    """تحلیل فرابعدی قیمت: ۵ بعد مستقل که باید همزمان تأیید شوند."""
    def __init__(self):
        self.prev_velocity={}
    def analyze(self,o,h,l,c,key=""):
        if c is None or len(c)<20: return {"ready":False}
        n=len(c)
        # بعد ۱: مومنتوم
        ...
        # امتیازدهی فرابعدی
        signal=0.0; reasons=[]; dimensions_confirmed=0
        # بعد ۱: مومنتوم
        if abs(velocity)>0.3:
            signal+=velocity*0.35
            dimensions_confirmed+=1
            if velocity>0.3: reasons.append("مومنتوم صعودی")
            else: reasons.append("مومنتوم نزولی")
        # بعد ۲: روند
        if abs(trend_strength)>0.5:
            signal+=trend_strength*0.30
            dimensions_confirmed+=1
            if trend_strength>0.5: reasons.append("روند صعودی پایدار")
            else: reasons.append("روند نزولی پایدار")
        # بعد ۳: الگوهای کندلی
        if is_strong_bull: signal+=0.25; reasons.append("کندل قوی صعودی"); dimensions_confirmed+=1
        if is_strong_bear: signal-=0.25; reasons.append("کندل قوی نزولی"); dimensions_confirmed+=1
        if bull_rejection: signal+=0.20; reasons.append("ریجکشن از پایین"); dimensions_confirmed+=1
        if bear_rejection: signal-=0.20; reasons.append("ریجکشن از بالا"); dimensions_confirmed+=1
        # بعد ۴: حمایت/مقاومت
        ...
        return {"ready":True,"price":float(c[-1]),"signal":float(signal),
                ...
                "reasons":reasons,"dimensions_confirmed":dimensions_confirmed}
...
class Organism:
    ...
    def _make_signal(self,coin,tk,an,geom,energy,mp,attention):
    ...
    def _trading_step(self,scanner_snapshot):
        ...
        for coin,d in deep.items():
            ...
            sig=self._make_signal(coin,tk,an,geom,energy,mp,attention)
            if sig:
                sig["geometry"]=geom
                signals.append(sig)
        ...
        for sig in signals:
            ...
            # 🎯 فقط معاملات با اطمینان بسیار بالا
    ...
    return html.Div(className="header",children=[
        ...
        html.Div(className="controls",children=[
            dcc.Dropdown(id="mode-selector",options=[
                {"label":"🎯 سویینگ نهادی (لوریج ۵)","value":"swing"},
                {"label":"⚡ اسکالپ نهادی (لوریج ۱۰)","value":"scalp"},
...
def render_mode_info(s):
    ...
    st=s["trade_stats"]
    mode_fa="🎯 سویینگ نهادی" if mode=="swing" else "⚡ اسکالپ نهادی"
    children=[
        ...
        html.Div(f"آستانه اطمینان: {int(mp['conviction_threshold']*100)}٪ | مارجین اسلات: {config.SLOT_MARGIN:.0f}$"),
...
def interact(*args):
    ...
    if trig=="btn-rebirth":
        ORGANISM.rebirth()
        return html.Span("🌌 موجود جدید با ژنوم تریدر نهادی متولد شد.",style={"color":"#a78bfa"})
