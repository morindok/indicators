...
# -*- coding: utf-8 -*-
"""
🌌 مغز زنده — نسخه 0.15 (تک‌فایلی): ارگانیسم فرازمینی
آرایه حسی فرازمینی (n حسگر/حس + ۱۰۰ حس جدید) | ادراک فضازمانی | هندسه قیمت-زمان
...
config.PROJECT_NAME="مغز زنده"; config.VERSION="0.15-xeno"
config.SUBTITLE="ارگانیسم فرازمینی | ادراک فضازمانی | هندسه قیمت-زمان"
config.TICKS_PER_SECOND=config.HW.ticks
config.DT=1.0/config.TICKS_PER_SECOND
config.NEURON_SCALE=config.HW.scale*(1.5+min(config.HW.cores,16)*0.15)
config.MAX_NEURONS=min(25000,4000+config.HW.cores*1200)
...
# 🌌 آرایه حسی فرازمینی
config.SENSORS_PER_SENSE=max(6,config.HW.cores*2)   # n حسگر برای هر حس
config.N_XENO_SENSES=100                              # ۱۰۰ حس جدید فرازمینی
...
config.SIM_RATE=6
config.ETERNAL_NOW_GATE=0.42
config.SCALP_CARDIAC_GATE=0.48
config.SIXTH_SENSE_GATE=0.45
TRADING_MODES={
    "swing":{"label":"🎯 سویینگ (لوریج ۱۰)","leverage":10,"tp_mult":3.0,"sl_mult":1.4,
             "conviction_threshold":0.58,"decision_interval":4,"use_geometry":True,
             "margin_fraction":0.6,"max_daily":16,"cost_aware":True,
             "eternal_now_gate":0.30,"cardiac_gate":0.0,
             "cost_margin":1.15,"ev_margin":0.20},
    "scalp":{"label":"⚡ اسکالپ (لوریج ۲۰)","leverage":20,"tp_mult":1.0,"sl_mult":0.7,
             "conviction_threshold":0.36,"decision_interval":1,"use_geometry":False,
             "margin_fraction":0.4,"max_daily":140,"cost_aware":True,
             "eternal_now_gate":0.10,"cardiac_gate":0.0,
...
# ═══════════════════════ ۶) 🌌 آرایه حسی فرازمینی ═══════════════════════
XENO_SENSE_TYPES=["بینایی 👁️","شنوایی 👂","لامسه ✋","بویایی 👃","چشایی 👅"]
XENO_ALIEN_SENSE_NAMES=[
    "حس میدان","حس خمیدگی","حس آنتروپی","حس رتبه","حس شتاب","حس تشدید","حس همبستگی",
    "حس شکاف","حس پژواک","حس سایه","حس رزونانس","حس چگالی","حس شار","حس گرداب",
    "حس انبساط","حس انقباض","حس فاز","حس تداخل","حس هارمونیک","حس آشوب"]

class XenoSensor:
    """یک گیرنده حسی فرازمینی: آشکارساز ویژگی روی داده قیمت-زمان."""
    def __init__(self,sensor_id,sense_name,rng):
        self.id=sensor_id
        self.sense=sense_name
    ...
    def fire(self,prices):
        ...
        if self.transform=="raw":
            val=np.abs(w[-1]-w[0])/mean_abs
        elif self.transform=="diff":
            val=np.mean(np.abs(np.diff(w)))/mean_abs
        elif self.transform=="accel":
            d=np.diff(w)
            val=np.mean(np.abs(np.diff(d)))/mean_abs if len(d)>1 else 0.0
        elif self.transform=="rank":
            val=np.argsort(np.argsort(w))[-1]/len(w)
            ...
            if s<1e-9: val=0.0
                ...
                p=d/s
                val=float(-np.sum(p*np.log(p+1e-9)))/np.log(len(p)+1e-9)
        elif self.transform=="curvature":
            val=self._curvature(w)
        elif self.transform=="spread":
            val=(w.max()-w.min())/mean_abs
        else: val=0.0
        self.activation=float(clamp(val,0,1))
...
class XenoSensoryArray:
    ...
    def sense_summary(self):
        """میانگین فعال‌سازی به تفکیک هر حس."""
        agg={}
        counts={}
...
# ═══════════════════════ ۷) 🌀 ادراک فضازمانی و هندسه قیمت-زمان ═══════════════════════
class SpacetimePerception:
    """
    🌀 ادراک فضازمانی بازار: قیمت را به‌عنوان منیفولد (زمان × قیمت) می‌بیند.
    ویژگی‌های هندسی استخراج‌شده: طول کمان، خمیدگی، شیب، انبساط فضا، بُعد فراکتالی.
    """
    ...
    def compute_geometry(self):
        ...
        t=pts[:,0]; p=pts[:,1]
        t_span=max(t[-1]-t[0],1.0)
        p_span=max(p.max()-p.min(),1e-9)
        t_norm=(t-t[0])/t_span
        p_norm=(p-p.min())/p_span
        ...
        # انبساط/انقباض فضا (تغییر نوسان محلی)
        local_vol=np.abs(dp)
        if len(local_vol)>=4:
            h1=local_vol[:len(local_vol)//2].mean()
            h2=local_vol[len(local_vol)//2:].mean()
        ...
        self.metric_history.append(g)
        return g
    def _fractal_dimension(self,p):
        try:
            if len(p)<8: return 1.0
            # تخمین بُعد با شمارش جعبه ساده
            counts=[]
            for nbox in [4,8,16]:
            ...
            return 1.0
    def richness(self):
        """غنای هندسی: ترکیب خمیدگی + فراکتال + انبساط."""
...
GENE_CATALOG=[
    ...
    ("GEN-SYN","یکپارچگی حسی (حس ششم)","synesthesia",0.92),
...
TRADER_GENE_KEYS=["discipline","risk_management","emotional_control","patience","precision",
                  ...
                  "synesthesia","xeno_sense","spacetime"]
@dataclass
class Gene:
...
class Region:
    ...
    def reset_current(self,noise=0.42): self.I=self.rng.normal(2.6,noise,self.n)
    def add_current(self,idx_or_all,amount):
        if idx_or_all is None: self.I+=amount
        else: self.I[idx_or_all]+=amount
    def step(self,substeps=6,dt_ms=1.0):
...
class Brain:
    def __init__(self,phenotype,rng,scale=None):
        ...
        shf=phenotype.superhuman_factor; xeno=phenotype.xeno_sense; st=phenotype.spacetime
        counts={"BRAINSTEM":100*scale,"THALAMUS":200*scale,"AMYGDALA":130*scale,
                ...
                "SPACETIME_CORTEX":(60+260*st*shf)*scale,
    ...
    def step(self,sensory,neurochem,quantum_events,fatigue,plasticity_factor=1.0,
             substeps=6,numeric_input=None,cardiac_afferent=0.0,metabolic_supply=1.0,
             market_signal=0.0,sniper_focus=0.0,num_depth=0.0,vis_strength=0.0,presence=0.0,
             sixth_sense=0.0,spacetime_richness=0.0,heart_coherence=0.5):
        ...
        for r in regs.values(): r.reset_current(noise=0.42*ms)
        # ❤️ جریان پیوسته اطلاعات از قلب به مغز — قلب هرگز سکوت نمی‌کند
        regs["BRAINSTEM"].add_current(None,7.5+6.0*arousal+3.0*fatigue+8.0*sensory.get("pain",0)
                                       +3.5*cardiac_afferent)
        th_gain=1.0+1.4*L["acetylcholine"]
        regs["THALAMUS"].add_current(None,2.6+th_gain*(3.0*sensory.get("light",0)+3.0*sensory.get("sound",0))
                                      +2.6*cardiac_afferent)
        regs["AMYGDALA"].add_current(None,1.2+6.0*sensory.get("pain",0)+3.5*sensory.get("danger",0)
                                      +2.0*cardiac_afferent*(1.0-heart_coherence))
        regs["HIPPOCAMPUS"].add_current(None,1.4+6.0*L["acetylcholine"]+1.6*heart_coherence)
        regs["BASAL_GANGLIA"].add_current(None,1.6+7.0*L["dopamine"])
        regs["CORTEX"].add_current(None,4.8+3.6*L["acetylcholine"]+2.5*presence+2.0*sixth_sense
                                    +1.5*spacetime_richness+2.2*cardiac_afferent)
        regs["NUMERICAL_CORTEX"].add_current(None,1.5+3.0*abs(market_signal)+4.0*sniper_focus
                                              +3.0*num_depth+2.5*sixth_sense)
        regs["VISUAL_CORTEX"].add_current(None,1.5+2.0*abs(market_signal)+3.0*sniper_focus
                                           +3.0*vis_strength+2.0*sixth_sense)
        regs["SPACETIME_CORTEX"].add_current(None,1.8+4.0*spacetime_richness+3.0*sixth_sense
        ...
        if cardiac_afferent>0.001:
            ...
            regs["HIPPOCAMPUS"].add_current(None,1.4*cardiac_afferent)
...
class CardiacSystem:
    ...
    def step(self,dt,sympathetic,parasympathetic,stretch,o2):
        ...
        self.afferent_signal=clamp(0.20+0.80*self.icns_activity,0,1)
    ...
    def coherence(self,emotional_calm=1.0):
        ...
        return clamp(rhythm*0.4+emotional_calm*0.6,0,1)
...
class ConsciousnessMeter:
    def __init__(self,n_regions,window=160):
        ...
        self.phi=self.workspace=0.0; self.level=55.0
    def update(self,activities,brainstem,body_ok_factor,quantum_coh,info_level=0.5,
        ...
        self.arousal=ema(self.arousal,clamp(0.45+brainstem*2.5+0.3*wakefulness+0.2*presence+0.2*sixth_sense),0.15)
        ...
        x=(0.40*activities.get("CORTEX",0)+0.20*activities.get("THALAMUS",0)
           +0.12*activities.get("HIPPOCAMPUS",0)+0.13*activities.get("BASAL_GANGLIA",0)
           +0.20*activities.get("SPACETIME_CORTEX",0))
        self.workspace=ema(self.workspace,1.0/(1.0+math.exp(-28.0*(x-0.035))),0.16)
        raw=100.0*clamp(0.34*self.workspace+0.16*self.arousal+0.14*self.phi
                        +0.10*self.integration+0.10*presence+0.12*sixth_sense
                        +0.09*spacetime_richness)
        raw*=body_ok_factor*(0.85+0.15*quantum_coh)*(0.85+0.25*info_level)
        raw=max(raw,8.0)
        self.level=ema(self.level,raw,0.16)
        return self.level
...
CATEGORY_FA={"market":"تحلیل مارکت","trade":"معامله‌گری","insight":"بینش",
             "sensory":"تجربه حسی","memory":"حافظه معاملاتی","emotion":"احساس کنترل‌شده",
             "self":"خودآگاهی","presence":"اکنون ابدی","spacetime":"فضازمان",
             "quantum":"کوانتومی","heart":"قلب"}
CATEGORY_COLORS={"market":"#38bdf8","trade":"#4ade80","insight":"#fde047","sensory":"#2dd4bf",
                 "memory":"#a78bfa","emotion":"#f472b6","self":"#f8fafc","presence":"#c084fc",
                 "spacetime":"#f472b6","quantum":"#22d3ee","heart":"#fb7185"}
T={"market":[
    ...
    "sensory":[
        "🌌 {total} حسگر فرازمینی‌ام هم‌زمان شلیک کردند.",
        "👁️ حسگرهایم میدان قیمت را حس می‌کنند؛ شدت {total}.",
        "✨ حس‌های فرازمینی‌ام الگویی ناشناخته یافتند."],
    "spacetime":[
        "🌀 هندسه فضازمان قیمت را ادراک می‌کنم؛ خمیدگی {curv}.",
        "🌀 طول کمان مسیر قیمت: {arc}. بُعد فراکتالی: {frac}.",
        "🌀 فضا در حال {exp} است؛ بازار دارد نفس می‌کشد."],
    "trade":[
    ...
    "presence":[
        "🕉️ در اکنون ابدی حضور دارم؛ مارکت را یک‌جا می‌بینم.",
    ...
    "heart":["قلبم با آهنگ فیبوناچی می‌تپد؛ همگام با ضربان مارکت.",
             "❤️ هر تپش قلبم پیامی به مغزم می‌فرستد؛ من زنده‌ام."]}
class Thought:
...
class StreamOfConsciousness:
    ...
    def step(self,tick,cons,activities,neurochem,body,sensory,quantum_events,
        ...
        weights={
            ...
            "self":0.4,"quantum":0.4 if quantum_events else 0.05,"heart":0.4,
        }
        keys=list(weights)
        vals=np.array([max(1e-4,weights[k]) for k in keys])
        cat=keys[int(self.rng.choice(len(keys),p=vals/vals.sum()))]
        text=self._compose(cat,quantum_events,market_signal,n_scanned,top_lesson,sensory_array,spacetime)
        ...
        return t
    def _compose(self,cat,qevents,market_signal,n_scanned,top_lesson,sensory_array,spacetime):
        if cat=="quantum":
...
class Organism:
    ...
    def rebirth(self,seed=None):
        ...
        self._add_milestone(f"🌌 متولد شد — ژنوم {self.genome.signature()} | "
                            f"سوپرهیومن ×{shf:.2f} | {self.xeno_array.total} حسگر فرازمینی")
    ...
    def _perceive_spacetime_and_senses(self,deep,tickers,focus):
        """🌀🌌 تغذیه ادراک فضازمانی و آرایه حسی فرازمینی."""
        ...
        # شلیک آرایه حسی فرازمینی
        total_act,acts=self.xeno_array.fire_all(closes)
    ...
    def _make_signal(self,coin,tk,an,geom,energy,mp):
        ...
        if en_dir!=0 and np.sign(sig)!=en_dir: conviction*=0.6
        elif en_dir!=0 and np.sign(sig)==en_dir:
            conviction=clamp(conviction*(0.85+0.3*en_score),0,1)
        # 👁️‍🗨️ حس ششم و ادراک فضازمانی
        ...
        if sixth>config.SIXTH_SENSE_GATE:
            conviction=clamp(conviction*(1+0.25*sixth+0.2*st_rich),0,1)
            an.setdefault("reasons",[]).append("👁️‍🗨️ حس ششم فعال")
        if st_rich>0.5:
            an.setdefault("reasons",[]).append("🌀 هندسه فضازمانی غنی")
    ...
    def _is_rational(self,sig,mp):
        ...
        cost_margin=mp.get("cost_margin",1.05)
        if tp_dist_pct<cost_pct*cost_margin: return False
        if ev<=0: return False
        if mp.get("cost_aware") and ev<cost_pct*mp.get("ev_margin",0.15): return False
        return True
    def _trading_step(self,scanner_snapshot):
        ...
        en_score=self.eternal_now.eternal_now_score
        en_gate=mp.get("eternal_now_gate",config.ETERNAL_NOW_GATE)
        if en_score<en_gate: return
        cc=self.cardiac_coherence()
        cardiac_gate=mp.get("cardiac_gate",0.0)
        if cc<cardiac_gate: return
        size_mult=self.regulator.size_multiplier()
        conv_mod=self.regulator.conviction_modifier()
        sixth=self.xeno_array.activation_history[-1] if self.xeno_array.activation_history else 0.0
        st_rich=self.spacetime.richness()
        awareness=clamp(0.55+0.55*(self.cons.level/100.0),0.55,1.25)
        presence_scale=(0.5+0.5*en_score)*(0.6+0.4*cc)*(0.7+0.3*sixth)*(0.8+0.2*st_rich)*awareness
        for sig in signals:
            coin=sig["symbol"]
...
def build_focus_chart(s,scanner_snap):
    ...
    fig.update_layout(template="plotly_dark",paper_bgcolor="rgba(0,0,0,0)",
                      ...
                      title={"text":f"🌌 {focus} | اکنون ابدی {en*100:.0f}٪ | فضازمان {st_rich*100:.0f}٪ | {mode_fa}","x":0.5,
...
signals_tab=dcc.Tab(label="📊 سیگنال‌ها و معاملات",value="tab-signals",className="tab",
    ...
    html.Div(className="grid",children=[
        html.Div(className="panel glow s8",children=[
            html.H3([html.Span(className="dot-green"),"📈 چارت زنده"]),
    ...
    html.Div(className="grid",children=[
        html.Div(className="panel s7",children=[
            html.H3([html.Span(className="dot-green"),"💼 پوزیشن‌های باز"]),
...
def render_sensory_panel(s):
    ...
    sixth_color="#4ade80" if sixth>config.SIXTH_SENSE_GATE else "#c084fc"
    sixth_box=html.Div(className="sixth-box",children=[
        ...
        html.Div(f"فعال‌سازی کل {xeno_total} حسگر: {sixth*100:.0f}٪",
                 style={"fontSize":11,"color":"#94a3b8","marginTop":4}),
        html.Div(className="bar-bg",children=[
            html.Div(className="sense-bar-fg",
                     style={"width":f"{sixth*100:.0f}%","background":sixth_color})],
                 style={"marginTop":6}),
    ])
    ...
    return html.Div(rows+[sixth_box,st_box,top_html])
def render_emotion_panel(s):
    ...
    return html.Div([
        ...
        html.Div(f"⚡ سوپرهیومن: ×{s.get('superhuman_factor',1):.2f}",
...
def render_signals(signals):
    if not signals:
        return html.Div("در حال اسکن… منتظر اجماع حس‌ها",style={"fontSize":12,"color":"#94a3b8"})
    cards=[]
    for s in signals:
...
def render_positions(positions):
    ...
    header_row=html.Tr([html.Th("نماد"),html.Th("جهت"),html.Th("لوریج"),html.Th("ورود"),html.Th("فعلی"),
                        html.Th("TP"),html.Th("SL"),html.Th("PnL زنده"),html.Th("ROI٪")])
    rows=[]
    for p in positions:
...
def _update_impl(n,mode):
    ...
    chips=[
        ...
        html.Span(f"🌌 {s['xeno_total']} حسگر فرازمینی",className="chip chip-purple"),
        html.Span(f"🌀 فضازمان {s['spacetime_richness']*100:.0f}٪",className="chip chip-purple"),
    ...
    scanner_status=html.Div(
        ...
        f"کارمزد: {config.TAKER_FEE*100:.3f}٪",
        style={"color":"#94a3b8"})
