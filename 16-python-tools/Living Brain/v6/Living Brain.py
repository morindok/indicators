...
# -*- coding: utf-8 -*-
"""
🧠 مغز زنده — نسخه ۱.۱ «تکینگیِ شکارچی» (تک‌فایلی کامل)
================================================================
🔮 شهود‌محور مطلق | ⚡ تفکر باینری صریح | 🍽️ گرسنگی معامله (تضمین تصمیم)
🖥 GPU/CPU بهینه | بدون هیچ اندیکاتور سنتی | بدون خطا
...
config.PROJECT_NAME = "مغز زنده"; config.VERSION = "1.1-hunter"
...
# 🔥 دروازه‌های متعادل (نسخه شکارچی)
config.INTUITION_GATE = 0.40          # آستانه پایه شهود
config.BINARY_PROBES = 14
config.BINARY_MIN_RATIO = 0.50        # حداقل ۷ از ۱۴
config.BINARY_FAST_LOCK_N = 3         # قفل سریع ۳ از 
config.BINARY_STRONG_RATIO = 0.65
config.HUNGER_MAX = 0.18              # حداکثر افت آستانه بر اثر گرسنگی
config.HUNGER_RAMP_TICKS = 480        # ~۸۰ ثانیه بی‌پوزیشن → گرسنگی کامل
TRADING_MODES = {
...
# ═══════════════════════ ۷) تحلیلگر میدان غیرعلی ═══════════════════════
class AcausalFieldAnalyzer:
    """تحلیل کاملاً غیرسنتی: آنتروپی جایگشت، فشردگی، طیف، هرست، خمیدگی، شار اطلاعاتی."""
...
# ═══════════════════════ ۱۵) هسته شهود (ترکیب نرم) ═══════════════════════
class IntuitionCore:
    """🔮 نسخه شکارچی: ترکیب هندسی+حسابی با کف ۰.۱۵ — دیگر یک حسِ بد کل شهود را نابود نمی‌کند."""
    ...
    def fuse(self, field_dir, field_conv, features, econ, sixth, st_rich, st_expansion,
        ...
        def align(v): return clamp(0.5+0.5*v*direction, 0.15, 1.0)
        comps = {
            "field": max(0.15, align(field_dir)*(0.5+0.5*field_conv)),
            "order": align((0.6-features.get("perm_entropy", 0.5))*2),
            "memory": align((features.get("hurst", 0.5)-0.5)*3),
            "econ": max(0.15, align(econ.get("pressure", 0))*(0.4+0.6*econ.get("heat", 0))),
            "xeno": max(0.15, align(1 if sixth > config.SIXTH_SENSE_GATE else -0.2)*(0.3+0.7*sixth)),
            "spacetime": max(0.15, align(st_expansion*direction)*(0.3+0.7*st_rich)),
            "eternal": max(0.15, align(eternal_dir*direction)*max(0.3, eternal_score)),
        ...
        gm = math.exp(sum(weights[k]*math.log(comps[k]) for k in comps)/W)
        am = sum(weights[k]*comps[k] for k in comps)/W
        base = 0.5*gm+0.5*am
        intuition = clamp(base*(0.9+0.25*shf)*(0.85+0.3*self.p.intuition_core)
                          *(0.75+0.5*field_conv), 0, 1)
...
# ═══════════════════════ ۱۶) جریان تفکر باینری (متعادل) ═══════════════════════
class BinaryCognitionStream:
    """⚡ ۱۴ کاوشگر متعادل؛ اجماع ≥۵۰٪ + قفل سریع ۳/۴ یا اجماع ≥۶۵٪."""
    def __init__(self, rng):
        self.rng = rng; self.history = deque(maxlen=16)
        self.total_cascades = 0; self.total_triggers = 0
    def cascade(self, direction, ctx):
        t0 = time.perf_counter()
        f = ctx.get("features", {}); econ = ctx.get("econ", {})
        probes = [
            ("شار اطلاعاتی هم‌سو", ctx.get("field_dir", 0)*direction > 0.05),
            ("نظم الگویی (آنتروپی)", f.get("perm_entropy", 1) < 0.78),
            ("حافظه هرست", f.get("hurst", 0.5) > 0.47),
            ("چرخه طیفی", f.get("spectral_entropy", 1) < 0.85),
            ("ساختار فشرده", f.get("complexity", 1) < 0.85),
            ("خمیدگی هم‌جهت", f.get("curvature", 0)*direction > -0.2),
            ("فشار اقتصادی", econ.get("pressure", 0)*direction > -0.05),
            ("گرمای نقدینگی", econ.get("heat", 0) > 0.35),
            ("حس ششم", ctx.get("sixth", 0) > 0.18),
            ("فضازمان غنی", ctx.get("st_rich", 0) > 0.20),
            ("اکنون ابدی هم‌سو", ctx.get("eternal_dir", 0)*direction >= 0),
            ("همدلی قلب", ctx.get("cardiac_coh", 0) > 0.30),
            ("انسجام کوانتومی", ctx.get("quantum_coh", 0) > 0.15),
            ("احتمال ریاضی", (ctx.get("math_prob", 0.5)-0.5)*direction > -0.05),
        ]
        ...
        passed = ratio >= config.BINARY_MIN_RATIO and (fast_lock or ratio >= config.BINARY_STRONG_RATIO)
...
class Genome:
    ...
    def build_phenotype(self):
        ...
        for key in ["discipline", "risk_management", "emotional_control", "patience", "precision",
                    "numerical_iq", "visual_iq", "adaptability", "calm", "foresight", "presence",
                    "synesthesia", "xeno_sense", "spacetime", "interoception", "attention",
                    "cardiac_neural", "intuition_core", "binary_cognition", "economic_sense",
                    "acausal_field", "gpu_sync"]:
...
class Organism:
    ...
    def _trading_step(self, scanner_snapshot):
        ...
        # 🍽️ گرسنگی معامله: بی‌پوزیشنِ طولانی → افت هوشمندانه آستانه‌ها
        ...
        for sig in signals:
            ...
            ctx = {"symbol": coin, "tick": self.tick, "features": sig["features"], "econ": sig["econ"],
                   ...
                   "quantum_coh": self.quantum.avg_coherence, "math_prob": self.math_mind.bayes_prob}
            ...
            if avail < 5: continue
            margin = min(avail*mp["margin_fraction"]*size_mult*presence_scale
                         *(0.7+0.3*cascade["ratio"]), avail)
...
def build_focus_chart(s, scanner_snap):
    ...
    fig.update_layout(template="plotly_dark", paper_bgcolor="rgba(0,0,0,0)",
                      ...
                      title={"text": f"🌌 {focus} | شهود {intuition*100:.0f}٪ | اکنون ابدی {en*100:.0f}٪ | {mode_fa}",
...
def render_binary_panel(s):
    ...
    summary = html.Div(f"📊 کل آبشارها: {s.get('binary_cascades', 0)} | تصمیم‌های صادرشده: {s.get('binary_triggers', 0)}",
...
if __name__ == "__main__":
    print(f"🌌 {config.PROJECT_NAME} v{config.VERSION} — تکینگیِ شکارچی")
    print(f"🖥 {GPU.label}")
    print(f"🧠 نورون‌ها با مقیاس ×{config.NEURON_SCALE:.2f} | سقف {config.MAX_NEURONS:,} | موازی: {config.PARALLEL_BRAIN}")
    print(f"🔮 دروازه شهود: {int(config.INTUITION_GATE*100)}٪ | ⚡ اجماع باینری: {int(config.BINARY_MIN_RATIO*100)}٪ | 🍽️ گرسنگی تا {int(config.HUNGER_MAX*100)}٪")
    print(f"💵 هر موتور: {config.INITIAL_CAPITAL}$ بین {config.N_SLOTS} ارز")
    print("   آدرس: http://127.0.0.1:8060")
    try:
