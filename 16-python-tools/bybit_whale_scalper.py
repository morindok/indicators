    ...
    with _DB_LOCK:
        ...
        con.execute("""
            INSERT INTO signals (id, ts_open, symbol, category, side, entry, sl, tp, leverage, qty,
                                  notional, capital, risk_usd, buy_usd, sell_usd, whale_count, status)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?, 'OPEN')
        """, (sig["id"], sig["ts_open"], sig["symbol"], sig["category"], sig["side"], sig["entry"],
              sig["sl"], sig["tp"], sig["leverage"], sig["qty"], sig["notional"], sig["capital"],
              sig["risk_usd"], sig["buy_usd"], sig["sell_usd"], sig["whale_count"]))
        con.commit()
        con.close()


def db_close_signal(sig_id, status, close_price, pnl_pct):
    with _DB_LOCK:
        con = sqlite3.connect(DB_PATH)
        con.execute("""
            UPDATE signals SET status=?, ts_close=?, close_price=?, pnl_pct=? WHERE id=?
        """, (status, datetime.now(timezone.utc).isoformat(timespec="seconds"), close_price, pnl_pct, sig_id))
        con.commit()
        con.close()
...
class WhaleMonitor:
    """
    یک نخ پس‌زمینه که به استریم Public Trade بایبیت وصل می‌شود، تیک‌های نهنگ (معاملات
    بزرگ‌تر از آستانه‌ی دلاری) را می‌شمارد، عدم‌تعادل خرید/فروش را در یک پنجره‌ی زمانی
    متحرک محاسبه می‌کند و در صورت عبور از آستانه، سیگنال اسکلپ صادر و در دیتابیس ثبت می‌کند.
    همچنین سیگنال‌های باز را در برابر قیمت زنده چک می‌کند تا برد/باخت آن‌ها مشخص شود.
    """
    ...
    def __init__(self, symbol, category, whale_usd, window_sec, min_whale_count,
        ...
        self._ticks = deque()  # (epoch_seconds, side('Buy'/'Sell'), usd_value)
        ...
        self._open_signals = {}  # id -> dict (in-memory mirror of OPEN rows for fast checks)

        for _, row in db_fetch_open().iterrows():
            if row["symbol"] == self.symbol:
                self._open_signals[row["id"]] = row.to_dict()
        ...
        with self._lock:
            for t in trades:
                ...
                if usd_val >= self.whale_usd:
                    self._ticks.append((now, side, usd_val))
                    if side == "Buy":
                        self._cum_buy += 1
                    else:
                        self._cum_sell += 1
            ...
            self._check_open_signals(self._last_price)
    ...
    # -------------------------- Signal engine --------------------------
    def _check_signal(self, now):
        if now < self._cooldown_until:
            return
        buy_usd = sum(v for _, s, v in self._ticks if s == "Buy")
        sell_usd = sum(v for _, s, v in self._ticks if s == "Sell")
        count = len(self._ticks)
        total = buy_usd + sell_usd
        if count < self.min_whale_count or total <= 0 or self._last_price is None:
            return
        ...
        if buy_ratio >= self.imbalance_ratio:
            side = "LONG"
        elif buy_ratio <= (1 - self.imbalance_ratio):
            side = "SHORT"
        if side is None:
            return
        ...
        sl_distance_pct = self.sl_pct / 100.0
        ...
        self._open_signals[sig["id"]] = sig
        self._last_signal = sig
        self._cooldown_until = now + self.cooldown_sec

    def _check_open_signals(self, price):
        if price is None or not self._open_signals:
            return
        closed_ids = []
        for sig_id, sig in self._open_signals.items():
            side = sig["side"]
            entry = sig["entry"]
            sl = sig["sl"]
            tp = sig["tp"]
            ...
            if hit:
                ...
                db_close_signal(sig_id, hit, price, pnl_pct)
                closed_ids.append(sig_id)
        for sig_id in closed_ids:
            self._open_signals.pop(sig_id, None)
    ...
    def get_state(self):
        with self._lock:
            buy_usd = sum(v for _, s, v in self._ticks if s == "Buy")
            sell_usd = sum(v for _, s, v in self._ticks if s == "Sell")
            window_buy_count = sum(1 for _, s, _ in self._ticks if s == "Buy")
            window_sell_count = sum(1 for _, s, _ in self._ticks if s == "Sell")
            return {
                "connected": self._connected,
                "last_price": self._last_price,
                "window_buy_count": window_buy_count,
                "window_sell_count": window_sell_count,
                "window_buy_usd": buy_usd,
                "window_sell_usd": sell_usd,
...
def _signal_card(sig, running, symbol):
    ...
    if not sig:
        return dbc.Alert(f"در انتظار عدم‌تعادل کافی در تیک‌های نهنگ برای {symbol}...", color="secondary", style={"fontSize": 13})

    side_color = UP if sig["side"] == "LONG" else DN
    side_fa = "لانگ (خرید) 🟢" if sig["side"] == "LONG" else "شورت (فروش) 🔴"
    return dbc.Card(dbc.CardBody([
        html.Div(f"⚡ سیگنال اسکلپ فعال — {sig['symbol']}", style={"color": GOLD, "fontWeight": "bold", "marginBottom": 6}),
        ...
        html.Hr(style={"borderColor": LINE, "margin": "8px 0"}),
        html.Div(f"مبنای سیگنال: {sig['whale_count']} تیک نهنگ در پنجره | خرید ${_fmt(sig['buy_usd'])} / فروش ${_fmt(sig['sell_usd'])}",
...
def refresh_whale_ui(n, config):
    ...
    if running:
        ...
        cd_s = f"⏳ خنک‌سازی: {st['cooldown_remaining']} ثانیه" if st["cooldown_remaining"] > 0 else "✅ آماده صدور سیگنال"
        sig_card = _signal_card(st["last_signal"], running, symbol)
    else:
