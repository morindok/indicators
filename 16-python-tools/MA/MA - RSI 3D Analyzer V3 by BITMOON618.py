# -*- coding: utf-8 -*-
"""
🌙 BITMOON INSTITUTIONAL 3D TREND TERMINAL — v2 (اصلاح‌شده + اعتبارسنجی تاریخی)
----------------------------------------------------------------------
تغییرات نسبت به نسخه قبل:
 1) fast_all_rsi برداری‌شده (بدون ساخت ۲۰۰ شیء pandas EWM جدا) → سریع‌تر.
 2) تشخیص واگرایی واقعی مبتنی بر سوئینگ (local extrema با scipy) به‌جای تقسیم دو نیمه.
 3) ماژول Backtest واقعی: نرخ برد و بازده تاریخی هر «قفل سیگنال» روی داده گذشته
    محاسبه می‌شود (نه یک فرمول دلبخواهی). این تنها راه علمی برای ادعای «نهادی» است.
 4) conf (اطمینان) اکنون از ترکیب انحراف CTI از ۵۰ + نرخ برد تاریخی همان تایم‌فریم می‌آید.
 5) تب جدید «اعتبارسنجی تاریخی» در UI که آمار backtest هر تایم‌فریم را نشان می‌دهد.

⚠️ توجه علمی مهم: حتی با این اصلاحات، این سیستم یک اندیکاتور ترکیبی heuristic است،
نه یک مدل آماری اثبات‌شده. نتایج backtest داخلی روی داده‌ی محدود (چند صد کندل) هستند و
جای walk-forward validation روی چند رژیم بازار متفاوت و مدیریت ریسک واقعی را نمی‌گیرند.
پیش از استفاده با پول واقعی، حتماً با دیتای تاریخی بیشتر و آزمون out-of-sample بسنجید.

pip install dash dash-bootstrap-components plotly pandas numpy requests scipy
"""
...
def fast_all_rsi(prices, P=200):
    """
    نسخه برداری Wilder RSI برای دوره‌های ۱..P هم‌زمان.
    به‌جای ساخت P شیء pandas EWM جدا (overhead بالا)، بازگشت نمایی را با
    آرایه‌های numpy روی محور «دوره» انجام می‌دهیم و فقط روی محور زمان لوپ می‌زنیم.
    خروجی دقیقاً هم‌ارز نسخه قبلی است (همان فرمول)، فقط سریع‌تر.
    """
    n = len(prices)
    diffs = np.diff(prices, prepend=prices[0])
    gains = np.clip(diffs, 0.0, None)
    losses = np.clip(-diffs, 0.0, None)
    periods = np.arange(1, P + 1)
    alphas = 1.0 / periods  # shape (P,)

    avg_gain = np.full(P, gains[0], dtype=float)
    avg_loss = np.full(P, losses[0], dtype=float)
...
def _rsi_single(prices, p):
    """RSI تک‌دوره‌ای ساده (برای واگرایی) — دقیقاً معادل فرمول اصلی."""
...
def detect_divergence(prices, rsi_vals, order=3, lookback=100):
    """
    تشخیص واگرایی مبتنی بر سوئینگ‌های واقعی (local extrema) — نسخه اصلاح‌شده.
    نسخه قبلی سری را به دو نیمه ثابت تقسیم می‌کرد که باعث خطای زیاد در تشخیص می‌شد.
    اینجا از argrelextrema برای پیدا کردن کف/سقف‌های واقعی استفاده می‌شود و آخرین
    دو سوئینگ هم‌نوع با هم مقایسه می‌شوند.
    """
    ...
    if len(lows) >= 2:
        i1, i2 = lows[-2], lows[-1]
        if p[i2] < p[i1] - 1e-9 and r[i2] > r[i1] + 1e-9:
            div = "bull"
    if len(highs) >= 2:
        j1, j2 = highs[-2], highs[-1]
        if p[j2] > p[j1] + 1e-9 and r[j2] < r[j1] - 1e-9:
            if div is None or highs[-1] > lows[-1]:  # نزدیک‌ترین سوئینگ به زمان حال اولویت دارد
                div = "bear"
    return div
...
# ------------------------------------------------------------------------------
# 2b) ماژول Backtest واقعی — این بخش جدید و کلیدی است
# ------------------------------------------------------------------------------
def backtest_chain_signal(closed_prices, cti, changes, horizon=10):
    """
    برای هر تغییر حالت سیگنال (خرید/فروش قفل‌شده) در تاریخچه، بازده k کندل بعد را
    اندازه می‌گیرد و نرخ برد / میانگین بازده / ضریب سود را حساب می‌کند.
    این جای فرمول دلبخواهی conf را می‌گیرد و شواهد واقعی می‌دهد.
    """
    n = len(closed_prices)
    trades = []
    for idx, s in changes:
        if s == 0 or idx + horizon >= n:
            continue
        entry = closed_prices[idx]
        exitp = closed_prices[idx + horizon]
        ret = (exitp - entry) / entry * 100.0
        directional_ret = ret if s == 1 else -ret
        trades.append(directional_ret)
    if not trades:
    ...
    return dict(n=len(trades), win_rate=win_rate, avg_ret=avg_ret, profit_factor=profit_factor)

def analyze_tf(df, bt_horizon=10):
    cl = closed_only(df)
    pc = cl["close"].values
    ...
    rsi14_series = _rsi_single(pc, 14)
    div = detect_divergence(pc, rsi14_series)

    bt = backtest_chain_signal(pc, cti, changes, horizon=bt_horizon)
    ad_hoc = min(99, abs(cti_l - 50) * 2 + (8 if (div == "bull" and state == 1) or (div == "bear" and state == -1) else 0))
    if bt["win_rate"] is not None:
        # اطمینان نهایی = میانگین وزنی فرمول لحظه‌ای و نرخ برد تاریخی واقعی
        conf = int(round(0.4 * ad_hoc + 0.6 * bt["win_rate"]))
    else:
        conf = int(round(ad_hoc))
    ...
    return dict(cur=cur, cti=cti_l, tii=tii_l, rii=rii_l, reg=reg, rs=rs, state=state,
                ...
                rsi14=float(R[13, -1]), bt=bt,
                series=dict(tii=tii, rii=rii, cti=cti, ts=cl["ts"], changes=changes))
...
# ==============================================================================
# 4) نمودارهای 3D + پنل زنجیره (تم تیره)
# ==============================================================================
    ...
    return html.Div([
        html.Div([html.Span(emoji, style={"fontSize": 32, "marginLeft": 12}),
                  html.Div([html.Div(f"روند کلی بازار: {reg}", style={"fontSize": 19, "fontWeight": "bold", "color": col}),
                            html.Div([html.Span(f"اطمینان لحظه‌ای {conf}% | اجماع: {bulls} صعودی / {bears} نزولی | "),
        ...
        html.Div("⚠️ اطمینان لحظه‌ای فقط یک شاخص هیوریستیک است، نه احتمال آماری. برای اعتبار واقعی به تب «اعتبارسنجی تاریخی» مراجعه کنید.",
                 style={"fontSize": 10, "color": MUT, "marginTop": 8, "textAlign": "center"})],
...
def build_backtest_table(rows, horizon):
    th = lambda t, w: html.Th(t, style={"width": w, "padding": "8px 3px", "fontSize": 11, "background": GOLD, "color": "#0b1220"})
    trs = []
    for tf in TIMEFRAMES:
        a = rows.get(tf)
        if a is None: continue
        bt = a["bt"]
        td = lambda ch, c=TXT: html.Td(ch, style={"padding": "5px 3px", "fontSize": 11, "textAlign": "center", "color": c,
                                                   "borderBottom": f"1px solid {LINE}"})
        if bt["n"] == 0 or bt["win_rate"] is None:
            trs.append(html.Tr([td(TF_NAMES[tf]), td("داده کافی نیست", MUT), td("—"), td("—"), td("—")]))
            continue
        wc = UP if bt["win_rate"] >= 50 else DN
        pfc = UP if bt["profit_factor"] >= 1 else DN
        trs.append(html.Tr([
            td(TF_NAMES[tf]), td(str(bt["n"])),
            td(f"{bt['win_rate']:.0f}%", wc),
            td(f"{bt['avg_ret']:+.2f}%", UP if bt["avg_ret"] >= 0 else DN),
            td(f"{bt['profit_factor']:.2f}", pfc)]))
    return html.Div([
        html.Div(f"محاسبه‌شده روی داده تاریخی موجود (تا ۵۰۰ کندل)، افق پیش‌رو {horizon} کندل. n = تعداد سیگنال‌های تاریخی.",
                 style={"fontSize": 11, "color": MUT, "marginBottom": 8, "textAlign": "center"}),
        html.Table([html.Thead(html.Tr([th("TF", "12%"), th("تعداد سیگنال (n)", "20%"), th("نرخ برد", "20%"),
                                        th("میانگین بازده", "24%"), th("ضریب سود", "24%")])), html.Tbody(trs)],
                   style={"width": "100%", "tableLayout": "fixed", "borderCollapse": "collapse", "background": CARD,
                          "borderRadius": 10, "overflow": "hidden", "border": f"1px solid {LINE}"}),
        html.Div("⚠️ نمونه تاریخی محدود است (حداکثر ~۵۰۰ کندل هر تایم‌فریم). این آمار برای قضاوت اولیه است، نه اثبات نهایی سودآوری. "
                 "برای اعتبار واقعی لازم است روی بازه‌های زمانی و رژیم‌های بازار متفاوت آزمایش شود.",
                 style={"fontSize": 10, "color": MUT, "marginTop": 10, "textAlign": "center"})
    ])

# ==============================================================================
# 6) اپ Dash — RTL کامل + تم تیره
# ==============================================================================
app = dash.Dash(__name__, external_stylesheets=[dbc.themes.DARKLY], title="BITMOON Terminal v2")
...
app.layout = html.Div([
    dbc.Navbar(color="dark", dark=True, style={"background": CARD, "borderBottom": f"2px solid {GOLD}"},
        ...
        dbc.NavbarBrand("🌙 BITMOON INSTITUTIONAL TERMINAL v2", style={"fontWeight": "bold", "color": GOLD}),
    ...
    dbc.Card(dbc.CardBody([dbc.Row([
        ...
        dbc.Col([html.Label("افق Backtest (کندل):", style={"fontSize": 12, "color": MUT}),
                 dcc.Dropdown(id="bt-horizon-dropdown", value=10, clearable=False,
                              options=[{"label": str(v), "value": v} for v in [5, 10, 20, 30]])], md=3),
    ...
    dbc.Card(dbc.CardBody(dbc.Tabs(id="tabs", active_tab="tab-ma", children=[
        ...
        dbc.Tab(label="🧪 اعتبارسنجی تاریخی", tab_id="tab-bt", children=html.Div(id="backtest-wrap", style={"padding": 20}))])),
...
@app.callback(Output("ma-3d-chart", "figure"), Output("rsi-3d-chart", "figure"),
              ...
              Output("backtest-wrap", "children"), Output("status-msg", "children"),
...
def update(n_clicks, n_int, active, ma_mode, bt_horizon, symbol, interval_3d):
    ...
    if not data:
        return (go.Figure(), go.Figure(), go.Figure(), html.Div(), html.Div(), html.Div(), html.Div(), "❌ اتصال برقرار نشد.")
    ...
    rows = {tf: analyze_tf(df, bt_horizon=bt_horizon) for tf, df in data.items()}
    ...
    fig_rsi = build_rsi_3d(df_3d, symbol, interval_3d, "value") if active == "tab-rsi" else dash.no_update
    return (fig_ma, fig_rsi, build_integrity(a, symbol, interval_3d), kpi_cards(a),
            ...
            build_backtest_table(rows, bt_horizon),
